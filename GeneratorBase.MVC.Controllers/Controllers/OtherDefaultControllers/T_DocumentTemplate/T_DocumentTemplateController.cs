...
{
/// <summary>A partial controller class for Document Template actions (helper methods and other actions).</summary>
///
/// <remarks></remarks>
public partial class T_DocumentTemplateController : BaseController
{
    ...
    private void LoadViewDataForCount(T_DocumentTemplate t_documenttemplate, string AssocType)
    {
    }
    /// <summary>Loads view data after on edit.</summary>
    ///
    /// <param name="t_documenttemplate">The Document Template.</param>
    private void LoadViewDataAfterOnEdit(T_DocumentTemplate t_documenttemplate)
    {
        LoadViewDataBeforeOnEdit(t_documenttemplate, false);
        CustomLoadViewDataListAfterEdit(t_documenttemplate);
    }
    /// <summary>Loads view data before on edit.</summary>
    ///
    /// <param name="t_documenttemplate">         The Document Template.</param>
    /// <param name="loadCustomViewData">(Optional) True to load custom view data.</param>
    private void LoadViewDataBeforeOnEdit(T_DocumentTemplate t_documenttemplate, bool loadCustomViewData = true)
    {
        var RoleList = (new GeneratorBase.MVC.Models.CustomRoleProvider()).GetAllRoles().ToList();
        RoleList.Add("All");
        ViewBag.RoleList = new SelectList(RoleList, "", "");
        ViewBag.TenantList = CommonFunction.Instance.SetSelectListTenantViewBag(User, null);
        ViewBag.T_EntityName = new SelectList(GeneratorBase.MVC.ModelReflector.Entities.Where(p => !p.IsAdminEntity && !p.IsnotificationEntity && !p.IsDefault).OrderBy(p=>p.DisplayName), "Name", "DisplayName", t_documenttemplate.T_EntityName);
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        ViewBag.T_DocumentTemplateIsHiddenRule = checkHidden(User,"T_DocumentTemplate", "OnEdit", false,null);
        ViewBag.T_DocumentTemplateIsGroupsHiddenRule = checkHidden(User,"T_DocumentTemplate", "OnEdit", true,null);
        ViewBag.T_DocumentTemplateIsSetValueUIRule = checkSetValueUIRule(User,"T_DocumentTemplate", "OnEdit",new long[] { 6, 8 },null,null);
        ViewBag.T_DocumentTemplateRestrictDropdownValueRule = RestrictDropdownValueRule(User, "T_DocumentTemplate", "OnEdit", new long[] { 6, 8 }, null, new string[] { "" });
        if(loadCustomViewData) CustomLoadViewDataListBeforeEdit(t_documenttemplate);
    }
    /// <summary>Loads view data after on create.</summary>
    ///
    /// <param name="t_documenttemplate">The Document Template.</param>
    private void LoadViewDataAfterOnCreate(T_DocumentTemplate t_documenttemplate)
    {
        CustomLoadViewDataListAfterOnCreate(t_documenttemplate);
        ViewBag.T_DocumentTemplateIsHiddenRule = checkHidden(User,"T_DocumentTemplate", "OnCreate", false,null);
        ViewBag.T_DocumentTemplateIsGroupsHiddenRule = checkHidden(User,"T_DocumentTemplate", "OnCreate", true,null);
        ViewBag.T_DocumentTemplateIsSetValueUIRule = checkSetValueUIRule(User,"T_DocumentTemplate", "OnCreate",new long[] { 6, 7},null,null);
        ViewBag.T_DocumentTemplateRestrictDropdownValueRule = RestrictDropdownValueRule(User, "T_DocumentTemplate", "OnCreate", new long[] { 6, 7 }, null, new string[] { "" });
    }
    ...
    private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
    {
        ...
        ViewBag.RoleList = new SelectList(RoleList, "", "");
        ViewBag.TenantList = CommonFunction.Instance.SetSelectListTenantViewBag(User, null);
        ViewBag.T_EntityName = new SelectList(GeneratorBase.MVC.ModelReflector.Entities.Where(p => !p.IsAdminEntity && !p.IsnotificationEntity && !p.IsDefault).OrderBy(p => p.DisplayName), "Name", "DisplayName");
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        ViewBag.T_DocumentTemplateIsHiddenRule = checkHidden(User,"T_DocumentTemplate", "OnCreate", false,null);
        ViewBag.T_DocumentTemplateIsGroupsHiddenRule = checkHidden(User,"T_DocumentTemplate", "OnCreate", true,null);
        ViewBag.T_DocumentTemplateIsSetValueUIRule = checkSetValueUIRule(User,"T_DocumentTemplate", "OnCreate",new long[] { 6, 7},null,null);
    ...
    public FileResult DownloadExcel(List<GeneratorBase.MVC.Models.T_DocumentTemplate> query)
    {
        foreach(var item in query)
        {
            item.setDateTimeToClientTime();
            item.ApplyHiddenRule(User.businessrules, "T_DocumentTemplate");
        }
        DataTable dt = ExcelExportHelper.ToDataTable<T_DocumentTemplate>(User, query,"T_DocumentTemplate");
        dt.AcceptChanges();
        string fileName = "Document Template.xlsx";
        using(ClosedXML.Excel.XLWorkbook wb = new ClosedXML.Excel.XLWorkbook())
        {
            var columns = EntityColumns().Select(p=>p.Value).ToArray();
            dt.SetColumnsOrder(columns);
            dt.RemoveColumns(new string[] { "Document"});
            wb.Properties.Author = "GemBoxReporting";
            wb.Properties.Title = "Document Template";
            wb.Properties.Subject = "Document Template data";
    ...
    private Dictionary<string,string> EntityColumns()
    {
        ...
        columns.Add("3", modelproperties.FirstOrDefault(q => q.Name == "T_EntityName").DisplayName);
        columns.Add("4", modelproperties.FirstOrDefault(q => q.Name == "T_Name").DisplayName);
        columns.Add("5", modelproperties.FirstOrDefault(q => q.Name == "T_Description").DisplayName);
        columns.Add("6", modelproperties.FirstOrDefault(q => q.Name == "T_Document").DisplayName);
        columns.Add("7", modelproperties.FirstOrDefault(q => q.Name == "T_AllowedRoles").DisplayName);
        columns.Add("8", modelproperties.FirstOrDefault(q => q.Name == "T_DisplayType").DisplayName);
        columns.Add("9", modelproperties.FirstOrDefault(q => q.Name == "T_Disable").DisplayName);
        columns.Add("10", modelproperties.FirstOrDefault(q => q.Name == "T_EnableDownload").DisplayName);
        columns.Add("11", modelproperties.FirstOrDefault(q => q.Name == "T_EnablePreview").DisplayName);
        return columns;
    }
    
    /// <summary>Append search conditions in IQueryable.</summary>
    ///
    /// <param name="lstT_DocumentTemplate">The list Document Template.</param>
    /// <param name="searchString">The search string.</param>
    /// <param name="IsDeepSearch">Is deep search.</param>
    ///
    /// <returns>The found records.</returns>
    private IQueryable<T_DocumentTemplate> searchRecords(IQueryable<T_DocumentTemplate> lstT_DocumentTemplate, string searchString, bool? IsDeepSearch)
    {
        searchString = searchString.Trim();
        if(Convert.ToBoolean(IsDeepSearch))
            lstT_DocumentTemplate = lstT_DocumentTemplate.Where(s => (s.T_AutoNo != null && SqlFunctions.StringConvert((double)s.T_AutoNo).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_EntityName) && s.T_EntityName.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_DocumentType) && s.T_DocumentType.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_ActionType) && s.T_ActionType.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_DefaultOutputFormat) && s.T_DefaultOutputFormat.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_AllowedRoles) && s.T_AllowedRoles.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_AttachDocumentTo) && s.T_AttachDocumentTo.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_DisplayType) && s.T_DisplayType.ToUpper().Contains(searchString)) ||(s.T_DisplayOrder != null && SqlFunctions.StringConvert((double)s.T_DisplayOrder).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_ToolTip) && s.T_ToolTip.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_BackGroundColor) && s.T_BackGroundColor.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_FontColor) && s.T_FontColor.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_RecordAddedUser) && s.T_RecordAddedUser.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        else
            lstT_DocumentTemplate = lstT_DocumentTemplate.Where(s => (s.T_AutoNo != null && SqlFunctions.StringConvert((double)s.T_AutoNo).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_EntityName) && s.T_EntityName.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_AllowedRoles) && s.T_AllowedRoles.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_DisplayType) && s.T_DisplayType.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        bool boolvalue;
        if(Boolean.TryParse(searchString, out boolvalue))
            lstT_DocumentTemplate = lstT_DocumentTemplate.Union(db.T_DocumentTemplates.Where(s => (s.T_Disable == boolvalue)));
        DateTime datevalue;
        if(DateTime.TryParse(searchString, out datevalue))
            lstT_DocumentTemplate = lstT_DocumentTemplate.Union(db.T_DocumentTemplates.Where(s => (s.T_RecordAdded == datevalue)));
        return lstT_DocumentTemplate;
    }
    /// <summary>Order by list on column.</summary>
    ///
    /// <param name="lstT_DocumentTemplate">The IQueryable list Document Template.</param>
    /// <param name="sortBy">      Column used to sort list.</param>
    /// <param name="isAsc">       Is ascending.</param>
    ///
    /// <returns>The sorted records.</returns>
    private IQueryable<T_DocumentTemplate> sortRecords(IQueryable<T_DocumentTemplate> lstT_DocumentTemplate, string sortBy, string isAsc)
    {
        string methodName = "";
        ...
        if(sortBy.Contains("."))
            return isAsc.ToLower() == "asc" ? lstT_DocumentTemplate.Sort(sortBy,true) : lstT_DocumentTemplate.Sort(sortBy,false);
        ParameterExpression paramExpression = Expression.Parameter(typeof(T_DocumentTemplate), "t_documenttemplate");
        MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
        LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
        return (IQueryable<T_DocumentTemplate>)lstT_DocumentTemplate.Provider.CreateQuery(
                   Expression.Call(
                       typeof(Queryable),
                       methodName,
                       new Type[] { lstT_DocumentTemplate.ElementType, lambda.Body.Type },
                       lstT_DocumentTemplate.Expression,
                       lambda));
    }
/// <summary>Searches for the similar records (Match Making).</summary>
    ...
    public ActionResult SetFSearch(string searchString, string HostingEntity, bool? RenderPartial, bool ShowDeleted = false)
    {
        ...
        SetFSearchViewBag("T_DocumentTemplate");
        ViewBag.HideColumns = new MultiSelectList(EntityColumns(), "Key", "Value");
        return View(new T_DocumentTemplate());
    }
    ...
    [Audit("FacetedSearch")]
    public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport,string T_AutoNoFrom,string T_AutoNoTo,string T_DisplayOrderFrom,string T_DisplayOrderTo,string T_Disable,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab, bool? IsFilter=false, bool ShowDeleted = false)
    {
        FSearchViewBag(currentFilter, searchString, FSFilter, sortBy, isAsc, page, itemsPerPage, search, FilterCondition, HostingEntity, AssociatedType, HostingEntityID, viewtype, SortOrder, HideColumns, GroupByColumn, IsReports, IsdrivedTab, IsFilter,"T_DocumentTemplate");
        ...
        {
            lstT_DocumentTemplate  = searchRecords(lstT_DocumentTemplate, searchString.ToUpper(),true);
        }
        ...
        if(!string.IsNullOrEmpty(search))
        {
            ...
            lstT_DocumentTemplate = searchRecords(lstT_DocumentTemplate, search,true);
        }
        if(!String.IsNullOrEmpty(sortBy) && !String.IsNullOrEmpty(isAsc))
        {
            lstT_DocumentTemplate  = sortRecords(lstT_DocumentTemplate, sortBy, isAsc);
        }
        else   lstT_DocumentTemplate  = lstT_DocumentTemplate.OrderByDescending(c => c.Id);
        lstT_DocumentTemplate = CustomSorting(lstT_DocumentTemplate,HostingEntity,AssociatedType,sortBy,isAsc);
        //lstT_DocumentTemplate = lstT_DocumentTemplate;
        if(!string.IsNullOrEmpty(FilterCondition))
        {
            ...
            foreach(var cond in conditions)
            {
                ...
                if(iCnt == conditions.Count())
                    LogicalConnector = "";
                ViewBag.SearchResult += " " + GetPropertyDP("T_DocumentTemplate", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                whereCondition.Append(conditionFSearch("T_DocumentTemplate",PropertyName, Operator, Value) + LogicalConnector);
                iCnt++;
            }
            if(!string.IsNullOrEmpty(whereCondition.ToString()))
                lstT_DocumentTemplate = lstT_DocumentTemplate.Where(whereCondition.ToString());
        ...
        if(!string.IsNullOrEmpty(DataOrdering))
            lstT_DocumentTemplate = Sorting.Sort<T_DocumentTemplate>(lstT_DocumentTemplate, DataOrdering);
        var _T_DocumentTemplate = lstT_DocumentTemplate;
        if(T_AutoNoFrom!=null || T_AutoNoTo !=null)
        {
            try
            {
                long from = T_AutoNoFrom == null ? 0 : Convert.ToInt64(T_AutoNoFrom);
                long to =  T_AutoNoTo == null ? long.MaxValue : Convert.ToInt64(T_AutoNoTo);
                _T_DocumentTemplate =  _T_DocumentTemplate.Where(o => o.T_AutoNo >= from && o.T_AutoNo <= to);
                ViewBag.SearchResult += "\r\n Auto No.= "+from+"-"+to;
            }
            catch(Exception ex)
            {
        ...
        if(T_DisplayOrderFrom!=null || T_DisplayOrderTo !=null)
        {
            try
            {
                int from = T_DisplayOrderFrom == null ? 0 : Convert.ToInt32(T_DisplayOrderFrom);
                int to =  T_DisplayOrderTo == null ? int.MaxValue : Convert.ToInt32(T_DisplayOrderTo);
                _T_DocumentTemplate =  _T_DocumentTemplate.Where(o => o.T_DisplayOrder >= from && o.T_DisplayOrder <= to);
        ...
        if(T_Disable!=null)
        {
            try
            {
                ...
                _T_DocumentTemplate =  _T_DocumentTemplate.Where(o => o.T_Disable == boolvalue);
                ViewBag.SearchResult += "\r\n Disable?= "+T_Disable;
            }
            catch(Exception ex)
            {
        ...
        _T_DocumentTemplate = FilterByHostingEntity(_T_DocumentTemplate, HostingEntity, AssociatedType, Convert.ToInt32(HostingEntityID));
        if(viewtype == "Metric")
        {
            ...
            if(aggregate!= "Count")
            {
                var result = _T_DocumentTemplate.Aggration(aggregateproperty, aggregate);
                return Json(result, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
            }
            return Json(_T_DocumentTemplate.Count(), "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
        ...
        if(!Request.IsAjaxRequest())
        {
            ...
            if(!string.IsNullOrEmpty(GroupByColumn))
                foreach(var item in list)
                {
                    ...
                    item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_DocumentTemplate", tagsSplit.ToArray());
                }
    ...
    ///
    /// <param name="_T_DocumentTemplate">IQueryable<T_DocumentTemplate>.</param>
    /// <param name="HostingEntity">Name of Hosting Entity.</param>
    /// <param name="AssociatedType">Association Name.</param>
    /// <param name="HostingEntityID">Id of Hosting entity.</param>
    ///
    /// <returns>Modified LINQ IQueryable<T_DocumentTemplate>.</returns>
    private IQueryable<T_DocumentTemplate> FilterByHostingEntity(IQueryable<T_DocumentTemplate> _T_DocumentTemplate, string HostingEntity, string AssociatedType, int? HostingEntityID)
    {
        return _T_DocumentTemplate;
    }
    ...
    public string GetDisplayValue(string id)
    {
        ...
        {
            var _Obj = appcontext.T_DocumentTemplates.FirstOrDefault(p => p.Id == idvalue);
    ...
    public IQueryable<T_DocumentTemplate> GetIQueryable(ApplicationContext appdb)
    {
        return appdb.T_DocumentTemplates.AsNoTracking();
    }
    ...
    public IQueryable<JournalEntry> GetExtraJournalEntry(int? id, Models.IUser user, JournalEntryContext jedb, string RelatedEntityRecords)
    {
        ...
        var t_documenttemplate = jedb.T_DocumentTemplates.Find(id);
        if(RelatedEntityRecords == "T_DocumentTemplate" || string.IsNullOrEmpty(RelatedEntityRecords))
        {
            predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "Document" && d.RecordId == t_documenttemplate.T_Document && d.Type != "View" && d.Type != "ViewList"));
        }
    ...
    public object GetRecordById(string id)
    {
        ...
        using(var appcontext = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = appcontext.T_DocumentTemplates.Find(Convert.ToInt64(id));
            return _Obj;
        }
    }
    ...
    {
        if(string.IsNullOrEmpty(id)) return Json(new T_DocumentTemplate(), JsonRequestBehavior.AllowGet); ;
        using(var appcontext = (new ApplicationContext(new SystemUser(), true)))
        {
            appcontext.Configuration.LazyLoadingEnabled = false;
            var _Obj = appcontext.T_DocumentTemplates.Find(Convert.ToInt64(id));
    ...
    public string GetRecordById_Reflection(string id)
    {
        ...
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = context.T_DocumentTemplates.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_DocumentTemplate>(_Obj, "T_DocumentTemplate", new string[] { "","T_RecordAdded","T_RecordAddedUser" });
        }
    }
    /// <summary>Creates a JSON result with the given data as its content.</summary>
    ///
    ...
    {
        IQueryable<T_DocumentTemplate> list = db.T_DocumentTemplates;
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<T_DocumentTemplate>(User, list, "T_DocumentTemplate", caller);
    ...
    {
        var result = DropdownHelper.GetAllValue<T_DocumentTemplate>(User, db.T_DocumentTemplates, caller, key, AssoNameWithParent, AssociationID, ExtraVal, "T_DocumentTemplate", RestrictDropdownVal);
    ...
    {
        var result = DropdownHelper.GetAllValueForRB<T_DocumentTemplate>(User, db.T_DocumentTemplates, caller, key, AssoNameWithParent, AssociationID, ExtraVal, "T_DocumentTemplate");
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public JsonResult GetAllMultiSelectValue(string caller,string key, string AssoNameWithParent, string AssociationID)
    {
        ...
        var result  = DropdownHelper.GetAllMultiSelectValue(User, db.T_DocumentTemplates, key, AssoNameWithParent, AssociationID, "T_DocumentTemplate", caller);
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public ActionResult Upload(string FileType, string AssociatedType, string HostingEntityName, string HostingEntityID, string UrlReferrer)
    {
        ...
        {
            if(!((CustomPrincipal)User).CanUseVerb("ImportCSV", "T_DocumentTemplate", User) || !User.CanAdd("T_DocumentTemplate"))
            {
        ...
        {
            if(!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_DocumentTemplate", User) || !User.CanAdd("T_DocumentTemplate"))
            {
                return RedirectToAction("Index", "Error");
            }
        }
        ...
        //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_DocumentTemplate")).Count() > 0 ? true : false;
        var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_DocumentTemplate").ToList();
    ...
    public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
    {
        if(FileUpload != null)
        {
            ...
            if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
            {
                ...
                var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_DocumentTemplate");
                if(entList != null)
                {
                    var entityprop = entList.Properties.Where(p => p.Name != "DisplayValue" && p.Name != "TenantId" && p.Name != "IsDeleted" && p.Name != "DeleteDateTime" && p.Name != "T_Document" && p.Name != "T_RecordAdded" && p.Name != "T_RecordAddedUser" && p.Name != "T_RecordAddedInsertBy" && p.Name != "T_RecordAddedInsertDate" && p.Name != "T_RecordAddedUser" && p.Name != "T_RecordAddedUserUser" && p.Name != "T_RecordAddedUserInsertBy" && p.Name != "T_RecordAddedUserInsertDate");
                    string[] allcolumns = new string[] { "T_AutoNo","T_EntityName","T_Name","T_Description","T_DocumentType","T_ActionType","T_DefaultOutputFormat","T_AllowedRoles","T_AttachDocumentTo","T_DisplayType","T_DisplayOrder","T_ToolTip","T_BackGroundColor","T_FontColor","T_Disable","T_RecordAdded","T_RecordAddedUser" };
                ...
                if(!string.IsNullOrEmpty(collection["ListOfMappings"]))
                {
                    ...
                    {
                        var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_DocumentTemplate" && p.IsDefaultMapping);
                    ...
                    DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new <b>Document Template</b>";
    ...
    public ActionResult ConfirmImportData(FormCollection collection)
    {
        ...
        //string excelConnectionString = string.Empty;
        string typename = "T_DocumentTemplate";
        ...
        if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
        {
            ...
            if(objDataSet != null && objDataSet.Tables.Count > 0 && selectedlist != null && columnlist != null)
            {
                ...
                for(int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    if(AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                        continue;
                    T_DocumentTemplate model = new T_DocumentTemplate();
                    ...
                    for(int j = 0; j < sheetColumns.Count; j++)
                    {
                        ...
                        case "T_Description":
                            ...
                            break;
                        case "T_DocumentType":
                            model.T_DocumentType = columnValue;
                            break;
                        case "T_ActionType":
                            model.T_ActionType = columnValue;
                            break;
                        case "T_DefaultOutputFormat":
                            model.T_DefaultOutputFormat = columnValue;
                            break;
                        case "T_AllowedRoles":
                            model.T_AllowedRoles = columnValue;
                            break;
                        case "T_AttachDocumentTo":
                            model.T_AttachDocumentTo = columnValue;
                            break;
                        case "T_DisplayType":
                            model.T_DisplayType = columnValue;
                            break;
                        case "T_DisplayOrder":
                        ...
                        case "T_Disable":
                            ...
                            break;
                        case "T_RecordAdded":
                            model.T_RecordAdded = DateTime.Parse(columnValue);
                            break;
                        default:
                            break;
                        }
                    ...
                    var flagUpdate = MatchUpdate.Update(model, MatchColumns, db, "T_DocumentTemplate", mappedColumns);
                    ...
                    if(AlrtMsg == "")
                    {
                        ...
                        {
                            var result = CheckMandatoryProperties(User,db,model,"T_DocumentTemplate");
                            var validatebr_result = GetValidateBeforeSavePropertiesDictionary(User, db, model, "OnCreate", "T_DocumentTemplate");
                                ...
                                //if (result == null || result.Count == 0)
                            {
                                ...
                                if(!CustomSaveOnImport(model, out customerror,i))
                                {
                                    if(!flagUpdate)
                                    {
                                        db.T_DocumentTemplates.Add(model);
                                        try
                                        {
                                            db.SaveChanges();
                                        }
...
/// <summary>Creates a JSON result with the given data as its content.</summary>
    ...
    public object GetFieldValueByEntityId(long Id, string PropName)
    {
        try
        {
            using(var appcontext  = (new ApplicationContext(new SystemUser(),true)))
            {
                var obj1 = appcontext.T_DocumentTemplates.Where(p=>p.Id == Id).FirstOrDefault();
    ...
    private JsonResult ApplyBusinessRuleOnSaving(T_DocumentTemplate t_documenttemplate)
    {
        ...
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionary(User,db,t_documenttemplate, "OnEdit","T_DocumentTemplate");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
            ...
            {
                var resultBR = GetMandatoryPropertiesDictionary(User,db,t_documenttemplate, "OnEdit","T_DocumentTemplate");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    string BRID = "";
                    foreach(var dic in resultBR)
                    {
                        ...
                        {
                            var type = t_documenttemplate.GetType();
                            if(type.GetProperty(dic.Key) != null)
                            {
                                var propertyvalue = type.GetProperty(dic.Key).GetValue(t_documenttemplate, null);
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User,db,t_documenttemplate, "OnEdit","T_DocumentTemplate");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    foreach(var dic in resultBR)
    ...
    private JsonResult ApplyBusinessRuleBefore(T_DocumentTemplate t_documenttemplate, bool IsReadOnlyIgnore=false)
    {
        var businessrule = User.businessrules.Where(p => p.EntityName == "T_DocumentTemplate").ToList();
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var validateLockResult = GetLockBusinessRulesDictionary(User, db,t_documenttemplate,"T_DocumentTemplate").Where(p => p.Key.Contains("FailureMessage"));
            ...
            {
                var validateMandatorypropertyResult = GetReadOnlyPropertiesDictionary(User,db,t_documenttemplate,"T_DocumentTemplate").Where(p => p.Key.Contains("FailureMessage"));
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetReadOnlyProperties(T_DocumentTemplate OModel)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_DocumentTemplate")));
        return Json(GetReadOnlyPropertiesDictionary(User, db, OModel, "T_DocumentTemplate"), JsonRequestBehavior.AllowGet);
    }
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetMandatoryProperties(T_DocumentTemplate OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_DocumentTemplate")));
        return Json(GetMandatoryPropertiesDictionary(User,db,OModel,ruleType,"T_DocumentTemplate"), JsonRequestBehavior.AllowGet);
    }
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetUIAlertBusinessRules(T_DocumentTemplate OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        var conditions = "";
        if(User.businessrules != null)
        {
            var BR = User.businessrules.Where(p => p.EntityName == "T_DocumentTemplate").ToList();
            var BRAll = BR;
            ...
            if(BR != null && BR.Count > 0)
            {
                ...
                var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_DocumentTemplate");
    ...
    ///
    /// <returns>T_DocumentTemplate object.</returns>
    public T_DocumentTemplate UpdateHiddenProperties(T_DocumentTemplate OModel, string cannotview = null)
    {
        Dictionary<string, string> hiddenProperties = new Dictionary<string, string>();
        if(OModel.Id > 0 && !string.IsNullOrEmpty(cannotview))
            using(var context = (new ApplicationContext(new SystemUser(), true)))
            {
                var obj1 = context.T_DocumentTemplates.Find(OModel.Id);
                if(obj1 != null)
                {
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetHiddenVerb(T_DocumentTemplate OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        //var conditions = "";
        if(User.businessrules != null)
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetValidateBeforeSaveProperties(T_DocumentTemplate OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_DocumentTemplate")));
        return Json(GetValidateBeforeSavePropertiesDictionary(User,db,OModel,ruleType,"T_DocumentTemplate"), JsonRequestBehavior.AllowGet);
    }
    /// <summary>(An Action that handles HTTP POST requests) creates a JSON result (Before save business rule).</summary>
    ///
    /// <param name="OModel">  The model.</param>
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetValidateBeforeSavePropertiesForPopupConfirm(T_DocumentTemplate  OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_DocumentTemplate")));
        return Json(GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User, db, OModel, ruleType, "T_DocumentTemplate"), JsonRequestBehavior.AllowGet);
    }
    ...
    [Audit(0)]
    [ValidateAntiForgeryToken]
    public JsonResult GetLockBusinessRules(T_DocumentTemplate OModel)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_DocumentTemplate")));
        return Json(GetLockBusinessRulesDictionary(User,db,OModel,"T_DocumentTemplate"), JsonRequestBehavior.AllowGet);
    }
    ...
    public long? GetIdFromDisplayValue(string displayvalue)
    {
        ...
        using(ApplicationContext db1 = new ApplicationContext(new SystemUser()))
        {
            ...
            var _Obj = db1.T_DocumentTemplates.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if(_Obj != null)
    ...
    public long? GetIdFromPropertyValue(string PropName, string PropValue)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            IQueryable query = context.T_DocumentTemplates;
            Type[] exprArgTypes = { query.ElementType };
            string propToWhere = PropName;
            ParameterExpression p = Expression.Parameter(typeof(T_DocumentTemplate), "p");
            MemberExpression member = Expression.PropertyOrField(p, propToWhere);
            LambdaExpression lambda = null;
            if(PropValue.ToLower().Trim() != "null")
                lambda = Expression.Lambda<Func<T_DocumentTemplate, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_DocumentTemplate, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_DocumentTemplate>)q);
    ...
    public JsonResult GetPropertyValueByEntityId(long Id, string PropName)
    {
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            var obj1 = context.T_DocumentTemplates.Find(Id);
            if(obj1 == null)
    ...
    public void SetPropertyValueByEntityId(long Id, string PropName, string value)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            T_DocumentTemplate obj1 = context.T_DocumentTemplates.Find(Id);
            if(obj1 != null)
            {
    ...
    [HttpPost]
    public JsonResult Check1MThresholdValue(T_DocumentTemplate t_documenttemplate)
    {
    ...
    {
        string[][] groupsarr = new string[][] { new string[] {"55873","Basic Information"},new string[] {"55877","UI Information"},new string[] {"55876","For Internal Use Only"} };
        return groupsarr;
    }
/// <summary>(An Action that handles HTTP POST requests) gets calculation values.</summary>
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetCalculationValues(T_DocumentTemplate t_documenttemplate)
    {
        t_documenttemplate.setCalculation();
        Dictionary<string, string> Calculations = new Dictionary<string, string>();
        System.Reflection.PropertyInfo[] properties = (t_documenttemplate.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetDerivedDetails(T_DocumentTemplate t_documenttemplate, string IgnoreEditable=null, string source=null)
    {
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetDerivedDetailsInline(string host, string value, T_DocumentTemplate t_documenttemplate, string IgnoreEditable)
    {
        Dictionary<string, string> derivedlist = new Dictionary<string, string>();
        return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
    }
