...
public partial class ReportListController : BaseController
{
    ...
    private void LoadViewDataForCount(ReportList ssrsreport, string AssocType)
    {
    }
    ...
    private void LoadViewDataAfterOnEdit(ReportList ssrsreport)
    {
        LoadViewDataBeforeOnEdit(ssrsreport, false);
        CustomLoadViewDataListAfterEdit(ssrsreport);
    }
    ...
    private void LoadViewDataBeforeOnEdit(ReportList ssrsreport, bool loadCustomViewData = true)
    {
        var _objReportsGroupSSRSReportAssociation = new List<ReportsGroup>();
        _objReportsGroupSSRSReportAssociation.Add(ssrsreport.reportsgroupssrsreportassociation);
        ViewBag.ReportsGroupSSRSReportAssociationID = new SelectList(_objReportsGroupSSRSReportAssociation, "ID", "DisplayValue", ssrsreport.ReportsGroupSSRSReportAssociationID);
        if(loadCustomViewData) CustomLoadViewDataListBeforeEdit(ssrsreport);
    }
    ...
    private void LoadViewDataAfterOnCreate(ReportList ssrsreport)
    {
        ViewBag.ReportsGroupSSRSReportAssociationID = new SelectList(db.ReportsGroups.Where(p => p.Id == ssrsreport.ReportsGroupSSRSReportAssociationID), "ID", "DisplayValue", ssrsreport.ReportsGroupSSRSReportAssociationID);
        CustomLoadViewDataListAfterOnCreate(ssrsreport);
    }
    ...
    {
        if(HostingEntityName == "ReportsGroup" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="ReportsGroupSSRSReportAssociation")
        {
            var hostid = Convert.ToInt64(HostingEntityID);
            var objList = db.ReportsGroups.Where(p=>p.Id==hostid).ToList();
            ViewBag.ReportsGroupSSRSReportAssociationID = new SelectList(objList, "ID", "DisplayValue",HostingEntityID);
            ViewBag.DisplayVal = objList.FirstOrDefault().DisplayValue;
        }
        else
        {
            var objReportsGroupSSRSReportAssociation = new List<ReportsGroup>();
            ViewBag.ReportsGroupSSRSReportAssociationID = new SelectList(objReportsGroupSSRSReportAssociation, "ID", "DisplayValue");
        }
    ...
    ///
    /// <param name="lstReportList">List of list reports.</param>
    /// <param name="searchString"> The search string.</param>
    /// <param name="IsDeepSearch"> The is deep search.</param>
    ///
    /// <returns>The found records.</returns>
    
    private IQueryable<ReportList> searchRecords(IQueryable<ReportList> lstReportList, string searchString, bool? IsDeepSearch)
    {
        searchString = searchString.Trim();
        if(Convert.ToBoolean(IsDeepSearch))
            lstReportList = lstReportList.Where(s => (s.ReportNo != null && SqlFunctions.StringConvert((double)s.ReportNo).Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayName) && s.DisplayName.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.Description) && s.Description.ToUpper().Contains(searchString)) ||(s.reportsgroupssrsreportassociation!= null && (s.reportsgroupssrsreportassociation.DisplayValue.ToUpper().Contains(searchString))) ||(!String.IsNullOrEmpty(s.Name) && s.Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.ReportID) && s.ReportID.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.ReportPath) && s.ReportPath.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.EntityName) && s.EntityName.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        else
            lstReportList = lstReportList.Where(s => (s.ReportNo != null && SqlFunctions.StringConvert((double)s.ReportNo).Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayName) && s.DisplayName.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.Description) && s.Description.ToUpper().Contains(searchString)) ||(s.reportsgroupssrsreportassociation!= null && (s.reportsgroupssrsreportassociation.DisplayValue.ToUpper().Contains(searchString))) ||(!String.IsNullOrEmpty(s.Name) && s.Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.ReportID) && s.ReportID.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.ReportPath) && s.ReportPath.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.EntityName) && s.EntityName.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        bool boolvalue;
        if(Boolean.TryParse(searchString, out boolvalue))
            lstReportList = lstReportList.Union(db.ReportLists.Where(s => (s.IsHidden == boolvalue)));
    ...
    private IQueryable<ReportList> sortRecords(IQueryable<ReportList> lstReportList, string sortBy, string isAsc)
    {
        string methodName = "";
        ...
        if(sortBy == "ReportsGroupSSRSReportAssociationID")
            return isAsc.ToLower() == "asc" ? lstReportList.OrderBy(p => p.reportsgroupssrsreportassociation.DisplayValue) : lstReportList.OrderByDescending(p => p.reportsgroupssrsreportassociation.DisplayValue);
        if(sortBy.Contains("."))
            return isAsc.ToLower() == "asc" ? lstReportList.Sort(sortBy,true) : lstReportList.Sort(sortBy,false);
        ParameterExpression paramExpression = Expression.Parameter(typeof(ReportList), "ssrsreport");
        MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
        LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
        return (IQueryable<ReportList>)lstReportList.Provider.CreateQuery(
                   Expression.Call(
                       typeof(Queryable),
                       methodName,
                       new Type[] { lstReportList.ElementType, lambda.Body.Type },
                       lstReportList.Expression,
                       lambda));
    }
    ...
    /// <param name="searchString">                     The search string.</param>
    /// <param name="HostingEntity">                    The hosting entity.</param>
    /// <param name="reportsgroupssrsreportassociation">    The reportsgroupssrsreportassociation.</param>
    /// <param name="RenderPartial">                    The render partial.</param>
    /// <param name="ShowDeleted">                          (Optional) True to show, false to hide
    ///                                                     the deleted.</param>
    ///
    /// <returns>A response stream to send to the SetFSearch View.</returns>
    
    public ActionResult SetFSearch(string searchString, string HostingEntity,string reportsgroupssrsreportassociation, bool? RenderPartial, bool ShowDeleted = false)
    {
        if(ShowDeleted)
        {
        ...
        if(Qcount > 0)
        {
            ...
            //var ReportsGroupSSRSReportAssociationlist = db.ReportsGroups.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            var ReportsGroupSSRSReportAssociationlist = _fad.FilterDropdown<ReportsGroup>(User, db.ReportsGroups, "ReportsGroup", null);
            if(reportsgroupssrsreportassociation != null)
            {
                var ids = reportsgroupssrsreportassociation.Split(",".ToCharArray());
                ...
                foreach(var str in ids)
                {
                    if(!string.IsNullOrEmpty(str))
                    {
                        ...
                        else
                        {
                            ...
                            ViewBag.SearchResult += db.ReportsGroups.Find(Convert.ToInt64(str)).DisplayValue+", ";
                        }
                    }
                }
                ids1 = ids1.ToList();
                //var list = ReportsGroupSSRSReportAssociationlist.Union(db.ReportsGroups.Where(p=>ids1.Contains(p.Id))).Distinct();
                var list = ReportsGroupSSRSReportAssociationlist.OrderBy(p => p.DisplayValue).Take(10).Union(db.ReportsGroups.Where(p => ids1.Contains(p.Id))).Distinct().ToList();
                //list = _fad.FilterDropdown<ReportsGroup>(User, list, "ReportsGroup", null);
                ViewBag.reportsgroupssrsreportassociation = new SelectList(list, "ID", "DisplayValue");
            }
            else
            {
                //var list = ReportsGroupSSRSReportAssociationlist;
                var list = ReportsGroupSSRSReportAssociationlist.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
                //list = _fad.FilterDropdown<ReportsGroup>(User, list, "ReportsGroup", null);
                ViewBag.reportsgroupssrsreportassociation = new SelectList(list, "ID", "DisplayValue");
            }
        }
        ...
        {
            var objReportsGroupSSRSReportAssociation = new List<ReportsGroup>();
            ViewBag.reportsgroupssrsreportassociation = new SelectList(objReportsGroupSSRSReportAssociation, "ID", "DisplayValue");
        }
        ...
        columns.Add("2", "Report No");
        columns.Add("3", "Display Name");
        columns.Add("4", "Description");
        columns.Add("5", "Group");
        columns.Add("6", "Name");
        columns.Add("7", "ReportID");
        columns.Add("8", "Report Path");
        columns.Add("9", "Entity Name");
        ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
        return View(new ReportList());
    }
    ...
    public string conditionFSearch(string property, string operators, string value)
    {
        ...
        var PrpertyType = GetDataTypeOfProperty("ReportList", property);
        var dataType = PrpertyType[0];
        property = PrpertyType[1];
        if(value.StartsWith("[") && value.EndsWith("]"))
        {
            var ValueType = GetDataTypeOfProperty("ReportList", value, true);
        ...
        case "DateTime":
        {
            ...
            {
                expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new ReportList()).m_Timezone)).Date);
            }
            else
            {
    ...
    /// <summary>GET: /ReportList/FSearch/.</summary>
    ///
    /// <param name="currentFilter">                    A filter specifying the current.</param>
    /// <param name="searchString">                     The search string.</param>
    ...
    /// <param name="IsExport">                         The is export.</param>
    /// <param name="reportsgroupssrsreportassociation">    The reportsgroupssrsreportassociation.</param>
    /// <param name="ReportNoFrom">                     The report no from.</param>
    /// <param name="ReportNoTo">                       The report no to.</param>
    ...
    public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport,string reportsgroupssrsreportassociation,string ReportNoFrom,string ReportNoTo,string IsHidden,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab, bool? IsFilter=false, bool ShowDeleted = false)
    {
        ...
        {
            lstReportList  = searchRecords(lstReportList,searchString.ToUpper(),true);
        }
        ...
        if(!string.IsNullOrEmpty(search))
        {
            ...
            lstReportList = searchRecords(lstReportList, search,true);
        }
        ...
        if(!string.IsNullOrEmpty(FilterCondition))
        {
            ...
            foreach(var cond in conditions)
            {
                ...
                if(iCnt == conditions.Count())
                    LogicalConnector = "";
                ViewBag.SearchResult += " " + GetPropertyDP("ReportList", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                iCnt++;
            }
            if(!string.IsNullOrEmpty(whereCondition.ToString()))
                lstReportList = lstReportList.Where(whereCondition.ToString());
        ...
        if(!string.IsNullOrEmpty(DataOrdering))
            lstReportList = Sorting.Sort<ReportList>(lstReportList, DataOrdering);
        var _ReportList = lstReportList;
        if(ReportNoFrom!=null || ReportNoTo !=null)
        {
            try
            {
                int from = ReportNoFrom == null ? 0 : Convert.ToInt32(ReportNoFrom);
                int to =  ReportNoTo == null ? int.MaxValue : Convert.ToInt32(ReportNoTo);
                _ReportList =  _ReportList.Where(o => o.ReportNo >= from && o.ReportNo <= to);
                ViewBag.SearchResult += "\r\n Report No= "+from+"-"+to;
            }
            catch(Exception ex)
            {
        ...
        if(IsHidden!=null)
        {
            try
            {
                bool boolvalue = Convert.ToBoolean(IsHidden);
                _ReportList =  _ReportList.Where(o => o.IsHidden == boolvalue);
                ViewBag.SearchResult += "\r\n Is Hidden= "+IsHidden;
            }
            catch(Exception ex)
            {
        ...
        //if (lstReportList.Where(p => p.reportsgroupssrsreportassociation != null).Count() <= 50)
        //ViewBag.reportsgroupssrsreportassociation = new SelectList(lstReportList.Where(p => p.reportsgroupssrsreportassociation != null).Select(P => P.reportsgroupssrsreportassociation).Distinct(), "ID", "DisplayValue");
        if(reportsgroupssrsreportassociation != null)
        {
            var ids = reportsgroupssrsreportassociation.Split(",".ToCharArray());
            List<long?> ids1 = new List<long?>();
            ...
            foreach(var str in ids)
            {
                ...
                if(!string.IsNullOrEmpty(str))
                {
                    ...
                    {
                        ids1.Add(Convert.ToInt64(str));
                        var obj = db.ReportsGroups.Find(Convert.ToInt64(str));
            ...
            ids1 = ids1.ToList();
            _ReportList = _ReportList.Where(p => ids1.Contains(p.ReportsGroupSSRSReportAssociationID));
        }
        ...
        if(!string.IsNullOrEmpty(SortOrder))
        {
            ...
            var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "ReportList");
        ...
        if(!Request.IsAjaxRequest())
        {
            ...
            if(!string.IsNullOrEmpty(GroupByColumn))
                foreach(var item in list)
                {
                    ...
                    item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "ReportList", tagsSplit.ToArray());
                }
    ...
    public string GetDisplayValue(string id)
    {
        ...
        {
            var _Obj = appcontext.ReportLists.FirstOrDefault(p => p.Id == idvalue);
    ...
    public object GetRecordById(string id)
    {
        ...
        using(var appcontext = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = appcontext.ReportLists.Find(Convert.ToInt64(id));
            return _Obj;
        }
    }
    ...
    {
        if(string.IsNullOrEmpty(id)) return Json(new ReportList(), JsonRequestBehavior.AllowGet); ;
        using(var appcontext = (new ApplicationContext(new SystemUser(), true)))
        {
            appcontext.Configuration.LazyLoadingEnabled = false;
            var _Obj = appcontext.ReportLists.Find(Convert.ToInt64(id));
    ...
    public string GetRecordById_Reflection(string id)
    {
        ...
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = context.ReportLists.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<ReportList>(_Obj, "ReportList", new string[] { ""  });
        }
    }
    ...
    {
        IQueryable<ReportList> list = db.ReportLists;
    ...
    public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
    {
        ...
        if(AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
        {
            ...
            {
                IQueryable query = db.ReportLists;
                string lambda = "";
                ...
                list = ((IQueryable<ReportList>)query);
            }
            else if(AssoID == 0)
            {
                IQueryable query = db.ReportLists;
                ...
                ParameterExpression p = Expression.Parameter(typeof(ReportList), "p");
                MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                LambdaExpression lambda = Expression.Lambda<Func<ReportList, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                IQueryable q = query.Provider.CreateQuery(methodCall);
                list = ((IQueryable<ReportList>)q);
            }
        }
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<ReportList>(User,list, "ReportList",caller);
        ...
        if(AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
        {
            try
            {
                ...
                if(AssoID != null && AssoID > 0)
                {
                    ...
                    MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                    IQueryable q = query.Provider.CreateQuery(methodCall);
                    list = ((IQueryable<ReportList>)q).Take(20);
                }
            }
            catch
            {
    ...
    public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
    {
        ...
        if(!string.IsNullOrEmpty(propNameBR))
        {
            ...
            //
            ParameterExpression param = Expression.Parameter(typeof(ReportList), "d");
            //bulid expression tree:data.Field1
            var Property = typeof(ReportList).GetProperty(propNameBR);
            ...
            if(targetType.GetGenericArguments().Count() > 0)
            {
                if(targetType.GetGenericArguments()[0].Name == "DateTime")
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                                                      new Type[] { typeof(ReportList), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
            ...
            if(targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
            {
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                                                      new Type[] { typeof(ReportList), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                                                      new Type[] { typeof(ReportList), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                                                      new Type[] { typeof(ReportList), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                                                      new Type[] { typeof(ReportList), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                                                      new Type[] { typeof(ReportList), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
            ...
            {
                Expression expr = Expression.Call(typeof(Queryable), "Select",
                                                  new Type[] { typeof(ReportList), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
    ...
    public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
    {
        ...
        if(AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
        {
            ...
            paramList.Add(Expression.Parameter(typeof(ReportList), "p"));
        ...
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<ReportList>(User, list, "ReportList", null);
        if(key != null && key.Length > 0)
        {
            var data = from x in list.Where(p=>p.DisplayValue.Contains(key)).OrderBy(s => s.DisplayValue.IndexOf(key)).ThenBy(p=>p.DisplayValue.Length).ThenBy(p => p.DisplayValue).Take(10).ToList()
                       select new { Id = x.Id, Name = x.DisplayValue };
    ...
    {
        if(!((CustomPrincipal)User).CanUseVerb("ImportExcel", "ReportList", User) || !User.CanAdd("ReportList"))
        {
            return RedirectToAction("Index", "Error");
        }
        //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "ReportList")).Count() > 0 ? true : false;
        var lstMappings = db.ImportConfigurations.Where(p => p.Name == "ReportList").ToList();
    ...
    public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
    {
        if(FileUpload != null)
        {
            ...
            if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
            {
                ...
                var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "ReportList");
                if(entList != null)
                {
                ...
                if(!string.IsNullOrEmpty(collection["ListOfMappings"]))
                {
                    ...
                    {
                        var lstMapping = db.ImportConfigurations.Where(p => p.Name == "ReportList" && p.IsDefaultMapping);
                    ...
                    DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new SSRS Reports";
                    ...
                    if(entList != null)
                    {
                        ...
                        foreach(var association in entList.Associations)
                        {
                            ...
                            for(int i = 0; i < uniqueCols.Rows.Count; i++)
                            {
                                ...
                                {
                                case "ReportsGroupSSRSReportAssociationID":
                                    var reportsgroupssrsreportassociationId = db.ReportsGroups.FirstOrDefault(p => p.DisplayValue == assovalue);
                                    if(reportsgroupssrsreportassociationId == null)
            ...
            if(entList != null)
            {
                ...
                foreach(var association in entList.Associations)
                {
                    ...
                    for(int i = 0; i < uniqueCols.Rows.Count; i++)
                    {
                        ...
                        {
                        case "ReportsGroupSSRSReportAssociationID":
                            var strPropertyReportsGroupSSRSReportAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "ReportsGroupSSRSReportAssociationID").Value;
                            ModelReflector.Property propReportsGroupSSRSReportAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "ReportsGroup").Properties.FirstOrDefault(p => p.DisplayName == strPropertyReportsGroupSSRSReportAssociation);
                            var reportsgroupssrsreportassociationId = db.ReportsGroups.Where(propReportsGroupSSRSReportAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
                            if(reportsgroupssrsreportassociationId == null)
                                uniqueassoValues.Add(assovalue);
                            break;
                        default:
            ...
            if(selectedlist != null && columnlist != null)
            {
                ...
                for(int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    ...
                    for(int j = 0; j < sheetColumns.Count; j++)
                    {
                        ...
                        {
                        case "ReportNo":
                            model.ReportNo = Int32.Parse(columnValue);
                            break;
                        case "DisplayName":
                            model.DisplayName = columnValue;
                            break;
                        case "Description":
                        ...
                        case "Name":
                            ...
                            break;
                        case "ReportID":
                            model.ReportID = columnValue;
                            break;
                        case "ReportPath":
                            model.ReportPath = columnValue;
                            break;
                        case "EntityName":
                            model.EntityName = columnValue;
                            break;
                        case "IsHidden":
                            model.IsHidden = Boolean.Parse(columnValue);
                            break;
                        case "ReportsGroupSSRSReportAssociationID":
                            dynamic reportsgroupssrsreportassociationId = null;
                            if(lstEntityProp.Count > 0)
                            {
                                var strPropertyReportsGroupSSRSReportAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "ReportsGroupSSRSReportAssociationID").Value;
                                ModelReflector.Property propReportsGroupSSRSReportAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "ReportsGroup").Properties.FirstOrDefault(p => p.DisplayName == strPropertyReportsGroupSSRSReportAssociation);
                                if(propReportsGroupSSRSReportAssociation.Name == "DisplayValue")
                                    reportsgroupssrsreportassociationId = db.ReportsGroups.AsNoTracking().FirstOrDefault(p=>p.DisplayValue == columnValue);
                                else
                                    reportsgroupssrsreportassociationId = db.ReportsGroups.AsNoTracking().Where(propReportsGroupSSRSReportAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
                            }
                            else
                                reportsgroupssrsreportassociationId = db.ReportsGroups.AsNoTracking().FirstOrDefault(p=>p.DisplayValue == columnValue);
                            if(reportsgroupssrsreportassociationId != null)
                                model.ReportsGroupSSRSReportAssociationID = reportsgroupssrsreportassociationId.Id;
                            else
                            {
                                if((collection["ReportsGroupSSRSReportAssociationID"].ToString() == "true,false"))
                                {
                                    try
                                    {
                                        ReportsGroup objReportsGroup = new ReportsGroup();
                                        objReportsGroup.Name  = (columnValue);
                                        db.ReportsGroups.Add(objReportsGroup);
                                        try
                                        {
                                            db.SaveChanges();
                                        }
                                        ...
                                        {
                                            db.Entry(objReportsGroup).State = EntityState.Detached;
                                            continue;
                                        }
                                        model.ReportsGroupSSRSReportAssociationID = objReportsGroup.Id;
                                    }
                                    catch
                                    {
                                        continue;
    ...
    public bool ValidateModel(ReportList validate)
    {
    ...
    public ICollection<ValidationResult> ValidateModelWithErrors(ReportList validate)
    {
    ...
    public object GetFieldValueByEntityId(long Id, string PropName)
    {
        try
        {
            using(var appcontext  = (new ApplicationContext(new SystemUser(),true)))
            {
                var obj1 = appcontext.ReportLists.Find(Id);
    ...
    private JsonResult ApplyBusinessRuleOnSaving(ReportList ssrsreport)
    {
        ...
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionary(ssrsreport, "OnEdit");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
            ...
            {
                var resultBR = GetMandatoryPropertiesDictionary(ssrsreport, "OnEdit");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    string BRID = "";
                    foreach(var dic in resultBR)
                    {
                        ...
                        {
                            var type = ssrsreport.GetType();
                            if(type.GetProperty(dic.Key) != null)
                            {
                                var propertyvalue = type.GetProperty(dic.Key).GetValue(ssrsreport, null);
    ...
    public JsonResult ApplyBusinessRuleBeforeById(long objectId)
    {
        ...
        if((businessrule != null && businessrule.Count > 0))
        {
            var ssrsreport = db.ReportLists.Find(objectId);
            if(ssrsreport != null)
            {
                var ruleids = businessrule.Select(q => q.Id).ToList();
                var typelist = (new GeneratorBase.MVC.Models.RuleActionContext()).RuleActions.Where(p => ruleids.Contains(p.RuleActionID.Value) && p.associatedactiontype.TypeNo.HasValue).Select(p => p.associatedactiontype.TypeNo.Value).Distinct().ToList();
                ...
                {
                    var validateLockResult = GetLockBusinessRulesDictionary(ssrsreport).Where(p => p.Key.Contains("FailureMessage"));
                ...
                {
                    var validateMandatorypropertyResult = GetReadOnlyPropertiesDictionary(ssrsreport);
    ...
    private JsonResult ApplyBusinessRuleBefore(ReportList ssrsreport, bool IsReadOnlyIgnore=false)
    {
        var businessrule = User.businessrules.Where(p => p.EntityName == "ReportList").ToList();
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var validateMandatorypropertyResult = GetReadOnlyPropertiesDictionary(ssrsreport).Where(p => p.Key.Contains("FailureMessage"));
                if(validateMandatorypropertyResult.Count() > 0)
                {
                    string stringResult = "";
                    foreach(var dic in validateMandatorypropertyResult)
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public JsonResult GetReadOnlyProperties(ReportList OModel)
    {
    ...
    public Dictionary<string, string> GetReadOnlyPropertiesDictionary(ReportList OModel)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        if(User.businessrules != null)
        {
            var BR = User.businessrules.Where(p => p.EntityName == "ReportList").ToList();
            var BRAll = BR;
            if(BR != null && BR.Count > 0)
            {
                OModel.setCalculation();
                var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "ReportList");
                ...
                if(ResultOfBusinessRules.Keys.Select(p => p.TypeNo).Contains(4))
                {
                    ...
                    foreach(var parametersArgs in Args)
                    {
                        ...
                        var entity = entityModel.FirstOrDefault(p => p.Name == "ReportList");
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public JsonResult GetMandatoryProperties(ReportList OModel, string ruleType)
    {
    ...
    private Dictionary<string, string> GetMandatoryPropertiesDictionary(ReportList OModel, string ruleType)
    {
        ...
        if(User.businessrules != null)
        {
            ...
            if(BR != null && BR.Count > 0)
            {
                ...
                OModel.setCalculation();
                var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "ReportList");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public JsonResult GetUIAlertBusinessRules(ReportList OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        var conditions = "";
        if(User.businessrules != null)
        {
            ...
            {
                OModel.setCalculation();
                var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "ReportList");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public JsonResult GetValidateBeforeSaveProperties(ReportList OModel, string ruleType)
    {
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public JsonResult GetLockBusinessRules(ReportList OModel)
    {
    ...
    private Dictionary<string, string> GetValidateBeforeSavePropertiesDictionary(ReportList OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        var conditions = "";
        if(User.businessrules != null)
        {
            ...
            {
                OModel.setCalculation();
                var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "ReportList",state);
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
    ...
    public Dictionary<string, string> GetLockBusinessRulesDictionary(ReportList OModel)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        if(User.businessrules != null)
        {
            ...
            {
                OModel.setCalculation();
                var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "ReportList");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
    ...
    private List<string> CheckMandatoryProperties(ReportList OModel)
    {
            ...
            if(BR != null && BR.Count > 0)
            {
                ...
                if(ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                {
                    ...
                    foreach(string paramName in Args.Select(p => p.ParameterName))
                    {
                        ...
                        {
                            var dispName = entityModel.FirstOrDefault(p => p.Name == "ReportList").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                            result.Add(dispName);
                        }
                    }
                }
    ...
    public long? GetIdFromDisplayValue(string displayvalue)
    {
        ...
        using(ApplicationContext db1 = new ApplicationContext(new SystemUser()))
        {
            ...
            var _Obj = db1.ReportLists.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if(_Obj != null)
    ...
    public long? GetIdFromPropertyValue(string PropName, string PropValue)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            IQueryable query = context.ReportLists;
            Type[] exprArgTypes = { query.ElementType };
            ...
            if(PropValue.ToLower().Trim() != "null")
                lambda = Expression.Lambda<Func<ReportList, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<ReportList, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<ReportList>)q);
    ...
    public JsonResult GetPropertyValueByEntityId(long Id, string PropName)
    {
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            var obj1 = context.ReportLists.Find(Id);
            if(obj1 == null)
    ...
    public void SetPropertyValueByEntityId(long Id, string PropName, string value)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            ReportList obj1 = context.ReportLists.Find(Id);
            if(obj1 != null)
            {
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public JsonResult Check1MThresholdValue(ReportList ssrsreport)
    {
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public ActionResult GetCalculationValues(ReportList ssrsreport)
    {
        ssrsreport.setCalculation();
        Dictionary<string, string> Calculations = new Dictionary<string, string>();
        System.Reflection.PropertyInfo[] properties = (ssrsreport.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public ActionResult GetDerivedDetails(ReportList ssrsreport, string IgnoreEditable=null, string source=null)
    {
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public ActionResult GetDerivedDetailsInline(string host, string value, ReportList ssrsreport, string IgnoreEditable)
    {
        Dictionary<string, string> derivedlist = new Dictionary<string, string>();
        return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
    }
