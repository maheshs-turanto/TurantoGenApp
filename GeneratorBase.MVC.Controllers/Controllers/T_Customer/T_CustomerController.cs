...
{
/// <summary>A partial controller class for Customer actions (helper methods and other actions).</summary>
///
/// <remarks></remarks>
public partial class T_CustomerController : BaseController
{
    ...
    private void LoadViewDataForCount(T_Customer t_customer, string AssocType)
    {
    }
    /// <summary>Loads view data after on edit.</summary>
    ///
    /// <param name="t_customer">The Customer.</param>
    private void LoadViewDataAfterOnEdit(T_Customer t_customer)
    {
        LoadViewDataBeforeOnEdit(t_customer, false);
        CustomLoadViewDataListAfterEdit(t_customer);
    }
    /// <summary>Loads view data before on edit.</summary>
    ///
    /// <param name="t_customer">         The Customer.</param>
    /// <param name="loadCustomViewData">(Optional) True to load custom view data.</param>
    private void LoadViewDataBeforeOnEdit(T_Customer t_customer, bool loadCustomViewData = true)
    {
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        ViewBag.T_CustomerIsHiddenRule = checkHidden(User,"T_Customer", "OnEdit", false,null);
        ViewBag.T_CustomerIsGroupsHiddenRule = checkHidden(User,"T_Customer", "OnEdit", true,null);
        ViewBag.T_CustomerIsSetValueUIRule = checkSetValueUIRule(User,"T_Customer", "OnEdit",new long[] { 6, 8 },null,null);
        ViewBag.T_CustomerRestrictDropdownValueRule = RestrictDropdownValueRule(User, "T_Customer", "OnEdit", new long[] { 6, 8 }, null, new string[] { "" });
        var DocumentTemplates = db.T_DocumentTemplates.Where(p => p.T_EntityName == "T_Customer" && !p.T_Disable.Value).OrderBy(p=>p.T_DisplayOrder).ToList();
        ...
        ViewBag.EntityHelp  = db.EntityPages.GetFromCache<IQueryable<EntityPage>, EntityPage>().ToList().Where(p => p.EntityName == "T_Customer" && p.Disable.Value == false).Count() > 0;
        if(loadCustomViewData) CustomLoadViewDataListBeforeEdit(t_customer);
    }
    /// <summary>Loads view data after on create.</summary>
    ///
    /// <param name="t_customer">The Customer.</param>
    private void LoadViewDataAfterOnCreate(T_Customer t_customer)
    {
        CustomLoadViewDataListAfterOnCreate(t_customer);
        ViewBag.T_CustomerIsHiddenRule = checkHidden(User,"T_Customer", "OnCreate", false,null);
        ViewBag.T_CustomerIsGroupsHiddenRule = checkHidden(User,"T_Customer", "OnCreate", true,null);
        ViewBag.T_CustomerIsSetValueUIRule = checkSetValueUIRule(User,"T_Customer", "OnCreate",new long[] { 6, 7},null,null);
        ViewBag.T_CustomerRestrictDropdownValueRule = RestrictDropdownValueRule(User, "T_Customer", "OnCreate", new long[] { 6, 7 }, null, new string[] { "" });
    }
    ...
    public ActionResult DoExport(T_CustomerIndexViewModel model, IQueryable<T_Customer> _T_Customer)
    {
        if(!((CustomPrincipal)User).CanUseVerb("ExportExcel", "T_Customer", User) || !User.CanView("T_Customer"))
        {
        ...
        {
            model.pageNumber = 1;
            if(_T_Customer.Count() > 0)
                model.PageSize = _T_Customer.Count();
            var csvdata = _T_Customer.ToCachedPagedList(model.pageNumber, model.PageSize);
            csvdata.ToList().ForEach(fr => fr.setDateTimeToClientTime());
            csvdata.ToList().ForEach(fr => fr.ApplyHiddenRule(User.businessrules, "T_Customer"));
            csvdata.ToList().ForEach(fr => fr.ApplyHiddenGroupRule(User.businessrules, "T_Customer"));
            return new CsvResult<T_Customer>(csvdata.ToList(), "Customer.csv", EntityColumns().Select(s => s.Value).ToArray(), User, new string[] {  });
        }
        else
        {
            model.pageNumber = 1;
            if(_T_Customer.Count() > 0)
                model.PageSize = _T_Customer.Count();
            return DownloadExcel(_T_Customer.ToCachedPagedList(model.pageNumber, model.PageSize).ToList());
        }
    }
    public ActionResult DoBulkOperations(T_CustomerIndexViewModel model, IQueryable<T_Customer> _T_Customer, IQueryable<T_Customer> lstT_Customer)
    {
        if(model.BulkOperation != null && (model.BulkOperation == "single" || model.BulkOperation == "multiple"))
        {
            ...
            {
                FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
                _T_Customer = _fad.FilterDropdown<T_Customer>(User,  _T_Customer, "T_Customer", model.caller);
            }
            if(Convert.ToBoolean(model.BulkAssociate))
            {
                if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
                {
                    model.list = sortRecords(lstT_Customer.Except(_T_Customer),model.sortBy,model.IsAsc).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
                else
                {
                    model.list = lstT_Customer.Except(_T_Customer).OrderByDescending(c => c.Id).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
            }
            else
            {
                if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
                {
                    model.list = _T_Customer.ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation",model);
                }
                else
                {
                    model.list =_T_Customer.OrderByDescending(c => c.Id).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
            }
        }
    ...
    public FileResult DownloadExcel(List<GeneratorBase.MVC.Models.T_Customer> query)
    {
        ...
        foreach(var item in query)
        {
            ...
            //item.ApplyHiddenRule(User.businessrules, "T_Customer");
            lstHiddenProp = item.ApplyHiddenRule(User.businessrules, "T_Customer");
            lstHiddenGroupProp = item.ApplyHiddenGroupRule(User.businessrules, "T_Customer");
        }
        ...
        DataTable dt = ExcelExportHelper.ToDataTable<T_Customer>(User, query,"T_Customer");
        dt.AcceptChanges();
        ...
        using(ClosedXML.Excel.XLWorkbook wb = new ClosedXML.Excel.XLWorkbook())
        {
            ...
            wb.Properties.Title = "Customer";
            wb.Properties.Subject = "Customer data";
    ...
    private Dictionary<string,string> EntityColumns()
    {
        ...
        columns.Add("4", modelproperties.FirstOrDefault(q => q.Name == "T_Description").DisplayName);
        return columns;
    }
    
    /// <summary>Append search conditions in IQueryable.</summary>
    ///
    /// <param name="lstT_Customer">The list Customer.</param>
    /// <param name="searchString">The search string.</param>
    /// <param name="IsDeepSearch">Is deep search.</param>
    ///
    /// <returns>The found records.</returns>
    private IQueryable<T_Customer> searchRecords(IQueryable<T_Customer> lstT_Customer, string searchString, bool? IsDeepSearch)
    {
        searchString = searchString.Trim();
        if(Convert.ToBoolean(IsDeepSearch))
        {
            lstT_Customer = lstT_Customer.Where(s => (s.T_AutoNo != null && SqlFunctions.StringConvert((double)s.T_AutoNo).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        }
        else
            lstT_Customer = lstT_Customer.Where(s => (s.T_AutoNo != null && SqlFunctions.StringConvert((double)s.T_AutoNo).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        return lstT_Customer;
    }
    /// <summary>Order by list on column.</summary>
    ///
    /// <param name="lstT_Customer">The IQueryable list Customer.</param>
    /// <param name="sortBy">      Column used to sort list.</param>
    /// <param name="isAsc">       Is ascending.</param>
    ///
    /// <returns>The sorted records.</returns>
    private IQueryable<T_Customer> sortRecords(IQueryable<T_Customer> lstT_Customer, string sortBy, string isAsc)
    {
        string methodName = "";
        ...
        if(sortBy == "DisplayValue")
            return isAsc.ToLower() == "asc" ? lstT_Customer.OrderBy(p => p.DisplayValue) : lstT_Customer.OrderByDescending(p => p.DisplayValue);
        if(sortBy.Contains("."))
            return isAsc.ToLower() == "asc" ? lstT_Customer.Sort(sortBy,true) : lstT_Customer.Sort(sortBy,false);
        ParameterExpression paramExpression = Expression.Parameter(typeof(T_Customer), "t_customer");
        MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
        LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
        return (IQueryable<T_Customer>)lstT_Customer.Provider.CreateQuery(
                   Expression.Call(
                       typeof(Queryable),
                       methodName,
                       new Type[] { lstT_Customer.ElementType, lambda.Body.Type },
                       lstT_Customer.Expression,
                       lambda));
    }
/// <summary>Searches for the similar records (Match Making).</summary>
    ...
    [Audit("FacetedSearch")]
//public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport  ,string T_AutoNoFrom,string T_AutoNoTo,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab, bool? IsFilter=false, bool ShowDeleted = false, string ExportType = null)
    public ActionResult FSearch(T_CustomerIndexArgsOption args)
    {
        args.EntityName="T_Customer";
        //FSearchViewBag(currentFilter, searchString, FSFilter, sortBy, isAsc, page, itemsPerPage, search, FilterCondition, HostingEntity, AssociatedType, HostingEntityID, viewtype, SortOrder, HideColumns, GroupByColumn, IsReports, IsdrivedTab, IsFilter,"T_Customer");
        IndexViewBag(args);
        ...
        {
            lstT_Customer  = searchRecords(lstT_Customer, model.searchString.ToUpper(),true);
        }
        ...
        if(!string.IsNullOrEmpty(model.search))
        {
            ...
            lstT_Customer = searchRecords(lstT_Customer, model.search,true);
        }
        if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
        {
        ...
        lstT_Customer = CustomSorting(lstT_Customer,model.HostingEntity,model.AssociatedType,model.sortBy,model.IsAsc);
        //lstT_Customer = lstT_Customer;
        if(!string.IsNullOrEmpty(model.FilterCondition))
        {
            ...
            foreach(var cond in conditions)
            {
                ...
                if(iCnt == conditions.Count())
                    LogicalConnector = "";
                model.SearchResult += " " + GetPropertyDP("T_Customer", PropertyName) + " " + Operator + " " + Value.Trim() + " " + LogicalConnector;
                whereCondition.Append(conditionFSearch("T_Customer",PropertyName, Operator, Value.Trim(),type) + LogicalConnector);
                iCnt++;
            }
            if(!string.IsNullOrEmpty(whereCondition.ToString()))
                lstT_Customer = lstT_Customer.Where(whereCondition.ToString());
        ...
        if(!string.IsNullOrEmpty(DataOrdering))
            lstT_Customer = Sorting.Sort<T_Customer>(lstT_Customer, DataOrdering);
        ...
        if(model.T_AutoNoFrom!=null || model.T_AutoNoTo !=null)
        {
            try
            {
                ...
                _T_Customer =  _T_Customer.Where(o => o.T_AutoNo >= from && o.T_AutoNo <= to);
                model.SearchResult += "\r\n Auto No.= "+from+"-"+to;
            }
            catch(Exception ex)
            {
        ...
        _T_Customer = FilterByHostingEntity(_T_Customer, model.HostingEntity, model.AssociatedType, Convert.ToInt32(model.HostingEntityID));
        if(model.viewtype == "Metric")
        {
            ...
            if(!string.IsNullOrEmpty(model.GroupByColumn))
            {
                ...
                var entityModel = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Customer");
                ...
                if(model.GroupByColumn.ToLower().Contains("displayvalue") == true)
                    if(aggregate != "Count")
                        result = _T_Customer.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key.DisplayValue as X, it."+aggregate+"(Value) as Y)").OrderBy("X");
                    else
                        result = _T_Customer.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key.DisplayValue as X, it."+aggregate+"() as Y)").OrderBy("X");
                else if(aggregate != "Count")
                    result = _T_Customer.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key." + model.GroupByColumn + " as X, it." + aggregate + "(Value) as Y)").OrderBy("X");
                else
                    result = _T_Customer.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key." + model.GroupByColumn + " as X, it." + aggregate + "() as Y)").OrderBy("X");
            ...
            if(aggregate!= "Count")
            {
                var result = _T_Customer.Aggration(aggregateproperty, aggregate);
                var entityName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Customer");
            ...
            return Json(_T_Customer.Count(), "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
        ...
        if(!Request.IsAjaxRequest())
        {
            ...
            if(!string.IsNullOrEmpty(model.GroupByColumn))
                foreach(var item in list)
                {
                    ...
                    item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_Customer", tagsSplit.ToArray());
                }
            model.list = list;
    ...
    ///
    /// <param name="_T_Customer">IQueryable<T_Customer>.</param>
    /// <param name="HostingEntity">Name of Hosting Entity.</param>
    /// <param name="AssociatedType">Association Name.</param>
    /// <param name="HostingEntityID">Id of Hosting entity.</param>
    ///
    /// <returns>Modified LINQ IQueryable<T_Customer>.</returns>
    private IQueryable<T_Customer> FilterByHostingEntity(IQueryable<T_Customer> _T_Customer, string HostingEntity, string AssociatedType, int? HostingEntityID)
    {
        return _T_Customer;
    }
    ...
    public string GetDisplayValue(string id)
    {
        ...
        {
            var _Obj = appcontext.T_Customers.FirstOrDefault(p => p.Id == idvalue);
    ...
    public Dictionary<long, string> GetDisplayValuePivotMatrix(string EntityName, string caller)
    {
        ...
        using(var appcontext = (new ApplicationContext(new SystemUser(), true)))
        {
            IQueryable<T_Customer> list = appcontext.T_Customers;
            FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_Customer>(new SystemUser(), list, "T_Customer", caller);
    ...
    public IQueryable<T_Customer> GetIQueryable(ApplicationContext appdb)
    {
        return appdb.T_Customers.AsNoTracking();
    }
    ...
    public object GetDetailsRecordById(string id, ApplicationContext appcontext)
    {
        ...
        var _Obj = appcontext.T_Customers.Where(fd => fd.Id == dataid).ToList().FirstOrDefault();
        return _Obj;
    }
    ...
    public object GetRecordById(string id)
    {
        ...
        using(var appcontext = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = appcontext.T_Customers.Find(Convert.ToInt64(id));
            return _Obj;
        }
    }
    ...
    public IQueryable<long> SearchRecords(string searchkey, List<long> RecordIds)
    {
        ...
        {
            appcontext.Configuration.LazyLoadingEnabled = false;
            var list = appcontext.T_Customers.Where(p => RecordIds.Contains(p.Id));
    ...
    {
        if(string.IsNullOrEmpty(id)) return Json(new T_Customer(), JsonRequestBehavior.AllowGet); ;
        using(var appcontext = (new ApplicationContext(new SystemUser(), true)))
        {
            appcontext.Configuration.LazyLoadingEnabled = false;
            var _Obj = appcontext.T_Customers.Find(Convert.ToInt64(id));
            long? tenantId = null;
            var _updatedAssocObj = UpdateAssociationValue("T_Customer", _Obj, tenantId);
    ...
    public string GetRecordById_Reflection(string id)
    {
        ...
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = context.T_Customers.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Customer>(_Obj, "T_Customer", new string[] { ""  });
        }
    }
    /// <summary>Creates a JSON result with the given data as its content.</summary>
    ///
    ...
    {
        IQueryable<T_Customer> list = db.T_Customers;
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<T_Customer>(User, list, "T_Customer", caller);
        var data = from x in list.OrderBy(q => q.DisplayValue).ToList()
                   select new { Id = x.Id, Name = x.DisplayValue };
    ...
    {
        var list = db.T_Customers.OrderBy(p => p.DisplayValue).AsQueryable();//;
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<T_Customer>(User, list, "T_Customer", associationName+"ID",RestrictDropdownVal);
    ...
    {
        var list = CustomDropdownFilter(db.T_Customers.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, ExtraVal, RestrictDropdownVal, CustomParameter, "GetAllValue");
        var result = DropdownHelper.GetAllValue<T_Customer>(User, list, caller, key, AssoNameWithParent, AssociationID, ExtraVal, "T_Customer", RestrictDropdownVal);
    ...
    {
        var result = DropdownHelper.GetAllValueForRB<T_Customer>(User, db.T_Customers, caller, key, AssoNameWithParent, AssociationID, ExtraVal, "T_Customer");
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
    {
        ...
        if(!string.IsNullOrEmpty(propNameBR))
        {
            ...
            {
                var EntityInfo = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Customer");
    ...
    public JsonResult GetAllMultiSelectValue(string caller,string key, string AssoNameWithParent, string AssociationID, string bulkAdd = null, string CustomParameter = null, string ExtraVal = null)
    {
        ...
        var list = CustomDropdownFilter(db.T_Customers.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, ExtraVal, null, CustomParameter, "GetAllMultiSelectValue");
        var result  = DropdownHelper.GetAllMultiSelectValue(User, list, key, AssoNameWithParent, AssociationID, "T_Customer", caller, ExtraVal);
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public JsonResult GetMultiSelectValueAllSelection(string caller,string key, string AssoNameWithParent, string AssociationID, string bulkAdd = null, string bulkSelection = null, string CustomParameter = null, string ExtraVal = null)
    {
        if(caller != null)
            caller = caller.Replace("?", "");
        var list = CustomDropdownFilter(db.T_Customers.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, ExtraVal, null, CustomParameter, "GetMultiSelectValueAllSelection");
        var result  = DropdownHelper.GetMultiSelectValueAllSelection(User, list, key, AssoNameWithParent, AssociationID, "T_Customer", caller, ExtraVal);
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public ActionResult Upload(string FileType, string AssociatedType, string HostingEntityName, string HostingEntityID, string UrlReferrer,string ImportType="")
    {
        ...
        {
            if(!((CustomPrincipal)User).CanUseVerb("ImportCSV", "T_Customer", User) || !User.CanAdd("T_Customer"))
            {
                return RedirectToAction("Index", "Error");
            }
        }
        ...
        {
            if(!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_Customer", User) || !User.CanAdd("T_Customer"))
            {
                return RedirectToAction("Index", "Error");
            }
        }
        ...
        //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Customer")).Count() > 0 ? true : false;
        var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Customer").ToList();
        ...
        if(FileUpload != null)
        {
            ...
            if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
            {
                ...
                if(entList != null)
                {
                    ...
                    allcolumns = new string[] { "T_AutoNo","T_Name","T_Description" };
                ...
                if(!string.IsNullOrEmpty(collection["ListOfMappings"]))
                {
                    ...
                    DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new <b>Customer</b>";
            ...
            if(!String.IsNullOrEmpty(mappingName))
            {
                ...
                for(int i = 0; i < tblcols.Count(); i++)
                {
                    ...
                    if(!string.IsNullOrEmpty(ImportType))
                    {
                        ...
                        if(Updatefields.ToList().Any(p => "T_Customer$"+p == objImtConfig.TableColumn))
                            objImtConfig.UpdateColumn = Updatefields.ToList().FirstOrDefault(p => "T_Customer$" + p == objImtConfig.TableColumn);
                    }
        ...
        if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
        {
            ...
            if(objDataSet != null && objDataSet.Tables.Count > 0 && selectedlist != null && columnlist != null)
            {
                ...
                for(int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    ...
                    var flagUpdate = MatchUpdate.Update(model, MatchColumns, db, "T_Customer", mappedColumns);
                    ...
                    if(AlrtMsg == "")
                    {
                        ...
                        {
                            var result = CheckMandatoryProperties(User,db,model,"T_Customer");
                            var validatebr_result = GetValidateBeforeSavePropertiesDictionary(User, db, model, "OnCreate", "T_Customer");
    ...
    {
        string MainEntityName = "T_Customer";
        db.Configuration.AutoDetectChangesEnabled = false;
        db.Configuration.LazyLoadingEnabled = false;
        string FilePath = collection["hdnFilePath"];
        var columnlist = collection["hdnColumnList"];
        ...
        if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
        {
            ...
            if(objDataSet != null && objDataSet.Tables.Count > 0 && selectedlist != null && columnlist != null)
            {
                ...
                for(int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    ...
                    model = LoadObjectFromSheet(model, sheetColumns, tblColumns, i, objDataSet, AssociatedType, HostingEntityID, lstEntityProp, mappedColumns, MainEntityName, dbnew, mapCount) as T_Customer;
                    var flagUpdate = MatchUpdate.Update(model, MatchColumns, dbnew, MainEntityName, mappedColumns, true);
                    if(flagUpdate) continue;
                    //
                    var AlrtMsg = CheckBeforeSave(model as T_Customer, "ImportData");
                    if(AlrtMsg == "")
                    {
                        var customimport_hasissues = false;
                        if(ValidateModel(model))
                        {
                            ...
                            if((result == null || result.Count == 0) && (validatebr_result == null || validatebr_result.Count == 0))
                            {
                                ...
                                if(!CustomSaveOnImport(model as T_Customer, out customerror, i))
                                {
                                    if(!flagUpdate)
                                    {
                                        dbnew.T_Customers.Add(model);
                                        try
                                        {
                                            dbnew.SaveChanges();
                                        }
...
/// <summary>Creates a JSON result with the given data as its content.</summary>
    ...
    public object GetFieldValueByEntityId(long Id, string PropName)
    {
        try
        {
            using(var appcontext  = (new ApplicationContext(new SystemUser(),true)))
            {
                var obj1 = appcontext.T_Customers.Where(p=>p.Id == Id).FirstOrDefault();
    ...
    private JsonResult ApplyBusinessRuleOnSaving(T_Customer t_customer)
    {
        ...
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionary(User,db,t_customer, "OnEdit","T_Customer");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
            ...
            {
                var resultBR = GetMandatoryPropertiesDictionary(User,db,t_customer, "OnEdit","T_Customer");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    string BRID = "";
                    foreach(var dic in resultBR)
                    {
                        ...
                        {
                            var type = t_customer.GetType();
                            if(type.GetProperty(dic.Key) != null)
                            {
                                var propertyvalue = type.GetProperty(dic.Key).GetValue(t_customer, null);
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User,db,t_customer, "OnEdit","T_Customer");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    foreach(var dic in resultBR)
    ...
    private JsonResult ApplyBusinessRuleBefore(T_Customer t_customer, bool IsReadOnlyIgnore=false)
    {
        var businessrule = User.businessrules.Where(p => p.EntityName == "T_Customer").ToList();
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var validateLockResult = GetLockBusinessRulesDictionary(User, db,t_customer,"T_Customer").Where(p => p.Key.Contains("FailureMessage"));
            ...
            {
                var validateMandatorypropertyResult = GetReadOnlyPropertiesDictionary(User,db,t_customer,"T_Customer").Where(p => p.Key.Contains("FailureMessage"));
    ...
    public JsonResult ResetToDefaultField(long id, string fieldName)
    {
        ...
        if(id > 0)
        {
            using(var context = (new ApplicationContext(new SystemUser(), true)))
            {
                var obj1 = context.T_Customers.Find(id);
                if(obj1 != null)
                {
                    System.Reflection.PropertyInfo[] properties = (obj1.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
    ...
    {
        var proplstgroupby = ModelReflector.Entities.FirstOrDefault(p=>p.Name == "T_Customer").Properties.Where(p => p.Proptype.ToLower() == "group").Where(p => ("T_Customer" + p.GroupInternalName) == groupName);
        Dictionary<string, string> hiddenProperties = new Dictionary<string, string>();
        if(id > 0)
        {
            using(var context = (new ApplicationContext(new SystemUser(), true)))
    ...
    public JsonResult GetReadOnlyProperties(T_Customer OModel,string ruleType=null)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_Customer")));
        return Json(GetReadOnlyPropertiesDictionary(User, db, OModel, "T_Customer",ruleType), JsonRequestBehavior.AllowGet);
    }
    ...
    public JsonResult GetMandatoryProperties(T_Customer OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_Customer")));
        return Json(new { dict = GetMandatoryPropertiesDictionary(User, db, OModel, ruleType, "T_Customer"), template = getBRTemplate(2) }, JsonRequestBehavior.AllowGet);
        //return Json(GetMandatoryPropertiesDictionary(User,db,OModel,ruleType,"T_Customer"), JsonRequestBehavior.AllowGet);
    }
    ...
    public JsonResult GetUIAlertBusinessRules(T_Customer OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        var conditions = "";
        if(User.businessrules != null)
        {
            var BR = User.businessrules.Where(p => p.EntityName == "T_Customer").ToList();
            var BRAll = BR;
            ...
            if(BR != null && BR.Count > 0)
            {
                ...
                var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_Customer");
    ...
    ///
    /// <returns>T_Customer object.</returns>
    public T_Customer UpdateHiddenProperties(T_Customer OModel, string cannotview = null)
    {
        Dictionary<string, string> hiddenProperties = new Dictionary<string, string>();
        if(OModel.Id > 0 && !string.IsNullOrEmpty(cannotview))
            using(var context = (new ApplicationContext(new SystemUser(), true)))
            {
                var obj1 = context.T_Customers.Find(OModel.Id);
                if(obj1 != null)
                {
    ...
    public JsonResult GetHiddenVerb(T_Customer OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        //var conditions = "";
        if(User.businessrules != null)
        {
            ...
            if(BR != null && BR.Count > 0)
            {
                OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_Customer")));
                OModel.setCalculation();
                var ResultOfBusinessRules = db.GetHiddenVerb(OModel, BR, "T_Customer");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
    ...
    public Dictionary<string, string> GetHiddenVerbDetails(T_Customer OModel, string ruleType, string pagetype = null)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        //var conditions = "";
        if(User.businessrules != null)
    ...
    public JsonResult GetValidateBeforeSaveProperties(T_Customer OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_Customer")));
        return Json(new { dict = GetValidateBeforeSavePropertiesDictionary(User, db, OModel, ruleType, "T_Customer"), template = getBRTemplate(10) }, JsonRequestBehavior.AllowGet);
        //return Json(GetValidateBeforeSavePropertiesDictionary(User,db,OModel,ruleType,"T_Customer"), JsonRequestBehavior.AllowGet);
    }
    /// <summary>(An Action that handles HTTP POST requests) creates a JSON result (Before save business rule).</summary>
    ///
    /// <param name="OModel">  The model.</param>
    ...
    public JsonResult GetValidateBeforeSavePropertiesForPopupConfirm(T_Customer  OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_Customer")));
        return Json(new { dict = GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User, db, OModel, ruleType, "T_Customer"), template = getBRTemplate(15)}, JsonRequestBehavior.AllowGet);
        //return Json(GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User, db, OModel, ruleType, "T_Customer"), JsonRequestBehavior.AllowGet);
    }
    ...
    public JsonResult GetLockBusinessRules(T_Customer OModel)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_Customer")));
        return Json(new { dict = GetLockBusinessRulesDictionary(User, db, OModel, "T_Customer"), template = getBRTemplate(1) }, JsonRequestBehavior.AllowGet);
        //return Json(GetLockBusinessRulesDictionary(User,db,OModel,"T_Customer"), JsonRequestBehavior.AllowGet);
    }
    ...
    public long? GetIdFromDisplayValue(string displayvalue)
    {
        ...
        using(ApplicationContext db1 = new ApplicationContext(new SystemUser()))
        {
            ...
            var _Obj = db1.T_Customers.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if(_Obj != null)
    ...
    public long? GetIdFromPropertyValue(string PropName, string PropValue)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            IQueryable query = context.T_Customers;
            Type[] exprArgTypes = { query.ElementType };
            string propToWhere = PropName;
            ParameterExpression p = Expression.Parameter(typeof(T_Customer), "p");
            ...
            if(PropValue.ToLower().Trim() != "null")
            {
                ...
                {
                    PropValue = GetIdofDisplayValueforSetIfZero(PropName, "T_Customer", PropValue);
                    ...
                    lambda = Expression.Lambda<Func<T_Customer, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue1), member.Type)), p);
                }
                else
                {
                    lambda = Expression.Lambda<Func<T_Customer, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
                }
            }
            else
                lambda = Expression.Lambda<Func<T_Customer, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Customer>)q);
    ...
    public void SetPropertyValueByEntityId(long Id, string PropName, string value)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            T_Customer obj1 = context.T_Customers.Find(Id);
            if(obj1 != null)
            {
                ...
                if(PropName.Substring(PropName.Length - 2) == "ID")
                {
                    value = GetIdofDisplayValueforSet(PropName, "T_Customer", value);
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public JsonResult Check1MThresholdValue(T_Customer t_customer)
    {
    ...
    {
        string[][] groupsarr = new string[][] { new string[] {"276327","Basic Information"},new string[] {"276328","More Information"} };
        return groupsarr;
    }
/// <summary>(An Action that handles HTTP POST requests) gets calculation values.</summary>
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetCalculationValues(T_Customer t_customer)
    {
        t_customer.setCalculation();
        Dictionary<string, string> Calculations = new Dictionary<string, string>();
        System.Reflection.PropertyInfo[] properties = (t_customer.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        var EntityInfo = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Customer");
        string DataType = string.Empty;
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetDerivedDetails(T_Customer t_customer, string IgnoreEditable=null, string source=null)
    {
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetDerivedDetailsInline(string host, string value, T_Customer t_customer, string IgnoreEditable)
    {
        Dictionary<string, string> derivedlist = new Dictionary<string, string>();
        return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
    }
    ...
    public async Task<ActionResult> BulkDocumentDownload(string Ids,bool Isflat = false)
    {
        ...
        var t_customers = await db.T_Customers.Where(s => eIds.Contains(s.Id)).ToListAsync();
        var derivedProp = new Dictionary<string, string>();
        foreach(var t_customer in t_customers)
        {
            await AddDownloadDirectoryForEntity(dir, entityName, t_customer, derivedProp);
        }
        ...
        Response.AddHeader("Content-Disposition", string.Format("attachment; filename={0}.zip", GetProperFileDirName(dir.Name) + "_" + String.Format("{0:MM-dd-yyyy HH-mm-ss-ffff}", TimeZoneInfo.ConvertTimeFromUtc(Convert.ToDateTime(DateTime.UtcNow), t_customers.FirstOrDefault().m_Timezone))));
    ...
    {
        if(!User.CanView("T_Customer") || !CustomAuthorizationBeforeCreate(HostingEntityName, HostingEntityID, AssociatedType))
        {
            return RedirectToAction("Index", "Error");
        }
    ...
    {
        T_Customer t_customer = null;
        if(db != null)
        {
            t_customer = db.T_Customers.AsNoTracking().FirstOrDefault(r => r.Id == id);
            t_customer.setDateTimeToClientTime();
        }
        documentName = documentName + "_" + t_customer.DisplayValue + " " + String.Format("{0:MM-dd-yyyy HH-mm}", TimeZoneInfo.ConvertTimeFromUtc(Convert.ToDateTime(DateTime.UtcNow), t_customer.m_Timezone));
        ...
        var result =  ValidateBeforeDocGenerated(t_customer, documentName, outputFormat, isdownload);
        ...
        using(Stream stream = new MemoryStream(bytedata))
        {
            ...
            OnDocGenerating(document, t_customer, documentName, outputFormat, isdownload);
            ...
            document.MailMerge.Execute(t_customer);
            ...
            AfterDocGenerated(t_customer, documentName, outputFormat, isdownload, outstream);
    ...
    public async Task<ActionResult> ExportData(string Ids, int btnid, string Notes)
    {
        ...
        if(IsBulk)
        {
            BulkfolderPath = Path.Combine(Server.MapPath("~/Files/" + entities.FirstOrDefault(fd => fd.Name == "T_Customer").DisplayName + "BulkExport_" + DateTime.UtcNow.Ticks));
            Bulkzipvalue = entities.FirstOrDefault(fd => fd.Name == "T_Customer").DisplayName + "-BulkExport-" + DateTime.UtcNow.ToString("dd-MM-yyyy hh-mm-ss") + " UTC";
        }
        foreach(var id in lstIds)
        {
            var zipdata = db.T_Customers.Find(id);
            zipvalue = entities.FirstOrDefault(fd => fd.Name == "T_Customer").DisplayName + "-" + zipdata.DisplayValue + "-" + DateTime.UtcNow.ToString("dd-MM-yyyy hh-mm-ss") + " UTC";
            ...
            {
                folderPath = Path.Combine(Server.MapPath("~/Files/" + entities.FirstOrDefault(fd => fd.Name == "T_Customer").DisplayName + "_" + DateTime.UtcNow.Ticks));
            ...
            foreach(var item in KeysData)
            {
                ...
                var rootlist = db.T_Customers.Where(wh => wh.Id == id);
                var ds = new DataSet();
            ...
            if(IsRootItem)
            {
                var rootdp = entities.FirstOrDefault(fd => fd.Name == "T_Customer").DisplayName;
                var rootlist = db.T_Customers.Where(wh => wh.Id == id);
                var ds = new DataSet();
                var lstcondition = new Dictionary<string, string>();
                ...
                if(IsSoftDeleteEnabled)
                    if(datalist.Count() > 0)
                    {
                        ...
                        {
                            Id = 0,
                            EntityName = "T_Customer",
                            Query = datalist
                        });
                    }
                DataTable dt = ExcelExportHelper.ToDataTable<object>(User, datalist.ToListAsync().Result.ToList(), "T_Customer", "T_Customer", null, folderPath, childfolderPath, false, IsSoftDeleteEnabled);
                ds.Tables.Add(dt);
            ...
            using(var dbnew = new ApplicationContext(User))
            {
                ...
                DoAuditEntry.AddJournalEntryRecordId("T_Customer", User.Name, objExportDataConfiguration.T_Name, zipdata.DisplayValue, Convert.ToString(id));
    ...
    public Dictionary<long, string> GetGroupList()
    {
        ...
        {
            { 276327, "BasicInformation" },
            { 276328, "MoreInformation" },
        };
        return GroupList;
    }
    [Audit(0)]
