...
{
/// <summary>A partial controller class for Menu Bar actions (helper methods and other actions).</summary>
///
/// <remarks></remarks>
public partial class T_MenuBarController : BaseController
{
    ...
    ///
    /// <param name="t_menubar">The Menu Bar.</param>
    /// <param name="AssocType">Type of the associated.</param>
    private void LoadViewDataForCount(T_MenuBar t_menubar, string AssocType)
    {
    }
    /// <summary>Loads view data after on edit.</summary>
    ///
    /// <param name="t_menubar">The Menu Bar.</param>
    private void LoadViewDataAfterOnEdit(T_MenuBar t_menubar)
    {
        LoadViewDataBeforeOnEdit(t_menubar, false);
        CustomLoadViewDataListAfterEdit(t_menubar);
    }
    /// <summary>Loads view data before on edit.</summary>
    ///
    /// <param name="t_menubar">         The Menu Bar.</param>
    /// <param name="loadCustomViewData">(Optional) True to load custom view data.</param>
    private void LoadViewDataBeforeOnEdit(T_MenuBar t_menubar, bool loadCustomViewData = true)
    {
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        ViewBag.T_MenuBarIsHiddenRule = checkHidden(User,"T_MenuBar", "OnEdit", false,null);
        ViewBag.T_MenuBarIsGroupsHiddenRule = checkHidden(User,"T_MenuBar", "OnEdit", true,null);
        ViewBag.T_MenuBarIsSetValueUIRule = checkSetValueUIRule(User,"T_MenuBar", "OnEdit",new long[] { 6, 8 },null,null);
        ViewBag.T_MenuBarRestrictDropdownValueRule = RestrictDropdownValueRule(User, "T_MenuBar", "OnEdit", new long[] { 6, 8 }, null, new string[] { "" });
        var DocumentTemplates = db.T_DocumentTemplates.Where(p => p.T_EntityName == "T_MenuBar" && !p.T_Disable.Value).OrderBy(p=>p.T_DisplayOrder).ToList();
        ...
        if(loadCustomViewData) CustomLoadViewDataListBeforeEdit(t_menubar);
    }
    /// <summary>Loads view data after on create.</summary>
    ///
    /// <param name="t_menubar">The Menu Bar.</param>
    private void LoadViewDataAfterOnCreate(T_MenuBar t_menubar)
    {
        CustomLoadViewDataListAfterOnCreate(t_menubar);
        ViewBag.T_MenuBarIsHiddenRule = checkHidden(User,"T_MenuBar", "OnCreate", false,null);
        ViewBag.T_MenuBarIsGroupsHiddenRule = checkHidden(User,"T_MenuBar", "OnCreate", true,null);
        ViewBag.T_MenuBarIsSetValueUIRule = checkSetValueUIRule(User,"T_MenuBar", "OnCreate",new long[] { 6, 7},null,null);
        ViewBag.T_MenuBarRestrictDropdownValueRule = RestrictDropdownValueRule(User, "T_MenuBar", "OnCreate", new long[] { 6, 7 }, null, new string[] { "" });
    }
    ...
    public ActionResult DoExport(T_MenuBarIndexViewModel model, IQueryable<T_MenuBar> _T_MenuBar)
    {
        if(!((CustomPrincipal)User).CanUseVerb("ExportExcel", "T_MenuBar", User) || !User.CanView("T_MenuBar"))
        {
        ...
        {
            model.pageNumber = 1;
            if(_T_MenuBar.Count() > 0)
                model.PageSize = _T_MenuBar.Count();
            var csvdata = _T_MenuBar.ToCachedPagedList(model.pageNumber, model.PageSize);
            csvdata.ToList().ForEach(fr => fr.setDateTimeToClientTime());
            csvdata.ToList().ForEach(fr => fr.ApplyHiddenRule(User.businessrules, "T_MenuBar"));
            return new CsvResult<T_MenuBar>(csvdata.ToList(), "Menu Bar.csv", EntityColumns().Select(s => s.Value).ToArray(), User, new string[] {  });
        }
        else
        {
            model.pageNumber = 1;
            if(_T_MenuBar.Count() > 0)
                model.PageSize = _T_MenuBar.Count();
            return DownloadExcel(_T_MenuBar.ToCachedPagedList(model.pageNumber, model.PageSize).ToList());
        }
    }
    public ActionResult DoBulkOperations(T_MenuBarIndexViewModel model, IQueryable<T_MenuBar> _T_MenuBar, IQueryable<T_MenuBar> lstT_MenuBar)
    {
        if(model.BulkOperation != null && (model.BulkOperation == "single" || model.BulkOperation == "multiple"))
        {
            ...
            {
                FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
                _T_MenuBar = _fad.FilterDropdown<T_MenuBar>(User,  _T_MenuBar, "T_MenuBar", model.caller);
            }
            if(Convert.ToBoolean(model.BulkAssociate))
            {
                if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
                {
                    model.list = sortRecords(lstT_MenuBar.Except(_T_MenuBar),model.sortBy,model.IsAsc).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
                else
                {
                    model.list = lstT_MenuBar.Except(_T_MenuBar).OrderByDescending(c => c.Id).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
            }
            else
            {
                if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
                {
                    model.list = _T_MenuBar.ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation",model);
                }
                else
                {
                    model.list =_T_MenuBar.OrderByDescending(c => c.Id).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
            }
        }
    ...
    public FileResult DownloadExcel(List<GeneratorBase.MVC.Models.T_MenuBar> query)
    {
        List<string> lstHiddenProp = new List<string>();
        foreach(var item in query)
        {
            item.setDateTimeToClientTime();
            //item.ApplyHiddenRule(User.businessrules, "T_MenuBar");
            lstHiddenProp = item.ApplyHiddenRule(User.businessrules, "T_MenuBar");
        }
        if(lstHiddenProp.Count > 0)
        {
            var modelproperties = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuBar").Properties;
            for(int i = 0; i < lstHiddenProp.Count; i++)
            {
                lstHiddenProp[i] = modelproperties.FirstOrDefault(q => q.Name == lstHiddenProp[i]).DisplayName;
            }
        }
        DataTable dt = ExcelExportHelper.ToDataTable<T_MenuBar>(User, query,"T_MenuBar");
        dt.AcceptChanges();
        string fileName = "Menu Bar.xlsx";
        using(ClosedXML.Excel.XLWorkbook wb = new ClosedXML.Excel.XLWorkbook())
        {
            ...
            wb.Properties.Title = "Menu Bar";
            wb.Properties.Subject = "Menu Bar data";
    ...
    private Dictionary<string,string> EntityColumns()
    {
        ...
        columns.Add("4", modelproperties.FirstOrDefault(q => q.Name == "T_Disabled").DisplayName);
        columns.Add("5", modelproperties.FirstOrDefault(q => q.Name == "T_Roles").DisplayName);
        columns.Add("6", modelproperties.FirstOrDefault(q => q.Name == "T_Horizontal").DisplayName);
        return columns;
    }
    
    /// <summary>Append search conditions in IQueryable.</summary>
    ///
    /// <param name="lstT_MenuBar">The list Menu Bar.</param>
    /// <param name="searchString">The search string.</param>
    /// <param name="IsDeepSearch">Is deep search.</param>
    ///
    /// <returns>The found records.</returns>
    private IQueryable<T_MenuBar> searchRecords(IQueryable<T_MenuBar> lstT_MenuBar, string searchString, bool? IsDeepSearch)
    {
        searchString = searchString.Trim();
        if(Convert.ToBoolean(IsDeepSearch))
        {
            lstT_MenuBar = lstT_MenuBar.Where(s => (s.T_AutoNo != null && SqlFunctions.StringConvert((double)s.T_AutoNo).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Roles) && s.T_Roles.ToUpper().Contains(searchString)) ||s.T_MenuBarMenuItemAssociation_t_menubar.Any(c => c.t_menuitem.DisplayValue.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        }
        else
            lstT_MenuBar = lstT_MenuBar.Where(s => (s.T_AutoNo != null && SqlFunctions.StringConvert((double)s.T_AutoNo).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Roles) && s.T_Roles.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        bool boolvalue;
        if(Boolean.TryParse(searchString, out boolvalue))
            lstT_MenuBar = lstT_MenuBar.Union(db.T_MenuBars.Where(s => (s.T_Disabled == boolvalue) ||(s.T_Horizontal == boolvalue)));
        return lstT_MenuBar;
    }
    /// <summary>Order by list on column.</summary>
    ///
    /// <param name="lstT_MenuBar">The IQueryable list Menu Bar.</param>
    /// <param name="sortBy">      Column used to sort list.</param>
    /// <param name="isAsc">       Is ascending.</param>
    ///
    /// <returns>The sorted records.</returns>
    private IQueryable<T_MenuBar> sortRecords(IQueryable<T_MenuBar> lstT_MenuBar, string sortBy, string isAsc)
    {
        string methodName = "";
        ...
        if(sortBy == "DisplayValue")
            return isAsc.ToLower() == "asc" ? lstT_MenuBar.OrderBy(p => p.DisplayValue) : lstT_MenuBar.OrderByDescending(p => p.DisplayValue);
        if(sortBy.Contains("."))
            return isAsc.ToLower() == "asc" ? lstT_MenuBar.Sort(sortBy,true) : lstT_MenuBar.Sort(sortBy,false);
        ParameterExpression paramExpression = Expression.Parameter(typeof(T_MenuBar), "t_menubar");
        MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
        LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
        return (IQueryable<T_MenuBar>)lstT_MenuBar.Provider.CreateQuery(
                   Expression.Call(
                       typeof(Queryable),
                       methodName,
                       new Type[] { lstT_MenuBar.ElementType, lambda.Body.Type },
                       lstT_MenuBar.Expression,
                       lambda));
    }
/// <summary>Searches for the similar records (Match Making).</summary>
    ...
    public ActionResult SetFSearch(string searchString, string HostingEntity,string t_menubarmenuitemassociation, bool? RenderPartial,  string FsearchId = "", bool ShowDeleted = false)
    {
        int Qcount = 0;
        ...
        {
            FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            var T_MenuBarMenuItemAssociationlist = db.T_MenuItems.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            SetFSearchDropdownViewBag(T_MenuBarMenuItemAssociationlist, t_menubarmenuitemassociation, "T_MenuItem", "Menu Item", "t_menubarmenuitemassociation");
        }
        else
        {
            var objT_MenuBarMenuItemAssociation = new List<T_MenuItem>();
            ViewBag.t_menubarmenuitemassociation = new SelectList(objT_MenuBarMenuItemAssociation, "ID", "DisplayValue");
        }
        SetFSearchViewBag("T_MenuBar");
        ViewBag.HideColumns = new MultiSelectList(EntityColumns(), "Key", "Value");
        ViewBag.FsearchId = FsearchId;
        return View(new T_MenuBar());
    }
    ...
    [Audit("FacetedSearch")]
//public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_menubarmenuitemassociation ,string T_AutoNoFrom,string T_AutoNoTo,string T_Disabled,string T_Horizontal,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab, bool? IsFilter=false, bool ShowDeleted = false, string ExportType = null)
    public ActionResult FSearch(T_MenuBarIndexArgsOption args)
    {
        //FSearchViewBag(currentFilter, searchString, FSFilter, sortBy, isAsc, page, itemsPerPage, search, FilterCondition, HostingEntity, AssociatedType, HostingEntityID, viewtype, SortOrder, HideColumns, GroupByColumn, IsReports, IsdrivedTab, IsFilter,"T_MenuBar");
        IndexViewBag(args);
        ...
        {
            lstT_MenuBar  = searchRecords(lstT_MenuBar, model.searchString.ToUpper(),true);
        }
        ...
        if(!string.IsNullOrEmpty(model.search))
        {
            ...
            lstT_MenuBar = searchRecords(lstT_MenuBar, model.search,true);
        }
        if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
        {
        ...
        lstT_MenuBar = CustomSorting(lstT_MenuBar,model.HostingEntity,model.AssociatedType,model.sortBy,model.IsAsc);
        //lstT_MenuBar = lstT_MenuBar;
        if(!string.IsNullOrEmpty(model.FilterCondition))
        {
            ...
            foreach(var cond in conditions)
            {
                ...
                if(iCnt == conditions.Count())
                    LogicalConnector = "";
                model.SearchResult += " " + GetPropertyDP("T_MenuBar", PropertyName) + " " + Operator + " " + Value.Trim() + " " + LogicalConnector;
                whereCondition.Append(conditionFSearch("T_MenuBar",PropertyName, Operator, Value.Trim(),type) + LogicalConnector);
                iCnt++;
            }
            if(!string.IsNullOrEmpty(whereCondition.ToString()))
                lstT_MenuBar = lstT_MenuBar.Where(whereCondition.ToString());
        ...
        if(!string.IsNullOrEmpty(DataOrdering))
            lstT_MenuBar = Sorting.Sort<T_MenuBar>(lstT_MenuBar, DataOrdering);
        ...
        if(model.T_AutoNoFrom!=null || model.T_AutoNoTo !=null)
        {
            try
            {
                ...
                _T_MenuBar =  _T_MenuBar.Where(o => o.T_AutoNo >= from && o.T_AutoNo <= to);
                model.SearchResult += "\r\n Auto No.= "+from+"-"+to;
            }
            catch(Exception ex)
            {
        ...
        if(model.T_Disabled!=null)
        {
            try
            {
                bool boolvalue = Convert.ToBoolean(model.T_Disabled);
                _T_MenuBar =  _T_MenuBar.Where(o => o.T_Disabled == boolvalue);
                model.SearchResult += "\r\n Disabled?= "+model.T_Disabled;
            }
            catch(Exception ex)
            {
                Elmah.ErrorSignal.FromCurrentContext().Raise(ex);
        ...
        if(model.T_Horizontal!=null)
        {
            try
            {
                bool boolvalue = Convert.ToBoolean(model.T_Horizontal);
                _T_MenuBar =  _T_MenuBar.Where(o => o.T_Horizontal == boolvalue);
                model.SearchResult += "\r\n Horizontal?= "+model.T_Horizontal;
            }
            catch(Exception ex)
            {
                Elmah.ErrorSignal.FromCurrentContext().Raise(ex);
        ...
        if(model.t_menubarmenuitemassociation != null)
        {
            ...
            foreach(var str in ids)
            {
                ...
                if(!string.IsNullOrEmpty(str))
                {
                    ...
                    else
                    {
                        ...
                        model.SearchResult += db.T_MenuItems.Find(Convert.ToInt64(str)).DisplayValue + ", ";
            ...
            ids1 = ids1.ToList();
            _T_MenuBar = _T_MenuBar.Where(p => ids1.Contains(p.Id));
        }
        _T_MenuBar = FilterByHostingEntity(_T_MenuBar, model.HostingEntity, model.AssociatedType, Convert.ToInt32(model.HostingEntityID));
        if(model.viewtype == "Metric")
        {
            ...
            if(!string.IsNullOrEmpty(model.GroupByColumn))
            {
                ...
                var entityModel = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuBar");
                ...
                if(model.GroupByColumn.ToLower().Contains("displayvalue") == true)
                    if(aggregate != "Count")
                        result = _T_MenuBar.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key.DisplayValue as X, it."+aggregate+"(Value) as Y)").OrderBy("X");
                    else
                        result = _T_MenuBar.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key.DisplayValue as X, it."+aggregate+"() as Y)").OrderBy("X");
                else if(aggregate != "Count")
                    result = _T_MenuBar.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key." + model.GroupByColumn + " as X, it." + aggregate + "(Value) as Y)").OrderBy("X");
                else
                    result = _T_MenuBar.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key." + model.GroupByColumn + " as X, it." + aggregate + "() as Y)").OrderBy("X");
            ...
            if(aggregate!= "Count")
            {
                var result = _T_MenuBar.Aggration(aggregateproperty, aggregate);
                var entityName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuBar");
            ...
            return Json(_T_MenuBar.Count(), "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
        ...
        if(!Request.IsAjaxRequest())
        {
            ...
            if(!string.IsNullOrEmpty(model.GroupByColumn))
                foreach(var item in list)
                {
                    ...
                    item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_MenuBar", tagsSplit.ToArray());
                }
            model.list = list;
    ...
    ///
    /// <param name="_T_MenuBar">IQueryable<T_MenuBar>.</param>
    /// <param name="HostingEntity">Name of Hosting Entity.</param>
    /// <param name="AssociatedType">Association Name.</param>
    /// <param name="HostingEntityID">Id of Hosting entity.</param>
    ///
    /// <returns>Modified LINQ IQueryable<T_MenuBar>.</returns>
    private IQueryable<T_MenuBar> FilterByHostingEntity(IQueryable<T_MenuBar> _T_MenuBar, string HostingEntity, string AssociatedType, int? HostingEntityID)
    {
        if(HostingEntity == "T_MenuItem" && AssociatedType == "T_MenuBarMenuItemAssociation_T_MenuItem")
        {
            if(HostingEntityID != null)
            {
                ...
                var ids = db.T_MenuBarMenuItemAssociations.AsNoTracking().Where(p => p.T_MenuItemID == hostid).Select(p=>p.T_MenuBarID).ToList();
                _T_MenuBar = _T_MenuBar.Where(p => ids.Contains(p.Id));
    ...
    public string GetDisplayValue(string id)
    {
        ...
        {
            var _Obj = appcontext.T_MenuBars.FirstOrDefault(p => p.Id == idvalue);
    ...
    public Dictionary<long, string> GetDisplayValuePivotMatrix(string EntityName, string caller)
    {
        ...
        using(var appcontext = (new ApplicationContext(new SystemUser(), true)))
        {
            IQueryable<T_MenuBar> list = appcontext.T_MenuBars;
            FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_MenuBar>(new SystemUser(), list, "T_MenuBar", caller);
    ...
    public IQueryable<T_MenuBar> GetIQueryable(ApplicationContext appdb)
    {
        return appdb.T_MenuBars.AsNoTracking();
    }
    ...
    public IQueryable<JournalEntry> GetExtraJournalEntry(int? id, Models.IUser user, JournalEntryContext jedb, string RelatedEntityRecords)
    {
        ...
        {
            predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_MenuBar" &&  d.RecordId == id && d.PropertyName == "Menu Bar Menu Item Association" && d.Type != "View" && d.Type != "ViewList"));
        }
    ...
    public object GetRecordById(string id)
    {
        ...
        using(var appcontext = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = appcontext.T_MenuBars.Find(Convert.ToInt64(id));
            return _Obj;
        }
    }
    ...
    public IQueryable<long> SearchRecords(string searchkey, List<long> RecordIds)
    {
        ...
        {
            appcontext.Configuration.LazyLoadingEnabled = false;
            var list = appcontext.T_MenuBars.Where(p => RecordIds.Contains(p.Id));
    ...
    {
        if(string.IsNullOrEmpty(id)) return Json(new T_MenuBar(), JsonRequestBehavior.AllowGet); ;
        using(var appcontext = (new ApplicationContext(new SystemUser(), true)))
        {
            appcontext.Configuration.LazyLoadingEnabled = false;
            var _Obj = appcontext.T_MenuBars.Find(Convert.ToInt64(id));
            long? tenantId = null;
            var _updatedAssocObj = UpdateAssociationValue("T_MenuBar", _Obj, tenantId);
    ...
    public string GetRecordById_Reflection(string id)
    {
        ...
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = context.T_MenuBars.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_MenuBar>(_Obj, "T_MenuBar", new string[] { ""  });
        }
    }
    /// <summary>Creates a JSON result with the given data as its content.</summary>
    ///
    ...
    {
        IQueryable<T_MenuBar> list = db.T_MenuBars;
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<T_MenuBar>(User, list, "T_MenuBar", caller);
        var data = from x in list.OrderBy(q => q.DisplayValue).ToList()
                   select new { Id = x.Id, Name = x.DisplayValue };
    ...
    {
        var list = db.T_MenuBars.OrderBy(p => p.DisplayValue).AsQueryable();//;
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<T_MenuBar>(User, list, "T_MenuBar", associationName+"ID",RestrictDropdownVal);
    ...
    {
        var list = CustomDropdownFilter(db.T_MenuBars.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, ExtraVal, RestrictDropdownVal, CustomParameter, "GetAllValue");
        var result = DropdownHelper.GetAllValue<T_MenuBar>(User, list, caller, key, AssoNameWithParent, AssociationID, ExtraVal, "T_MenuBar", RestrictDropdownVal);
    ...
    {
        var result = DropdownHelper.GetAllValueForRB<T_MenuBar>(User, db.T_MenuBars, caller, key, AssoNameWithParent, AssociationID, ExtraVal, "T_MenuBar");
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public JsonResult GetAllMultiSelectValue(string caller,string key, string AssoNameWithParent, string AssociationID, string bulkAdd = null, string CustomParameter = null)
    {
        ...
        var list = CustomDropdownFilter(db.T_MenuBars.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, null, null, CustomParameter, "GetAllMultiSelectValue");
        var result  = DropdownHelper.GetAllMultiSelectValue(User, list, key, AssoNameWithParent, AssociationID, "T_MenuBar", caller);
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public JsonResult GetMultiSelectValueAllSelection(string caller,string key, string AssoNameWithParent, string AssociationID, string bulkAdd = null, string bulkSelection = null, string CustomParameter = null)
    {
        if(caller != null)
            caller = caller.Replace("?", "");
        var list = CustomDropdownFilter(db.T_MenuBars.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, null, null, CustomParameter, "GetMultiSelectValueAllSelection");
        var result  = DropdownHelper.GetMultiSelectValueAllSelection(User, list, key, AssoNameWithParent, AssociationID, "T_MenuBar", caller);
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public ActionResult Upload(string FileType, string AssociatedType, string HostingEntityName, string HostingEntityID, string UrlReferrer,string ImportType="")
    {
        ...
        {
            if(!((CustomPrincipal)User).CanUseVerb("ImportCSV", "T_MenuBar", User) || !User.CanAdd("T_MenuBar"))
            {
                return RedirectToAction("Index", "Error");
            }
        }
        ...
        {
            if(!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_MenuBar", User) || !User.CanAdd("T_MenuBar"))
            {
                return RedirectToAction("Index", "Error");
            }
        }
        ...
        //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_MenuBar")).Count() > 0 ? true : false;
        var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_MenuBar").ToList();
        ...
        if(FileUpload != null)
        {
            ...
            if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
            {
                ...
                if(entList != null)
                {
                    ...
                    allcolumns = new string[] { "T_AutoNo","T_Name","T_Disabled","T_Roles","T_Horizontal" };
                ...
                if(!string.IsNullOrEmpty(collection["ListOfMappings"]))
                {
                    ...
                    DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new <b>Menu Bar</b>";
                ...
                for(int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    ...
                    for(int j = 0; j < sheetColumns.Count; j++)
                    {
                        ...
                        case "T_Roles":
                            ...
                            break;
                        case "T_Disabled":
                            model.T_Disabled = Boolean.Parse(columnValue);
                            break;
                        case "T_Horizontal":
                            model.T_Horizontal = Boolean.Parse(columnValue);
                            break;
                        default:
                            break;
                        }
                    ...
                    var flagUpdate = MatchUpdate.Update(model, MatchColumns, db, "T_MenuBar", mappedColumns);
                    ...
                    if(AlrtMsg == "")
                    {
                        ...
                        {
                            var result = CheckMandatoryProperties(User,db,model,"T_MenuBar");
                            var validatebr_result = GetValidateBeforeSavePropertiesDictionary(User, db, model, "OnCreate", "T_MenuBar");
    ...
    {
        string MainEntityName = "T_MenuBar";
        db.Configuration.AutoDetectChangesEnabled = false;
        db.Configuration.LazyLoadingEnabled = false;
        string FilePath = collection["hdnFilePath"];
        var columnlist = collection["hdnColumnList"];
        ...
        if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
        {
            ...
            if(objDataSet != null && objDataSet.Tables.Count > 0 && selectedlist != null && columnlist != null)
            {
                ...
                for(int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    ...
                    model = LoadObjectFromSheet(model, sheetColumns, tblColumns, i, objDataSet, AssociatedType, HostingEntityID, lstEntityProp, mappedColumns, MainEntityName,Db) as T_MenuBar;
                    var MatchColumns = collection["hdnListChkUpdate"];
                    var flagUpdate = MatchUpdate.Update(model, MatchColumns, db, MainEntityName, mappedColumns);
                    if(flagUpdate) continue;
                    //
                    var AlrtMsg = CheckBeforeSave(model as T_MenuBar, "ImportData");
                    if(AlrtMsg == "")
                    {
                        var customimport_hasissues = false;
                        if(ValidateModel(model))
                        {
                            ...
                            if((result == null || result.Count == 0) && (validatebr_result == null || validatebr_result.Count == 0))
                            {
                                ...
                                if(!CustomSaveOnImport(model as T_MenuBar, out customerror, i))
                                {
                                    if(!flagUpdate)
                                    {
                                        IEntity obj = null;
...
/// <summary>Creates a JSON result with the given data as its content.</summary>
    ...
    public object GetFieldValueByEntityId(long Id, string PropName)
    {
        try
        {
            using(var appcontext  = (new ApplicationContext(new SystemUser(),true)))
            {
                var obj1 = appcontext.T_MenuBars.Where(p=>p.Id == Id).FirstOrDefault();
    ...
    private JsonResult ApplyBusinessRuleOnSaving(T_MenuBar t_menubar)
    {
        ...
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionary(User,db,t_menubar, "OnEdit","T_MenuBar");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
            ...
            {
                var resultBR = GetMandatoryPropertiesDictionary(User,db,t_menubar, "OnEdit","T_MenuBar");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    string BRID = "";
                    foreach(var dic in resultBR)
                    {
                        ...
                        {
                            var type = t_menubar.GetType();
                            if(type.GetProperty(dic.Key) != null)
                            {
                                var propertyvalue = type.GetProperty(dic.Key).GetValue(t_menubar, null);
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User,db,t_menubar, "OnEdit","T_MenuBar");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    foreach(var dic in resultBR)
    ...
    private JsonResult ApplyBusinessRuleBefore(T_MenuBar t_menubar, bool IsReadOnlyIgnore=false)
    {
        var businessrule = User.businessrules.Where(p => p.EntityName == "T_MenuBar").ToList();
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var validateLockResult = GetLockBusinessRulesDictionary(User, db,t_menubar,"T_MenuBar").Where(p => p.Key.Contains("FailureMessage"));
            ...
            {
                var validateMandatorypropertyResult = GetReadOnlyPropertiesDictionary(User,db,t_menubar,"T_MenuBar").Where(p => p.Key.Contains("FailureMessage"));
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetReadOnlyProperties(T_MenuBar OModel,string ruleType=null)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuBar")));
        return Json(GetReadOnlyPropertiesDictionary(User, db, OModel, "T_MenuBar",ruleType), JsonRequestBehavior.AllowGet);
    }
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetMandatoryProperties(T_MenuBar OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuBar")));
        return Json(GetMandatoryPropertiesDictionary(User,db,OModel,ruleType,"T_MenuBar"), JsonRequestBehavior.AllowGet);
    }
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetUIAlertBusinessRules(T_MenuBar OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        var conditions = "";
        if(User.businessrules != null)
        {
            var BR = User.businessrules.Where(p => p.EntityName == "T_MenuBar").ToList();
            var BRAll = BR;
            ...
            if(BR != null && BR.Count > 0)
            {
                ...
                var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_MenuBar");
    ...
    ///
    /// <returns>T_MenuBar object.</returns>
    public T_MenuBar UpdateHiddenProperties(T_MenuBar OModel, string cannotview = null)
    {
        Dictionary<string, string> hiddenProperties = new Dictionary<string, string>();
        if(OModel.Id > 0 && !string.IsNullOrEmpty(cannotview))
            using(var context = (new ApplicationContext(new SystemUser(), true)))
            {
                var obj1 = context.T_MenuBars.Find(OModel.Id);
                if(obj1 != null)
                {
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetHiddenVerb(T_MenuBar OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        //var conditions = "";
        if(User.businessrules != null)
        {
            ...
            if(BR != null && BR.Count > 0)
            {
                OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuBar")));
                OModel.setCalculation();
                var ResultOfBusinessRules = db.GetHiddenVerb(OModel, BR, "T_MenuBar");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
    ...
    [HttpPost]
    [Audit(0)]
    public Dictionary<string, string> GetHiddenVerbDetails(T_MenuBar OModel, string ruleType, string pagetype = null)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        //var conditions = "";
        if(User.businessrules != null)
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetValidateBeforeSaveProperties(T_MenuBar OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuBar")));
        return Json(GetValidateBeforeSavePropertiesDictionary(User,db,OModel,ruleType,"T_MenuBar"), JsonRequestBehavior.AllowGet);
    }
    /// <summary>(An Action that handles HTTP POST requests) creates a JSON result (Before save business rule).</summary>
    ///
    /// <param name="OModel">  The model.</param>
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetValidateBeforeSavePropertiesForPopupConfirm(T_MenuBar  OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuBar")));
        return Json(GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User, db, OModel, ruleType, "T_MenuBar"), JsonRequestBehavior.AllowGet);
    }
    ...
    [Audit(0)]
    [ValidateAntiForgeryToken]
    public JsonResult GetLockBusinessRules(T_MenuBar OModel)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuBar")));
        return Json(GetLockBusinessRulesDictionary(User,db,OModel,"T_MenuBar"), JsonRequestBehavior.AllowGet);
    }
    ...
    public long? GetIdFromDisplayValue(string displayvalue)
    {
        ...
        using(ApplicationContext db1 = new ApplicationContext(new SystemUser()))
        {
            ...
            var _Obj = db1.T_MenuBars.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if(_Obj != null)
    ...
    public long? GetIdFromPropertyValue(string PropName, string PropValue)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            IQueryable query = context.T_MenuBars;
            Type[] exprArgTypes = { query.ElementType };
            string propToWhere = PropName;
            ParameterExpression p = Expression.Parameter(typeof(T_MenuBar), "p");
            ...
            if(PropValue.ToLower().Trim() != "null")
            {
                ...
                {
                    PropValue = GetIdofDisplayValueforSetIfZero(PropName, "T_MenuBar", PropValue);
                    ...
                    lambda = Expression.Lambda<Func<T_MenuBar, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue1), member.Type)), p);
                }
                else
                {
                    lambda = Expression.Lambda<Func<T_MenuBar, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
                }
            }
            else
                lambda = Expression.Lambda<Func<T_MenuBar, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_MenuBar>)q);
    ...
    public JsonResult GetPropertyValueByEntityId(long Id, string PropName, string propertyofAssociation = "")
    {
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var obj1 = context.T_MenuBars.Find(Id);
            if(obj1 == null)
    ...
    {
        var EntityInfo = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuBar");
        var AssociationInfo = EntityInfo.Associations.FirstOrDefault(p => p.AssociationProperty == PropName);
    ...
    public void SetPropertyValueByEntityId(long Id, string PropName, string value)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            T_MenuBar obj1 = context.T_MenuBars.Find(Id);
            if(obj1 != null)
            {
                ...
                if(PropName.Substring(PropName.Length - 2) == "ID")
                {
                    value = GetIdofDisplayValueforSet(PropName, "T_MenuBar", value);
    ...
    [HttpPost]
    public JsonResult Check1MThresholdValue(T_MenuBar t_menubar)
    {
    ...
    {
        string[][] groupsarr = new string[][] { new string[] {"208381","Basic Information"},new string[] {"208383","Menu Bar Display Information"} };
        return groupsarr;
    }
/// <summary>(An Action that handles HTTP POST requests) gets calculation values.</summary>
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetCalculationValues(T_MenuBar t_menubar)
    {
        t_menubar.setCalculation();
        Dictionary<string, string> Calculations = new Dictionary<string, string>();
        System.Reflection.PropertyInfo[] properties = (t_menubar.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        var EntityInfo = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuBar");
        string DataType = string.Empty;
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetDerivedDetails(T_MenuBar t_menubar, string IgnoreEditable=null, string source=null)
    {
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetDerivedDetailsInline(string host, string value, T_MenuBar t_menubar, string IgnoreEditable)
    {
        Dictionary<string, string> derivedlist = new Dictionary<string, string>();
        return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
    }
    ...
    public async Task<ActionResult> BulkDocumentDownload(string Ids,bool Isflat = false)
    {
        ...
        var t_menubars = await db.T_MenuBars.Where(s => eIds.Contains(s.Id)).ToListAsync();
        foreach(var t_menubar in t_menubars)
        {
            await AddDownloadDirectoryForEntity(dir, entityName, t_menubar);
        }
        ...
        Response.AddHeader("Content-Disposition", string.Format("attachment; filename={0}.zip", GetProperFileDirName(dir.Name) + "_" + String.Format("{0:MM-dd-yyyy HH-mm-ss-ffff}", TimeZoneInfo.ConvertTimeFromUtc(Convert.ToDateTime(DateTime.UtcNow), t_menubars.FirstOrDefault().m_Timezone))));
    ...
    {
        if(!User.CanView("T_MenuBar") || !CustomAuthorizationBeforeCreate(HostingEntityName, HostingEntityID, AssociatedType))
        {
            return RedirectToAction("Index", "Error");
        }
    ...
    {
        T_MenuBar t_menubar = null;
        if(db != null)
        {
            t_menubar = db.T_MenuBars.AsNoTracking().FirstOrDefault(r => r.Id == id);
            t_menubar.setDateTimeToClientTime();
        }
        documentName = documentName + "_" + t_menubar.DisplayValue + " " + String.Format("{0:MM-dd-yyyy HH-mm}", TimeZoneInfo.ConvertTimeFromUtc(Convert.ToDateTime(DateTime.UtcNow), t_menubar.m_Timezone));
        ...
        var result =  ValidateBeforeDocGenerated(t_menubar, documentName, outputFormat, isdownload);
        ...
        using(Stream stream = new MemoryStream(bytedata))
        {
            ...
            OnDocGenerating(document, t_menubar, documentName, outputFormat, isdownload);
            ...
            document.MailMerge.Execute(t_menubar);
            ...
            AfterDocGenerated(t_menubar, documentName, outputFormat, isdownload, outstream);
    ...
    public ActionResult ExportData(string Ids, int btnid, string Notes)
    {
        ...
        if(IsBulk)
        {
            BulkfolderPath = Path.Combine(Server.MapPath("~/Files/" + entities.FirstOrDefault(fd => fd.Name == "T_MenuBar").DisplayName + "BulkExport_" + DateTime.UtcNow.Ticks));
            Bulkzipvalue = entities.FirstOrDefault(fd => fd.Name == "T_MenuBar").DisplayName + "-BulkExport-" + DateTime.UtcNow.ToString("dd-MM-yyyy hh-mm-ss") + " UTC";
        }
        foreach(var id in lstIds)
        {
            var zipdata = db.T_MenuBars.Find(id);
            zipvalue = entities.FirstOrDefault(fd => fd.Name == "T_MenuBar").DisplayName + "-" + zipdata.DisplayValue + "-" + DateTime.UtcNow.ToString("dd-MM-yyyy hh-mm-ss") + " UTC";
            ...
            {
                folderPath = Path.Combine(Server.MapPath("~/Files/" + entities.FirstOrDefault(fd => fd.Name == "T_MenuBar").DisplayName + "_" + DateTime.UtcNow.Ticks));
            ...
            foreach(var item in KeysData)
            {
                ...
                var rootlist = db.T_MenuBars.Where(wh => wh.Id == id);
                var ds = new DataSet();
            ...
            using(var dbnew = new ApplicationContext(User))
            {
                ...
                DoAuditEntry.AddJournalEntryRecordId("T_MenuBar", User.Name, objExportDataConfiguration.T_Name, zipdata.DisplayValue, Convert.ToString(id));
