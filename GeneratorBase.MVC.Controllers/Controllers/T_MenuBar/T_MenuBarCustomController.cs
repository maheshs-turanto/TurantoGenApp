...
{
/// <summary>A controller for Menu Bar.</summary>
///
/// <remarks></remarks>
public partial class T_MenuBarController : BaseController
{
    ...
    public bool CheckBeforeDelete(T_MenuBar t_menubar)
    {
        var result = true;
    ...
    public string CheckBeforeSave(T_MenuBar t_menubar, string command ="")
    {
        var AlertMsg = "";
        // Write your logic here
    ...
    public void OnDeleting(T_MenuBar t_menubar, ApplicationContext unitdb, GeneratorBase.MVC.Models.IUser ondeletinguser)
    {
        // Write your logic here
        if(t_menubar.Id > 0)
        {
    ...
    public void OnSaving(T_MenuBar t_menubar, ApplicationContext db, GeneratorBase.MVC.Models.IUser onsavinguser)
    {
        // Write your logic here
    }
    ...
    public void AfterSave(T_MenuBar t_menubar, GeneratorBase.MVC.Models.IUser aftersaveuser, EntityState entityState)
    {
        // Write your logic here
        CacheHelper.RemoveCache("T_MenuBar");
        CacheHelper.RemoveCache("T_MenuItem");
    ...
    ///
    /// <returns>A string.</returns>
    public string ValidateBeforeDocGenerated(T_MenuBar t_menubar, string documentName, string outputFormat, bool? isdownload)
    {
        var AlertMsg = "";
        // Write your logic here
        //Make sure to assign AlertMsg with proper message
    ...
    /// <param name="outputFormat">The string.</param>
    /// <param name="isdownload">The bool.</param>
    public void OnDocGenerating(GemBox.Document.DocumentModel document, T_MenuBar t_menubar, string documentName, string outputFormat, bool? isdownload)
    {
        // Modify t_menubar before generating document
        // e.g. trim some values, add/calculate some in other fields and use in template
        // call execute merge for custom/extra fields
        //e.g.
        //Stream stream = new MemoryStream(db.Documents.FirstOrDefault(p => p.Id == t_menubar.T_Picture).Byte);
        //document.MailMerge.Execute(new { ImageField = stream });
        ////To resize and insert use following code
        //var wi = new WebImage(db.Documents.FirstOrDefault(p => p.Id == t_menubar.T_Picture).Byte);
    ...
    public void AfterDocGenerated(T_MenuBar t_menubar, string documentName, string outputFormat, bool? isdownload, Stream stream)
    {
        // Write your logic here
...
/// <summary>Custom load view data list after on create.</summary>
    ...
    private void CustomLoadViewDataListAfterOnCreate(T_MenuBar t_menubar)
    {
        var RoleList = (new GeneratorBase.MVC.Models.CustomRoleProvider()).GetAllRoles().ToList();
        RoleList.Add("All");
        ViewBag.RoleList = new SelectList(RoleList, "", "");
...
/// <summary>Custom load view data list before edit.</summary>
    ///
    /// <param name="t_menubar">The Menu Bar.</param>
    private void CustomLoadViewDataListBeforeEdit(T_MenuBar t_menubar)
    {
        var RoleList = (new GeneratorBase.MVC.Models.CustomRoleProvider()).GetAllRoles().ToList();
        RoleList.Add("All");
        ViewBag.RoleList = new SelectList(RoleList, "", "");
...
/// <summary>Custom load view data list after edit.</summary>
    ///
    /// <param name="t_menubar">The Menu Bar.</param>
    private void CustomLoadViewDataListAfterEdit(T_MenuBar t_menubar)
    {
        var RoleList = (new GeneratorBase.MVC.Models.CustomRoleProvider()).GetAllRoles().ToList();
        RoleList.Add("All");
        ViewBag.RoleList = new SelectList(RoleList, "", "");
...
/// <summary>Custom load view data list on index.</summary>
    ...
    ///
    /// <returns>True if it succeeds, false if it fails.</returns>
    private bool CustomSaveOnCreate(T_MenuBar t_menubar, out bool customcreate_hasissues, string command="")
    {
        var result = false;
...
/// <summary>Custom save on edit.</summary>
    ...
    ///
    /// <returns>True if it succeeds, false if it fails.</returns>
    private bool CustomSaveOnEdit(T_MenuBar t_menubar, out bool customsave_hasissues, string command="")
    {
        var result = false;
...
/// <summary>Custom save on import.</summary>
    ...
    ///
    /// <returns>True if it succeeds, false if it fails.</returns>
    private bool CustomSaveOnImport(T_MenuBar t_menubar, out string customerror, int i)
    {
        var result = false;
        customerror = "";
        return result;
...
/// <summary>Custom import data validate.</summary>
    ...
    ///
    /// <returns>True if it succeeds, false if it fails.</returns>
    private bool CustomDelete(T_MenuBar t_menubar, out bool customdelete_hasissues, string command="")
    {
        var result = false;
    ...
    ///
    /// <param name="list">          The IQueryable<T_MenuBar> list.</param>
    /// <param name="HostingEntity"> The hosting entity.</param>
    /// <param name="AssociatedType">Type of the associated.</param>
    /// <param name="sortBy">        Describes who sort this object.</param>
    /// <param name="isAsc">         The is ascending.</param>
    ///
    /// <returns>An IQueryable<T_MenuBar></returns>
    private IQueryable<T_MenuBar> CustomSorting(IQueryable<T_MenuBar> list, string HostingEntity, string AssociatedType, string sortBy, string isAsc, string CustomParameter = null)
    {
        IQueryable<T_MenuBar> orderedList = list;
...
/// <summary>Creates a result that redirects to the given route.</summary>
    ...
    private RedirectToRouteResult CustomRedirectUrl(T_MenuBar t_menubar,string action,string command="", string viewmode="")
    {
        // Sample Custom implemention below
        // return RedirectToAction("Index", "T_MenuBar");
        return null;
    }
    
    private IQueryable<T_MenuBar> CustomDropdownFilter(IQueryable<T_MenuBar> list, string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal, string RestrictDropdownVal, string CustomParameter, string CallFrom = null)
    {
        return list;
    }
}
