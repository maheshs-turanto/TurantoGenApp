...
{
/// <summary>A partial controller class for Export Data Log actions (helper methods and other actions).</summary>
///
/// <remarks></remarks>
public partial class T_ExportDataLogController : BaseController
{
    ...
    ///
    /// <param name="t_exportdatalog">The Export Data Log.</param>
    /// <param name="AssocType">Type of the associated.</param>
    private void LoadViewDataForCount(T_ExportDataLog t_exportdatalog, string AssocType)
    {
    }
    /// <summary>Loads view data after on edit.</summary>
    ///
    /// <param name="t_exportdatalog">The Export Data Log.</param>
    private void LoadViewDataAfterOnEdit(T_ExportDataLog t_exportdatalog)
    {
        LoadViewDataBeforeOnEdit(t_exportdatalog, false);
        CustomLoadViewDataListAfterEdit(t_exportdatalog);
    }
    /// <summary>Loads view data before on edit.</summary>
    ///
    /// <param name="t_exportdatalog">         The Export Data Log.</param>
    /// <param name="loadCustomViewData">(Optional) True to load custom view data.</param>
    private void LoadViewDataBeforeOnEdit(T_ExportDataLog t_exportdatalog, bool loadCustomViewData = true)
    {
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        var _objT_ExportDataConfigurationExportDataLogAssociation = new List<T_ExportDataConfiguration>();
        _objT_ExportDataConfigurationExportDataLogAssociation.Add(t_exportdatalog.t_exportdataconfigurationexportdatalogassociation);
        ViewBag.T_ExportDataConfigurationExportDataLogAssociationID = new SelectList(_objT_ExportDataConfigurationExportDataLogAssociation, "ID", "DisplayValue", t_exportdatalog.T_ExportDataConfigurationExportDataLogAssociationID);
        var _objT_AssociatedExportDataLogStatus = new List<T_ExportDataLogstatus>();
        _objT_AssociatedExportDataLogStatus.Add(t_exportdatalog.t_associatedexportdatalogstatus);
        ViewBag.T_AssociatedExportDataLogStatusID = new SelectList(_objT_AssociatedExportDataLogStatus, "ID", "DisplayValue", t_exportdatalog.T_AssociatedExportDataLogStatusID);
        ViewBag.T_ExportDataLogIsHiddenRule = checkHidden(User,"T_ExportDataLog", "OnEdit", false,null);
        ViewBag.T_ExportDataLogIsGroupsHiddenRule = checkHidden(User,"T_ExportDataLog", "OnEdit", true,null);
        ViewBag.T_ExportDataLogIsSetValueUIRule = checkSetValueUIRule(User,"T_ExportDataLog", "OnEdit",new long[] { 6, 8 },null,null);
        ViewBag.T_ExportDataLogRestrictDropdownValueRule = RestrictDropdownValueRule(User, "T_ExportDataLog", "OnEdit", new long[] { 6, 8 }, null, new string[] { "" });
        var DocumentTemplates = db.T_DocumentTemplates.Where(p => p.T_EntityName == "T_ExportDataLog" && !p.T_Disable.Value).OrderBy(p=>p.T_DisplayOrder).ToList();
        ...
        var ExportData = db.T_ExportDataConfigurations.Where(p => p.T_EntityName == "T_ExportDataLog" && !p.T_Disable.Value).OrderBy(p => p.Id).ToList();
        ...
        if(loadCustomViewData) CustomLoadViewDataListBeforeEdit(t_exportdatalog);
    }
    /// <summary>Loads view data after on create.</summary>
    ///
    /// <param name="t_exportdatalog">The Export Data Log.</param>
    private void LoadViewDataAfterOnCreate(T_ExportDataLog t_exportdatalog)
    {
        ViewBag.T_ExportDataConfigurationExportDataLogAssociationID = new SelectList(db.T_ExportDataConfigurations.Where(p => p.Id == t_exportdatalog.T_ExportDataConfigurationExportDataLogAssociationID), "ID", "DisplayValue", t_exportdatalog.T_ExportDataConfigurationExportDataLogAssociationID);
        ViewBag.T_AssociatedExportDataLogStatusID = new SelectList(db.T_ExportDataLogstatuss.Where(p => p.Id == t_exportdatalog.T_AssociatedExportDataLogStatusID), "ID", "DisplayValue", t_exportdatalog.T_AssociatedExportDataLogStatusID);
        CustomLoadViewDataListAfterOnCreate(t_exportdatalog);
        ViewBag.T_ExportDataLogIsHiddenRule = checkHidden(User,"T_ExportDataLog", "OnCreate", false,null);
        ViewBag.T_ExportDataLogIsGroupsHiddenRule = checkHidden(User,"T_ExportDataLog", "OnCreate", true,null);
        ViewBag.T_ExportDataLogIsSetValueUIRule = checkSetValueUIRule(User,"T_ExportDataLog", "OnCreate",new long[] { 6, 7},null,null);
        ViewBag.T_ExportDataLogRestrictDropdownValueRule = RestrictDropdownValueRule(User, "T_ExportDataLog", "OnCreate", new long[] { 6, 7 }, null, new string[] { "" });
    }
    ...
    {
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        if(HostingEntityName == "T_ExportDataConfiguration" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_ExportDataConfigurationExportDataLogAssociation")
        {
            var hostid = Convert.ToInt64(HostingEntityID);
            var objList = db.T_ExportDataConfigurations.Where(p=>p.Id==hostid).ToList();
            ViewBag.T_ExportDataConfigurationExportDataLogAssociationID = new SelectList(objList, "ID", "DisplayValue",HostingEntityID);
            ViewBag.DisplayVal = objList.FirstOrDefault().DisplayValue;
        }
        else
        {
            var objT_ExportDataConfigurationExportDataLogAssociation = new List<T_ExportDataConfiguration>();
            ViewBag.T_ExportDataConfigurationExportDataLogAssociationID = new SelectList(objT_ExportDataConfigurationExportDataLogAssociation, "ID", "DisplayValue");
            //
        }
        if(HostingEntityName == "T_ExportDataLogstatus" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_AssociatedExportDataLogStatus")
        {
            var hostid = Convert.ToInt64(HostingEntityID);
            var objList = db.T_ExportDataLogstatuss.Where(p=>p.Id==hostid).ToList();
            ViewBag.T_AssociatedExportDataLogStatusID = new SelectList(objList, "ID", "DisplayValue",HostingEntityID);
            ViewBag.DisplayVal = objList.FirstOrDefault().DisplayValue;
        }
        else
        {
            var objT_AssociatedExportDataLogStatus = new List<T_ExportDataLogstatus>();
            ViewBag.T_AssociatedExportDataLogStatusID = new SelectList(objT_AssociatedExportDataLogStatus, "ID", "DisplayValue");
            //
        }
        ViewBag.T_ExportDataLogIsHiddenRule = checkHidden(User,"T_ExportDataLog", "OnCreate", false,null);
        ViewBag.T_ExportDataLogIsGroupsHiddenRule = checkHidden(User,"T_ExportDataLog", "OnCreate", true,null);
    ...
    public ActionResult DoExport(T_ExportDataLogIndexViewModel model, IQueryable<T_ExportDataLog> _T_ExportDataLog)
    {
        if(!((CustomPrincipal)User).CanUseVerb("ExportExcel", "T_ExportDataLog", User) || !User.CanView("T_ExportDataLog"))
        {
        ...
        {
            model.pageNumber = 1;
            if(_T_ExportDataLog.Count() > 0)
                model.PageSize = _T_ExportDataLog.Count();
            var csvdata = _T_ExportDataLog.ToCachedPagedList(model.pageNumber, model.PageSize);
            csvdata.ToList().ForEach(fr => fr.setDateTimeToClientTime());
            csvdata.ToList().ForEach(fr => fr.ApplyHiddenRule(User.businessrules, "T_ExportDataLog"));
            return new CsvResult<T_ExportDataLog>(csvdata.ToList(), "Export Data Log.csv", EntityColumns().Select(s => s.Value).ToArray(), User, new string[] {  });
        }
        else
        {
            model.pageNumber = 1;
            if(_T_ExportDataLog.Count() > 0)
                model.PageSize = _T_ExportDataLog.Count();
            return DownloadExcel(_T_ExportDataLog.ToCachedPagedList(model.pageNumber, model.PageSize).ToList());
        }
    }
    public ActionResult DoBulkOperations(T_ExportDataLogIndexViewModel model, IQueryable<T_ExportDataLog> _T_ExportDataLog, IQueryable<T_ExportDataLog> lstT_ExportDataLog)
    {
        if(model.BulkOperation != null && (model.BulkOperation == "single" || model.BulkOperation == "multiple"))
        {
            ...
            {
                FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
                _T_ExportDataLog = _fad.FilterDropdown<T_ExportDataLog>(User,  _T_ExportDataLog, "T_ExportDataLog", model.caller);
            }
            if(Convert.ToBoolean(model.BulkAssociate))
            {
                if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
                {
                    model.list = sortRecords(lstT_ExportDataLog.Except(_T_ExportDataLog),model.sortBy,model.IsAsc).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
                else
                {
                    model.list = lstT_ExportDataLog.Except(_T_ExportDataLog).OrderByDescending(c => c.Id).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
            }
            else
            {
                if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
                {
                    model.list = _T_ExportDataLog.ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation",model);
                }
                else
                {
                    model.list =_T_ExportDataLog.OrderByDescending(c => c.Id).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
            }
        }
    ...
    public FileResult DownloadExcel(List<GeneratorBase.MVC.Models.T_ExportDataLog> query)
    {
        List<string> lstHiddenProp = new List<string>();
        foreach(var item in query)
        {
            item.setDateTimeToClientTime();
            //item.ApplyHiddenRule(User.businessrules, "T_ExportDataLog");
            lstHiddenProp = item.ApplyHiddenRule(User.businessrules, "T_ExportDataLog");
        }
        ...
        DataTable dt = ExcelExportHelper.ToDataTable<T_ExportDataLog>(User, query,"T_ExportDataLog");
        dt.AcceptChanges();
        string fileName = "Export Data Log.xlsx";
        using(ClosedXML.Excel.XLWorkbook wb = new ClosedXML.Excel.XLWorkbook())
        {
            ...
            wb.Properties.Title = "Export Data Log";
            wb.Properties.Subject = "Export Data Log data";
    ...
    private Dictionary<string,string> EntityColumns()
    {
        ...
        columns.Add("3", modelproperties.FirstOrDefault(q => q.Name == "T_ExportDataConfigurationExportDataLogAssociationID").DisplayName);
        columns.Add("4", modelproperties.FirstOrDefault(q => q.Name == "T_Tag").DisplayName);
        columns.Add("5", modelproperties.FirstOrDefault(q => q.Name == "T_Notes").DisplayName);
        columns.Add("6", modelproperties.FirstOrDefault(q => q.Name == "T_AssociatedExportDataLogStatusID").DisplayName);
        return columns;
    }
    
    /// <summary>Append search conditions in IQueryable.</summary>
    ///
    /// <param name="lstT_ExportDataLog">The list Export Data Log.</param>
    /// <param name="searchString">The search string.</param>
    /// <param name="IsDeepSearch">Is deep search.</param>
    ///
    /// <returns>The found records.</returns>
    private IQueryable<T_ExportDataLog> searchRecords(IQueryable<T_ExportDataLog> lstT_ExportDataLog, string searchString, bool? IsDeepSearch)
    {
        searchString = searchString.Trim();
        if(Convert.ToBoolean(IsDeepSearch))
        {
            lstT_ExportDataLog = lstT_ExportDataLog.Where(s => (s.T_AutoNo != null && SqlFunctions.StringConvert((double)s.T_AutoNo).Contains(searchString)) ||(s.t_exportdataconfigurationexportdatalogassociation!= null && (s.t_exportdataconfigurationexportdatalogassociation.DisplayValue.ToUpper().Contains(searchString))) ||(!String.IsNullOrEmpty(s.T_Tag) && s.T_Tag.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Notes) && s.T_Notes.ToUpper().Contains(searchString)) ||(s.t_associatedexportdatalogstatus!= null && (s.t_associatedexportdatalogstatus.DisplayValue.ToUpper().Contains(searchString))) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        }
        else
            lstT_ExportDataLog = lstT_ExportDataLog.Where(s => (s.T_AutoNo != null && SqlFunctions.StringConvert((double)s.T_AutoNo).Contains(searchString)) ||(s.t_exportdataconfigurationexportdatalogassociation!= null && (s.t_exportdataconfigurationexportdatalogassociation.DisplayValue.ToUpper().Contains(searchString))) ||(!String.IsNullOrEmpty(s.T_Tag) && s.T_Tag.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Notes) && s.T_Notes.ToUpper().Contains(searchString)) ||(s.t_associatedexportdatalogstatus!= null && (s.t_associatedexportdatalogstatus.DisplayValue.ToUpper().Contains(searchString))) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        return lstT_ExportDataLog;
    }
    /// <summary>Order by list on column.</summary>
    ///
    /// <param name="lstT_ExportDataLog">The IQueryable list Export Data Log.</param>
    /// <param name="sortBy">      Column used to sort list.</param>
    /// <param name="isAsc">       Is ascending.</param>
    ///
    /// <returns>The sorted records.</returns>
    private IQueryable<T_ExportDataLog> sortRecords(IQueryable<T_ExportDataLog> lstT_ExportDataLog, string sortBy, string isAsc)
    {
        string methodName = "";
        ...
        if(sortBy == "T_ExportDataConfigurationExportDataLogAssociationID")
            return isAsc.ToLower() == "asc" ? lstT_ExportDataLog.OrderBy(p => p.t_exportdataconfigurationexportdatalogassociation.DisplayValue) : lstT_ExportDataLog.OrderByDescending(p => p.t_exportdataconfigurationexportdatalogassociation.DisplayValue);
        if(sortBy == "T_AssociatedExportDataLogStatusID")
            return isAsc.ToLower() == "asc" ? lstT_ExportDataLog.OrderBy(p => p.t_associatedexportdatalogstatus.DisplayValue) : lstT_ExportDataLog.OrderByDescending(p => p.t_associatedexportdatalogstatus.DisplayValue);
        if(sortBy == "DisplayValue")
            return isAsc.ToLower() == "asc" ? lstT_ExportDataLog.OrderBy(p => p.DisplayValue) : lstT_ExportDataLog.OrderByDescending(p => p.DisplayValue);
        if(sortBy.Contains("."))
            return isAsc.ToLower() == "asc" ? lstT_ExportDataLog.Sort(sortBy,true) : lstT_ExportDataLog.Sort(sortBy,false);
        ParameterExpression paramExpression = Expression.Parameter(typeof(T_ExportDataLog), "t_exportdatalog");
        MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
        LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
        return (IQueryable<T_ExportDataLog>)lstT_ExportDataLog.Provider.CreateQuery(
                   Expression.Call(
                       typeof(Queryable),
                       methodName,
                       new Type[] { lstT_ExportDataLog.ElementType, lambda.Body.Type },
                       lstT_ExportDataLog.Expression,
                       lambda));
    }
/// <summary>Searches for the similar records (Match Making).</summary>
    ...
    [Audit("KanBan")]
    /// <summary>Returns Kan ban View.</summary>
    ///
    /// <param name="currentFilter">  A filter specifying the current.</param>
    /// <param name="searchString">   The search string.</param>
    /// <param name="sortBy">         Describes column for sorting.</param>
    /// <param name="isAsc">          Is ascending.</param>
    ...
    /// <param name="IsExport">       Is export to excel.</param>
    ...
    /// <param name="Wfsearch">       The workflow search.</param>
    ...
    /// <param name="IsHomeList">     Is Home List.</param>
    ...
    public ActionResult KanBan(string currentFilter, string searchString, string sortBy, string isAsc, int? page, int? itemsPerPage, string HostingEntity, int? HostingEntityID, string AssociatedType, bool? IsExport, bool? IsDeepSearch, bool? IsFilter, bool? RenderPartial, string BulkOperation, string parent, string Wfsearch, string caller, bool? BulkAssociate, string viewtype, string isMobileHome, bool? IsHomeList, bool IsDivRender = false, bool ShowDeleted = false)
    {
        ...
        if(HostingEntity == "T_ExportDataLogstatus" && AssociatedType == "T_AssociatedExportDataLogStatus")
            dictionary = db.T_ExportDataLogstatuss.OrderByDescending(c => c.Id).Select(p => p.Id);
    ...
    public ActionResult SetFSearch(string searchString, string HostingEntity,string t_exportdataconfigurationexportdatalogassociation,string t_associatedexportdatalogstatus, bool? RenderPartial,  string FsearchId = "", bool ShowDeleted = false)
    {
        int Qcount = 0;
        ...
        {
            FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            var T_ExportDataConfigurationExportDataLogAssociationlist = _fad.FilterDropdown<T_ExportDataConfiguration>(User, db.T_ExportDataConfigurations, "T_ExportDataConfiguration", null);
            //SetFSearchDropdownViewBag(T_ExportDataConfigurationExportDataLogAssociationlist, t_exportdataconfigurationexportdatalogassociation, "T_ExportDataConfiguration", "Export Data Configuration", "t_exportdataconfigurationexportdatalogassociation");
            var T_AssociatedExportDataLogStatuslist = _fad.FilterDropdown<T_ExportDataLogstatus>(User, db.T_ExportDataLogstatuss, "T_ExportDataLogstatus", null);
            //SetFSearchDropdownViewBag(T_AssociatedExportDataLogStatuslist, t_associatedexportdatalogstatus, "T_ExportDataLogstatus", "Status", "t_associatedexportdatalogstatus");
        }
        else
        {
            var objT_ExportDataConfigurationExportDataLogAssociation = new List<T_ExportDataConfiguration>();
            ViewBag.t_exportdataconfigurationexportdatalogassociation = new SelectList(objT_ExportDataConfigurationExportDataLogAssociation, "ID", "DisplayValue");
            var objT_AssociatedExportDataLogStatus = new List<T_ExportDataLogstatus>();
            ViewBag.t_associatedexportdatalogstatus = new SelectList(objT_AssociatedExportDataLogStatus, "ID", "DisplayValue");
        }
        SetFSearchViewBag("T_ExportDataLog");
        ViewBag.HideColumns = new MultiSelectList(EntityColumns(), "Key", "Value");
        ViewBag.FsearchId = FsearchId;
        return View(new T_ExportDataLog());
    }
    ...
    [Audit("FacetedSearch")]
//public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_exportdataconfigurationexportdatalogassociation,string t_associatedexportdatalogstatus ,string T_AutoNoFrom,string T_AutoNoTo,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab, bool? IsFilter=false, bool ShowDeleted = false, string ExportType = null)
    public ActionResult FSearch(T_ExportDataLogIndexArgsOption args)
    {
        //FSearchViewBag(currentFilter, searchString, FSFilter, sortBy, isAsc, page, itemsPerPage, search, FilterCondition, HostingEntity, AssociatedType, HostingEntityID, viewtype, SortOrder, HideColumns, GroupByColumn, IsReports, IsdrivedTab, IsFilter,"T_ExportDataLog");
        IndexViewBag(args);
        ...
        {
            lstT_ExportDataLog  = searchRecords(lstT_ExportDataLog, model.searchString.ToUpper(),true);
        }
        ...
        if(!string.IsNullOrEmpty(model.search))
        {
            ...
            lstT_ExportDataLog = searchRecords(lstT_ExportDataLog, model.search,true);
        }
        if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
        {
        ...
        lstT_ExportDataLog = CustomSorting(lstT_ExportDataLog,model.HostingEntity,model.AssociatedType,model.sortBy,model.IsAsc);
        //lstT_ExportDataLog = lstT_ExportDataLog.IncludeOptimized(t=>t.t_exportdataconfigurationexportdatalogassociation).IncludeOptimized(t=>t.t_associatedexportdatalogstatus);
        if(!string.IsNullOrEmpty(model.FilterCondition))
        {
            ...
            foreach(var cond in conditions)
            {
                ...
                if(iCnt == conditions.Count())
                    LogicalConnector = "";
                model.SearchResult += " " + GetPropertyDP("T_ExportDataLog", PropertyName) + " " + Operator + " " + Value.Trim() + " " + LogicalConnector;
                whereCondition.Append(conditionFSearch("T_ExportDataLog",PropertyName, Operator, Value.Trim(),type) + LogicalConnector);
                iCnt++;
            }
            if(!string.IsNullOrEmpty(whereCondition.ToString()))
                lstT_ExportDataLog = lstT_ExportDataLog.Where(whereCondition.ToString());
        ...
        if(!string.IsNullOrEmpty(DataOrdering))
            lstT_ExportDataLog = Sorting.Sort<T_ExportDataLog>(lstT_ExportDataLog, DataOrdering);
        ...
        if(model.T_AutoNoFrom!=null || model.T_AutoNoTo !=null)
        {
            try
            {
                ...
                _T_ExportDataLog =  _T_ExportDataLog.Where(o => o.T_AutoNo >= from && o.T_AutoNo <= to);
                model.SearchResult += "\r\n Auto No.= "+from+"-"+to;
            }
            catch(Exception ex)
            {
        ...
        //if (lstT_ExportDataLog.Where(p => p.t_exportdataconfigurationexportdatalogassociation != null).Count() <= 50)
        //ViewBag.t_exportdataconfigurationexportdatalogassociation = new SelectList(lstT_ExportDataLog.Where(p => p.t_exportdataconfigurationexportdatalogassociation != null).Select(P => P.t_exportdataconfigurationexportdatalogassociation).Distinct(), "ID", "DisplayValue");
        if(model.t_exportdataconfigurationexportdatalogassociation != null)
        {
            var ids = model.t_exportdataconfigurationexportdatalogassociation.Split(",".ToCharArray());
            ...
            ids1 = ids1.ToList();
            _T_ExportDataLog = _T_ExportDataLog.Where(p => ids1.Contains(p.T_ExportDataConfigurationExportDataLogAssociationID));
        }
        //if (lstT_ExportDataLog.Where(p => p.t_associatedexportdatalogstatus != null).Count() <= 50)
        //ViewBag.t_associatedexportdatalogstatus = new SelectList(lstT_ExportDataLog.Where(p => p.t_associatedexportdatalogstatus != null).Select(P => P.t_associatedexportdatalogstatus).Distinct(), "ID", "DisplayValue");
        if(model.t_associatedexportdatalogstatus != null)
        {
            var ids = model.t_associatedexportdatalogstatus.Split(",".ToCharArray());
            List<long?> ids1 = new List<long?>();
            model.SearchResult += "\r\n Status= ";
            foreach(var str in ids)
            {
                ...
                if(!string.IsNullOrEmpty(str))
                {
                    ...
                    {
                        ids1.Add(Convert.ToInt64(str));
                        var obj = db.T_ExportDataLogstatuss.Find(Convert.ToInt64(str));
                        model.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                    }
                }
                //
            ...
            ids1 = ids1.ToList();
            _T_ExportDataLog = _T_ExportDataLog.Where(p => ids1.Contains(p.T_AssociatedExportDataLogStatusID));
        }
        _T_ExportDataLog = FilterByHostingEntity(_T_ExportDataLog, model.HostingEntity, model.AssociatedType, Convert.ToInt32(model.HostingEntityID));
        if(model.viewtype == "Metric")
        {
            ...
            if(!string.IsNullOrEmpty(model.GroupByColumn))
            {
                ...
                var entityModel = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ExportDataLog");
                ...
                if(model.GroupByColumn.ToLower().Contains("displayvalue") == true)
                    if(aggregate != "Count")
                        result = _T_ExportDataLog.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key.DisplayValue as X, it."+aggregate+"(Value) as Y)").OrderBy("X");
                    else
                        result = _T_ExportDataLog.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key.DisplayValue as X, it."+aggregate+"() as Y)").OrderBy("X");
                else if(aggregate != "Count")
                    result = _T_ExportDataLog.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key." + model.GroupByColumn + " as X, it." + aggregate + "(Value) as Y)").OrderBy("X");
                else
                    result = _T_ExportDataLog.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key." + model.GroupByColumn + " as X, it." + aggregate + "() as Y)").OrderBy("X");
            ...
            if(aggregate!= "Count")
            {
                var result = _T_ExportDataLog.Aggration(aggregateproperty, aggregate);
                var entityName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ExportDataLog");
            ...
            return Json(_T_ExportDataLog.Count(), "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
        ...
        if(!Request.IsAjaxRequest())
        {
            ...
            if(!string.IsNullOrEmpty(model.GroupByColumn))
                foreach(var item in list)
                {
                    ...
                    item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_ExportDataLog", tagsSplit.ToArray());
                }
            model.list = list;
    ...
    ///
    /// <param name="_T_ExportDataLog">IQueryable<T_ExportDataLog>.</param>
    /// <param name="HostingEntity">Name of Hosting Entity.</param>
    /// <param name="AssociatedType">Association Name.</param>
    /// <param name="HostingEntityID">Id of Hosting entity.</param>
    ///
    /// <returns>Modified LINQ IQueryable<T_ExportDataLog>.</returns>
    private IQueryable<T_ExportDataLog> FilterByHostingEntity(IQueryable<T_ExportDataLog> _T_ExportDataLog, string HostingEntity, string AssociatedType, int? HostingEntityID)
    {
        if(HostingEntity == "T_ExportDataConfiguration" && AssociatedType == "T_ExportDataConfigurationExportDataLogAssociation")
        {
            if(HostingEntityID != null)
            {
                ...
                _T_ExportDataLog = _T_ExportDataLog.Where(p => p.T_ExportDataConfigurationExportDataLogAssociationID == hostid);
                ViewBag.HostingEntityIDData = db.T_ExportDataConfigurations.FirstOrDefault(fd => fd.Id == hostid);
            }
            else
                _T_ExportDataLog = _T_ExportDataLog.Where(p => p.T_ExportDataConfigurationExportDataLogAssociationID == null);
        }
        if(HostingEntity == "T_ExportDataLogstatus" && AssociatedType == "T_AssociatedExportDataLogStatus")
        {
            if(HostingEntityID != null)
            {
                long hostid = Convert.ToInt64(HostingEntityID);
                _T_ExportDataLog = _T_ExportDataLog.Where(p => p.T_AssociatedExportDataLogStatusID == hostid);
                ViewBag.HostingEntityIDData = db.T_ExportDataLogstatuss.FirstOrDefault(fd => fd.Id == hostid);
            }
            else
                _T_ExportDataLog = _T_ExportDataLog.Where(p => p.T_AssociatedExportDataLogStatusID == null);
        }
        return _T_ExportDataLog;
    }
    ...
    public string GetDisplayValue(string id)
    {
        ...
        {
            var _Obj = appcontext.T_ExportDataLogs.FirstOrDefault(p => p.Id == idvalue);
    ...
    public Dictionary<long, string> GetDisplayValuePivotMatrix(string EntityName, string caller)
    {
        ...
        using(var appcontext = (new ApplicationContext(new SystemUser(), true)))
        {
            IQueryable<T_ExportDataLog> list = appcontext.T_ExportDataLogs;
            FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_ExportDataLog>(new SystemUser(), list, "T_ExportDataLog", caller);
    ...
    public IQueryable<T_ExportDataLog> GetIQueryable(ApplicationContext appdb)
    {
        return appdb.T_ExportDataLogs.AsNoTracking();
    }
    ...
    public object GetRecordById(string id)
    {
        ...
        using(var appcontext = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = appcontext.T_ExportDataLogs.Find(Convert.ToInt64(id));
            return _Obj;
        }
    }
    ...
    public IQueryable<long> SearchRecords(string searchkey, List<long> RecordIds)
    {
        ...
        {
            appcontext.Configuration.LazyLoadingEnabled = false;
            var list = appcontext.T_ExportDataLogs.Where(p => RecordIds.Contains(p.Id));
    ...
    {
        if(string.IsNullOrEmpty(id)) return Json(new T_ExportDataLog(), JsonRequestBehavior.AllowGet); ;
        using(var appcontext = (new ApplicationContext(new SystemUser(), true)))
        {
            appcontext.Configuration.LazyLoadingEnabled = false;
            var _Obj = appcontext.T_ExportDataLogs.Find(Convert.ToInt64(id));
            long? tenantId = null;
            var _updatedAssocObj = UpdateAssociationValue("T_ExportDataLog", _Obj, tenantId);
    ...
    public string GetRecordById_Reflection(string id)
    {
        ...
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = context.T_ExportDataLogs.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_ExportDataLog>(_Obj, "T_ExportDataLog", new string[] { ""  });
        }
    }
    /// <summary>Creates a JSON result with the given data as its content.</summary>
    ///
    ...
    {
        IQueryable<T_ExportDataLog> list = db.T_ExportDataLogs;
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<T_ExportDataLog>(User, list, "T_ExportDataLog", caller);
        var data = from x in list.OrderBy(q => q.DisplayValue).ToList()
                   select new { Id = x.Id, Name = x.DisplayValue };
    ...
    {
        var list = db.T_ExportDataLogs.OrderBy(p => p.DisplayValue).AsQueryable();//;
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<T_ExportDataLog>(User, list, "T_ExportDataLog", associationName+"ID",RestrictDropdownVal);
    ...
    {
        var list = CustomDropdownFilter(db.T_ExportDataLogs.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, ExtraVal, RestrictDropdownVal, CustomParameter, "GetAllValue");
        var result = DropdownHelper.GetAllValue<T_ExportDataLog>(User, list, caller, key, AssoNameWithParent, AssociationID, ExtraVal, "T_ExportDataLog", RestrictDropdownVal);
    ...
    {
        var result = DropdownHelper.GetAllValueForRB<T_ExportDataLog>(User, db.T_ExportDataLogs, caller, key, AssoNameWithParent, AssociationID, ExtraVal, "T_ExportDataLog");
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public JsonResult GetAllMultiSelectValue(string caller,string key, string AssoNameWithParent, string AssociationID, string bulkAdd = null, string CustomParameter = null)
    {
        ...
        var list = CustomDropdownFilter(db.T_ExportDataLogs.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, null, null, CustomParameter, "GetAllMultiSelectValue");
        var result  = DropdownHelper.GetAllMultiSelectValue(User, list, key, AssoNameWithParent, AssociationID, "T_ExportDataLog", caller);
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public JsonResult GetMultiSelectValueAllSelection(string caller,string key, string AssoNameWithParent, string AssociationID, string bulkAdd = null, string bulkSelection = null, string CustomParameter = null)
    {
        if(caller != null)
            caller = caller.Replace("?", "");
        var list = CustomDropdownFilter(db.T_ExportDataLogs.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, null, null, CustomParameter, "GetMultiSelectValueAllSelection");
        var result  = DropdownHelper.GetMultiSelectValueAllSelection(User, list, key, AssoNameWithParent, AssociationID, "T_ExportDataLog", caller);
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    private string GetDisplayValueForImport(string AssociatedType, string HostingEntityID, string columnValue)
    {
        ...
        {
        case "T_ExportDataConfigurationExportDataLogAssociationID":
            columnValue = db.T_ExportDataConfigurations.FirstOrDefault(p => p.Id == id).DisplayValue;
            break;
        case "T_AssociatedExportDataLogStatusID":
            columnValue = db.T_ExportDataLogstatuss.FirstOrDefault(p => p.Id == id).DisplayValue;
            break;
        default:
            break;
        }
    ...
    public ActionResult Upload(string FileType, string AssociatedType, string HostingEntityName, string HostingEntityID, string UrlReferrer,string ImportType="")
    {
        ...
        {
            if(!((CustomPrincipal)User).CanUseVerb("ImportCSV", "T_ExportDataLog", User) || !User.CanAdd("T_ExportDataLog"))
            {
                return RedirectToAction("Index", "Error");
            }
        }
        ...
        {
            if(!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_ExportDataLog", User) || !User.CanAdd("T_ExportDataLog"))
            {
                return RedirectToAction("Index", "Error");
            }
        }
        ...
        //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_ExportDataLog")).Count() > 0 ? true : false;
        var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_ExportDataLog").ToList();
    ...
    {
        string typeName = "T_ExportDataLog";
        if(FileUpload != null)
        {
            ...
            if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
            {
                ...
                if(entList != null)
                {
                    ...
                    allcolumns = new string[] { "T_AutoNo","T_ExportDataConfigurationExportDataLogAssociationID","T_Tag","T_Notes","T_AssociatedExportDataLogStatusID" };
                ...
                if(!string.IsNullOrEmpty(collection["ListOfMappings"]))
                {
                    ...
                    DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new <b>Export Data Log</b>";
                    ...
                    if(entList != null)
                    {
                        ...
                        foreach(var association in entList.Associations)
                        {
                            ...
                            for(int i = 0; i < uniqueCols.Rows.Count; i++)
                            {
                                ...
                                {
                                case "T_ExportDataConfigurationExportDataLogAssociationID":
                                    var t_exportdataconfigurationexportdatalogassociationId = db.T_ExportDataConfigurations.FirstOrDefault(p => p.DisplayValue == assovalue);
                                    if(t_exportdataconfigurationexportdatalogassociationId == null)
                                        uniqueassoValues.Add(assovalue);
                                    break;
                                case "T_AssociatedExportDataLogStatusID":
                                    var t_associatedexportdatalogstatusId = db.T_ExportDataLogstatuss.FirstOrDefault(p => p.DisplayValue == assovalue);
                                    if(t_associatedexportdatalogstatusId == null)
                                        uniqueassoValues.Add(assovalue);
                                    break;
                                default:
                                    break;
            ...
            if(entList != null)
            {
                ...
                foreach(var association in entList.Associations)
                {
                    ...
                    for(int i = 0; i < uniqueCols.Rows.Count; i++)
                    {
                        ...
                        {
                        case "T_ExportDataConfigurationExportDataLogAssociationID":
                            var strPropertyT_ExportDataConfigurationExportDataLogAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_ExportDataConfigurationExportDataLogAssociationID").Value;
                            ModelReflector.Property propT_ExportDataConfigurationExportDataLogAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ExportDataConfiguration").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_ExportDataConfigurationExportDataLogAssociation);
                            var t_exportdataconfigurationexportdatalogassociationId = tenantDb.T_ExportDataConfigurations.Where(propT_ExportDataConfigurationExportDataLogAssociation.Name + "=(@0)", assovalue).FirstOrDefault();
                            //var t_exportdataconfigurationexportdatalogassociationId = tenantDb.T_ExportDataConfigurations.Where(propT_ExportDataConfigurationExportDataLogAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
                            if(t_exportdataconfigurationexportdatalogassociationId == null)
                                uniqueassoValues.Add(assovalue);
                            break;
                        case "T_AssociatedExportDataLogStatusID":
                            var strPropertyT_AssociatedExportDataLogStatus = lstEntityProp.FirstOrDefault(p => p.Key == "T_AssociatedExportDataLogStatusID").Value;
                            ModelReflector.Property propT_AssociatedExportDataLogStatus = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ExportDataLogstatus").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_AssociatedExportDataLogStatus);
                            var t_associatedexportdatalogstatusId = tenantDb.T_ExportDataLogstatuss.Where(propT_AssociatedExportDataLogStatus.Name + "=(@0)", assovalue).FirstOrDefault();
                            //var t_associatedexportdatalogstatusId = tenantDb.T_ExportDataLogstatuss.Where(propT_AssociatedExportDataLogStatus.Name+"=\""+assovalue+"\"").FirstOrDefault();
                            if(t_associatedexportdatalogstatusId == null)
                                uniqueassoValues.Add(assovalue);
                            break;
                        default:
        ...
        if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
        {
            ...
            if(objDataSet != null && objDataSet.Tables.Count > 0 && selectedlist != null && columnlist != null)
            {
                ...
                for(int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    if(AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                        continue;
                    T_ExportDataLog model = new T_ExportDataLog();
                    ...
                    for(int j = 0; j < sheetColumns.Count; j++)
                    {
                        ...
                        case "T_AutoNo":
                            ...
                            break;
                        case "T_Tag":
                            model.T_Tag = columnValue;
                            break;
                        case "T_Notes":
                            model.T_Notes = columnValue;
                            break;
                        case "T_ExportDataConfigurationExportDataLogAssociationID":
                            dynamic t_exportdataconfigurationexportdatalogassociationId = null;
                            if(AssociatedType != null && AssociatedType == "T_ExportDataConfigurationExportDataLogAssociation" && HostingEntityID != null)
                            {
                                long id = Convert.ToInt64(HostingEntityID);
                                model.T_ExportDataConfigurationExportDataLogAssociationID = id;
                            }
                            else
                            {
                                if(lstEntityProp.Count > 0)
                                {
                                    var strPropertyT_ExportDataConfigurationExportDataLogAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_ExportDataConfigurationExportDataLogAssociationID").Value;
                                    ModelReflector.Property propT_ExportDataConfigurationExportDataLogAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ExportDataConfiguration").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_ExportDataConfigurationExportDataLogAssociation);
                                    if(propT_ExportDataConfigurationExportDataLogAssociation.Name == "DisplayValue")
                                        t_exportdataconfigurationexportdatalogassociationId = tenantDb.T_ExportDataConfigurations.AsNoTracking().FirstOrDefault(p=>p.DisplayValue == columnValue);
                                    else
                                        t_exportdataconfigurationexportdatalogassociationId = tenantDb.T_ExportDataConfigurations.AsNoTracking().Where(propT_ExportDataConfigurationExportDataLogAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
                                }
                                else
                                    t_exportdataconfigurationexportdatalogassociationId = tenantDb.T_ExportDataConfigurations.AsNoTracking().FirstOrDefault(p=>p.DisplayValue == columnValue);
                            }
                            if(t_exportdataconfigurationexportdatalogassociationId != null)
                                model.T_ExportDataConfigurationExportDataLogAssociationID = t_exportdataconfigurationexportdatalogassociationId.Id;
                            else
                            {
                                if((collection["T_ExportDataConfigurationExportDataLogAssociationID"] != null) && (collection["T_ExportDataConfigurationExportDataLogAssociationID"].ToString() == "true,false"))
                                {
                                    try
                                    {
                                        ...
                                        model.T_ExportDataConfigurationExportDataLogAssociationID = objT_ExportDataConfiguration.Id;
                                    }
                                    catch
                                    {
                                        continue;
                            ...
                            break;
                        case "T_AssociatedExportDataLogStatusID":
                            dynamic t_associatedexportdatalogstatusId = null;
                            if(AssociatedType != null && AssociatedType == "T_AssociatedExportDataLogStatus" && HostingEntityID != null)
                            {
                                long id = Convert.ToInt64(HostingEntityID);
                                model.T_AssociatedExportDataLogStatusID = id;
                            }
                            else
                            {
                                if(lstEntityProp.Count > 0)
                                {
                                    var strPropertyT_AssociatedExportDataLogStatus = lstEntityProp.FirstOrDefault(p => p.Key == "T_AssociatedExportDataLogStatusID").Value;
                                    ModelReflector.Property propT_AssociatedExportDataLogStatus = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ExportDataLogstatus").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_AssociatedExportDataLogStatus);
                                    if(propT_AssociatedExportDataLogStatus.Name == "DisplayValue")
                                        t_associatedexportdatalogstatusId = tenantDb.T_ExportDataLogstatuss.AsNoTracking().FirstOrDefault(p=>p.DisplayValue == columnValue);
                                    else
                                        t_associatedexportdatalogstatusId = tenantDb.T_ExportDataLogstatuss.AsNoTracking().Where(propT_AssociatedExportDataLogStatus.Name + "=\"" + columnValue + "\"").FirstOrDefault();
                                }
                                else
                                    t_associatedexportdatalogstatusId = tenantDb.T_ExportDataLogstatuss.AsNoTracking().FirstOrDefault(p=>p.DisplayValue == columnValue);
                            }
                            if(t_associatedexportdatalogstatusId != null)
                                model.T_AssociatedExportDataLogStatusID = t_associatedexportdatalogstatusId.Id;
                            else
                            {
                                if((collection["T_AssociatedExportDataLogStatusID"] != null) && (collection["T_AssociatedExportDataLogStatusID"].ToString() == "true,false"))
                                {
                                    try
                                    {
                                        T_ExportDataLogstatus objT_ExportDataLogstatus = new T_ExportDataLogstatus();
                                        objT_ExportDataLogstatus.T_Name  = (columnValue);
                                        db.T_ExportDataLogstatuss.Add(objT_ExportDataLogstatus);
                                        try
                                        {
                                            db.SaveChanges();
                                        }
                                        ...
                                        {
                                            db.Entry(objT_ExportDataLogstatus).State = EntityState.Detached;
                                            continue;
                                        }
                                        model.T_AssociatedExportDataLogStatusID = objT_ExportDataLogstatus.Id;
                                    }
                                    catch
                                    {
                                        continue;
                    ...
                    var flagUpdate = MatchUpdate.Update(model, MatchColumns, db, "T_ExportDataLog", mappedColumns);
                    ...
                    if(AlrtMsg == "")
                    {
                        ...
                        {
                            var result = CheckMandatoryProperties(User,db,model,"T_ExportDataLog");
                            var validatebr_result = GetValidateBeforeSavePropertiesDictionary(User, db, model, "OnCreate", "T_ExportDataLog");
    ...
    {
        string MainEntityName = "T_ExportDataLog";
        db.Configuration.AutoDetectChangesEnabled = false;
        db.Configuration.LazyLoadingEnabled = false;
        string FilePath = collection["hdnFilePath"];
        var columnlist = collection["hdnColumnList"];
        ...
        if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
        {
            ...
            if(objDataSet != null && objDataSet.Tables.Count > 0 && selectedlist != null && columnlist != null)
            {
                ...
                for(int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    ...
                    model = LoadObjectFromSheet(model, sheetColumns, tblColumns, i, objDataSet, AssociatedType, HostingEntityID, lstEntityProp, mappedColumns, MainEntityName,Db) as T_ExportDataLog;
                    var MatchColumns = collection["hdnListChkUpdate"];
                    var flagUpdate = MatchUpdate.Update(model, MatchColumns, db, MainEntityName, mappedColumns);
                    if(flagUpdate) continue;
                    //
                    var AlrtMsg = CheckBeforeSave(model as T_ExportDataLog, "ImportData");
                    if(AlrtMsg == "")
                    {
                        var customimport_hasissues = false;
                        if(ValidateModel(model))
                        {
                            ...
                            if((result == null || result.Count == 0) && (validatebr_result == null || validatebr_result.Count == 0))
                            {
                                ...
                                if(!CustomSaveOnImport(model as T_ExportDataLog, out customerror, i))
                                {
                                    if(!flagUpdate)
                                    {
                                        IEntity obj = null;
...
/// <summary>Creates a JSON result with the given data as its content.</summary>
    ...
    public object GetFieldValueByEntityId(long Id, string PropName)
    {
        try
        {
            using(var appcontext  = (new ApplicationContext(new SystemUser(),true)))
            {
                var obj1 = appcontext.T_ExportDataLogs.Where(p=>p.Id == Id).FirstOrDefault();
    ...
    private JsonResult ApplyBusinessRuleOnSaving(T_ExportDataLog t_exportdatalog)
    {
        ...
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionary(User,db,t_exportdatalog, "OnEdit","T_ExportDataLog");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
            ...
            {
                var resultBR = GetMandatoryPropertiesDictionary(User,db,t_exportdatalog, "OnEdit","T_ExportDataLog");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    string BRID = "";
                    foreach(var dic in resultBR)
                    {
                        ...
                        {
                            var type = t_exportdatalog.GetType();
                            if(type.GetProperty(dic.Key) != null)
                            {
                                var propertyvalue = type.GetProperty(dic.Key).GetValue(t_exportdatalog, null);
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User,db,t_exportdatalog, "OnEdit","T_ExportDataLog");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    foreach(var dic in resultBR)
    ...
    private JsonResult ApplyBusinessRuleBefore(T_ExportDataLog t_exportdatalog, bool IsReadOnlyIgnore=false)
    {
        var businessrule = User.businessrules.Where(p => p.EntityName == "T_ExportDataLog").ToList();
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var validateLockResult = GetLockBusinessRulesDictionary(User, db,t_exportdatalog,"T_ExportDataLog").Where(p => p.Key.Contains("FailureMessage"));
            ...
            {
                var validateMandatorypropertyResult = GetReadOnlyPropertiesDictionary(User,db,t_exportdatalog,"T_ExportDataLog").Where(p => p.Key.Contains("FailureMessage"));
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetReadOnlyProperties(T_ExportDataLog OModel,string ruleType=null)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_ExportDataLog")));
        return Json(GetReadOnlyPropertiesDictionary(User, db, OModel, "T_ExportDataLog",ruleType), JsonRequestBehavior.AllowGet);
    }
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetMandatoryProperties(T_ExportDataLog OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_ExportDataLog")));
        return Json(GetMandatoryPropertiesDictionary(User,db,OModel,ruleType,"T_ExportDataLog"), JsonRequestBehavior.AllowGet);
    }
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetUIAlertBusinessRules(T_ExportDataLog OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        var conditions = "";
        if(User.businessrules != null)
        {
            var BR = User.businessrules.Where(p => p.EntityName == "T_ExportDataLog").ToList();
            var BRAll = BR;
            ...
            if(BR != null && BR.Count > 0)
            {
                ...
                var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_ExportDataLog");
    ...
    ///
    /// <returns>T_ExportDataLog object.</returns>
    public T_ExportDataLog UpdateHiddenProperties(T_ExportDataLog OModel, string cannotview = null)
    {
        Dictionary<string, string> hiddenProperties = new Dictionary<string, string>();
        if(OModel.Id > 0 && !string.IsNullOrEmpty(cannotview))
            using(var context = (new ApplicationContext(new SystemUser(), true)))
            {
                var obj1 = context.T_ExportDataLogs.Find(OModel.Id);
                if(obj1 != null)
                {
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetHiddenVerb(T_ExportDataLog OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        //var conditions = "";
        if(User.businessrules != null)
        {
            ...
            if(BR != null && BR.Count > 0)
            {
                OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_ExportDataLog")));
                OModel.setCalculation();
                var ResultOfBusinessRules = db.GetHiddenVerb(OModel, BR, "T_ExportDataLog");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
    ...
    [HttpPost]
    [Audit(0)]
    public Dictionary<string, string> GetHiddenVerbDetails(T_ExportDataLog OModel, string ruleType, string pagetype = null)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        //var conditions = "";
        if(User.businessrules != null)
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetValidateBeforeSaveProperties(T_ExportDataLog OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_ExportDataLog")));
        return Json(GetValidateBeforeSavePropertiesDictionary(User,db,OModel,ruleType,"T_ExportDataLog"), JsonRequestBehavior.AllowGet);
    }
    /// <summary>(An Action that handles HTTP POST requests) creates a JSON result (Before save business rule).</summary>
    ///
    /// <param name="OModel">  The model.</param>
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetValidateBeforeSavePropertiesForPopupConfirm(T_ExportDataLog  OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_ExportDataLog")));
        return Json(GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User, db, OModel, ruleType, "T_ExportDataLog"), JsonRequestBehavior.AllowGet);
    }
    ...
    [Audit(0)]
    [ValidateAntiForgeryToken]
    public JsonResult GetLockBusinessRules(T_ExportDataLog OModel)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_ExportDataLog")));
        return Json(GetLockBusinessRulesDictionary(User,db,OModel,"T_ExportDataLog"), JsonRequestBehavior.AllowGet);
    }
    ...
    public long? GetIdFromDisplayValue(string displayvalue, long tenantId=0)
    {
        ...
        using(ApplicationContext db1 = new ApplicationContext(new SystemUser(),0))
        {
            ...
            var _Obj = db1.T_ExportDataLogs.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if(_Obj != null)
    ...
    public long? GetIdFromPropertyValue(string PropName, string PropValue)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            IQueryable query = context.T_ExportDataLogs;
            Type[] exprArgTypes = { query.ElementType };
            string propToWhere = PropName;
            ParameterExpression p = Expression.Parameter(typeof(T_ExportDataLog), "p");
            ...
            if(PropValue.ToLower().Trim() != "null")
            {
                ...
                {
                    PropValue = GetIdofDisplayValueforSetIfZero(PropName, "T_ExportDataLog", PropValue);
                    ...
                    lambda = Expression.Lambda<Func<T_ExportDataLog, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue1), member.Type)), p);
                }
                else
                {
                    lambda = Expression.Lambda<Func<T_ExportDataLog, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
                }
            }
            else
                lambda = Expression.Lambda<Func<T_ExportDataLog, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_ExportDataLog>)q);
    ...
    public JsonResult GetPropertyValueByEntityId(long Id, string PropName, string propertyofAssociation = "")
    {
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var obj1 = context.T_ExportDataLogs.Find(Id);
            if(obj1 == null)
    ...
    {
        var EntityInfo = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ExportDataLog");
        var AssociationInfo = EntityInfo.Associations.FirstOrDefault(p => p.AssociationProperty == PropName);
    ...
    public void SetPropertyValueByEntityId(long Id, string PropName, string value)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            T_ExportDataLog obj1 = context.T_ExportDataLogs.Find(Id);
            if(obj1 != null)
            {
                ...
                if(PropName.Substring(PropName.Length - 2) == "ID")
                {
                    value = GetIdofDisplayValueforSet(PropName, "T_ExportDataLog", value);
    ...
    [HttpPost]
    public JsonResult Check1MThresholdValue(T_ExportDataLog t_exportdatalog)
    {
    ...
    {
        string[] Verbsarr = new string[] { "BulkUpdate","BulkDelete","ImportExcel","ImportCSV","ExportExcel","ExportCSV","ImportExcelAdvanced","ImportCSVAdvanced","T_Purge","T_Restore","T_RestoreData","T_PurgeData" };
        return Verbsarr;
    }
    ...
    {
        string[][] groupsarr = new string[][] { new string[] {"199362","Basic Information"},new string[] {"199366","Status Information"} };
        return groupsarr;
    }
/// <summary>(An Action that handles HTTP POST requests) gets calculation values.</summary>
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetCalculationValues(T_ExportDataLog t_exportdatalog)
    {
        t_exportdatalog.setCalculation();
        Dictionary<string, string> Calculations = new Dictionary<string, string>();
        System.Reflection.PropertyInfo[] properties = (t_exportdatalog.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        var EntityInfo = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ExportDataLog");
        string DataType = string.Empty;
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetDerivedDetails(T_ExportDataLog t_exportdatalog, string IgnoreEditable=null, string source=null)
    {
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetDerivedDetailsInline(string host, string value, T_ExportDataLog t_exportdatalog, string IgnoreEditable)
    {
        Dictionary<string, string> derivedlist = new Dictionary<string, string>();
        return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
    }
    ...
    {
        var entityName = "T_ExportDataLog";
        ...
        var t_exportdatalogs = await db.T_ExportDataLogs.Where(s => eIds.Contains(s.Id)).ToListAsync();
        foreach(var t_exportdatalog in t_exportdatalogs)
        {
            await AddDownloadDirectoryForEntity(dir, entityName, t_exportdatalog);
        }
        ...
        Response.AddHeader("Content-Disposition", string.Format("attachment; filename={0}.zip", GetProperFileDirName(dir.Name) + "_" + String.Format("{0:MM-dd-yyyy HH-mm-ss-ffff}", TimeZoneInfo.ConvertTimeFromUtc(Convert.ToDateTime(DateTime.UtcNow), t_exportdatalogs.FirstOrDefault().m_Timezone))));
    ...
    {
        if(!User.CanView("T_ExportDataLog") || !CustomAuthorizationBeforeCreate(HostingEntityName, HostingEntityID, AssociatedType))
        {
            return RedirectToAction("Index", "Error");
        }
    ...
    {
        ViewBag.EntityName = "T_ExportDataLog";
        ViewBag.EntityId = id;
    ...
    public ActionResult GenerateDocument(int? id, int? DocumentId, string documentName, string outputFormat, bool? isdownload, bool? ispreview, string forcedOutput="")
    {
        ...
        if(db != null)
        {
            t_exportdatalog = db.T_ExportDataLogs.AsNoTracking().FirstOrDefault(r => r.Id == id);
            t_exportdatalog.setDateTimeToClientTime();
        }
        documentName = documentName + "_" + t_exportdatalog.DisplayValue + " " + String.Format("{0:MM-dd-yyyy HH-mm}", TimeZoneInfo.ConvertTimeFromUtc(Convert.ToDateTime(DateTime.UtcNow), t_exportdatalog.m_Timezone));
        ...
        var result =  ValidateBeforeDocGenerated(t_exportdatalog, documentName, outputFormat, isdownload);
        ...
        using(Stream stream = new MemoryStream(bytedata))
        {
            ...
            OnDocGenerating(document, t_exportdatalog, documentName, outputFormat, isdownload);
            ...
            document.MailMerge.Execute(t_exportdatalog);
            ...
            AfterDocGenerated(t_exportdatalog, documentName, outputFormat, isdownload, outstream);
    ...
    public ActionResult ExportData(int? id, int btnid)
    {
        ...
        var zipdata = db.T_ExportDataLogs.Find(id);
        ...
        var folderPath = Path.Combine(Server.MapPath("~/Files/" + entities.FirstOrDefault(fd => fd.Name == "T_ExportDataLog").DisplayName + "_" + DateTime.Now.Ticks));
        ...
        foreach(var item in KeysData)
        {
            ...
            var rootlist = db.T_ExportDataLogs.Where(wh => wh.Id == id);
            var ds = new DataSet();
        ...
        return DownloadZipFile(folderPath, entities.FirstOrDefault(fd => fd.Name == "T_ExportDataLog").DisplayName + "-" + zipdata.DisplayValue);
    }
