...
{
/// <summary>A partial controller class for Menu Item actions (helper methods and other actions).</summary>
///
/// <remarks></remarks>
public partial class T_MenuItemController : BaseController
{
    ...
    ///
    /// <param name="t_menuitem">The Menu Item.</param>
    /// <param name="AssocType">Type of the associated.</param>
    private void LoadViewDataForCount(T_MenuItem t_menuitem, string AssocType)
    {
    }
    /// <summary>Loads view data after on edit.</summary>
    ///
    /// <param name="t_menuitem">The Menu Item.</param>
    private void LoadViewDataAfterOnEdit(T_MenuItem t_menuitem)
    {
        LoadViewDataBeforeOnEdit(t_menuitem, false);
        CustomLoadViewDataListAfterEdit(t_menuitem);
    }
    /// <summary>Loads view data before on edit.</summary>
    ///
    /// <param name="t_menuitem">         The Menu Item.</param>
    /// <param name="loadCustomViewData">(Optional) True to load custom view data.</param>
    private void LoadViewDataBeforeOnEdit(T_MenuItem t_menuitem, bool loadCustomViewData = true)
    {
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        var _objT_MenuItemMenuItemAssociation = new List<T_MenuItem>();
        _objT_MenuItemMenuItemAssociation.Add(t_menuitem.t_menuitemmenuitemassociation);
        ViewBag.T_MenuItemMenuItemAssociationID = new SelectList(_objT_MenuItemMenuItemAssociation, "ID", "DisplayValue", t_menuitem.T_MenuItemMenuItemAssociationID);
        ViewBag.T_MenuItemMenuItemAssociationCount = t_menuitem.Self_t_menuitemmenuitemassociation.Count();
        ViewBag.T_MenuItemIsHiddenRule = checkHidden(User,"T_MenuItem", "OnEdit", false,null);
        ViewBag.T_MenuItemIsGroupsHiddenRule = checkHidden(User,"T_MenuItem", "OnEdit", true,null);
        ViewBag.T_MenuItemIsSetValueUIRule = checkSetValueUIRule(User,"T_MenuItem", "OnEdit",new long[] { 6, 8 },null,null);
        ViewBag.T_MenuItemRestrictDropdownValueRule = RestrictDropdownValueRule(User, "T_MenuItem", "OnEdit", new long[] { 6, 8 }, null, new string[] { "" });
        var DocumentTemplates = db.T_DocumentTemplates.Where(p => p.T_EntityName == "T_MenuItem" && !p.T_Disable.Value).OrderBy(p=>p.T_DisplayOrder).ToList();
        ...
        if(loadCustomViewData) CustomLoadViewDataListBeforeEdit(t_menuitem);
    }
    /// <summary>Loads view data after on create.</summary>
    ///
    /// <param name="t_menuitem">The Menu Item.</param>
    private void LoadViewDataAfterOnCreate(T_MenuItem t_menuitem)
    {
        ViewBag.T_MenuItemMenuItemAssociationID = new SelectList(db.T_MenuItems.Where(p => p.Id == t_menuitem.T_MenuItemMenuItemAssociationID), "ID", "DisplayValue", t_menuitem.T_MenuItemMenuItemAssociationID);
        CustomLoadViewDataListAfterOnCreate(t_menuitem);
        ViewBag.T_MenuItemIsHiddenRule = checkHidden(User,"T_MenuItem", "OnCreate", false,null);
        ViewBag.T_MenuItemIsGroupsHiddenRule = checkHidden(User,"T_MenuItem", "OnCreate", true,null);
        ViewBag.T_MenuItemIsSetValueUIRule = checkSetValueUIRule(User,"T_MenuItem", "OnCreate",new long[] { 6, 7},null,null);
        ViewBag.T_MenuItemRestrictDropdownValueRule = RestrictDropdownValueRule(User, "T_MenuItem", "OnCreate", new long[] { 6, 7 }, null, new string[] { "" });
    }
    ...
    {
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        if(HostingEntityName == "T_MenuItem" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_MenuItemMenuItemAssociation")
        {
            var hostid = Convert.ToInt64(HostingEntityID);
            var objList = db.T_MenuItems.Where(p=>p.Id==hostid).ToList();
            ViewBag.T_MenuItemMenuItemAssociationID = new SelectList(objList, "ID", "DisplayValue",HostingEntityID);
            ViewBag.DisplayVal = objList.FirstOrDefault().DisplayValue;
        }
        else
        {
            var objT_MenuItemMenuItemAssociation = new List<T_MenuItem>();
            ViewBag.T_MenuItemMenuItemAssociationID = new SelectList(objT_MenuItemMenuItemAssociation, "ID", "DisplayValue");
            //
        }
        if(IsBulkUpdate)
            ViewBag.T_MenuItemIsHiddenRule = checkHidden(User,"T_MenuItem", "OnEdit", false,null);
    ...
    public ActionResult DoExport(T_MenuItemIndexViewModel model, IQueryable<T_MenuItem> _T_MenuItem)
    {
        if(!((CustomPrincipal)User).CanUseVerb("ExportExcel", "T_MenuItem", User) || !User.CanView("T_MenuItem"))
        {
        ...
        {
            model.pageNumber = 1;
            if(_T_MenuItem.Count() > 0)
                model.PageSize = _T_MenuItem.Count();
            var csvdata = _T_MenuItem.ToCachedPagedList(model.pageNumber, model.PageSize);
            csvdata.ToList().ForEach(fr => fr.setDateTimeToClientTime());
            csvdata.ToList().ForEach(fr => fr.ApplyHiddenRule(User.businessrules, "T_MenuItem"));
            return new CsvResult<T_MenuItem>(csvdata.ToList(), "Menu Item.csv", EntityColumns().Select(s => s.Value).ToArray(), User, new string[] {  });
        }
        else
        {
            model.pageNumber = 1;
            if(_T_MenuItem.Count() > 0)
                model.PageSize = _T_MenuItem.Count();
            return DownloadExcel(_T_MenuItem.ToCachedPagedList(model.pageNumber, model.PageSize).ToList());
        }
    }
    public ActionResult DoBulkOperations(T_MenuItemIndexViewModel model, IQueryable<T_MenuItem> _T_MenuItem, IQueryable<T_MenuItem> lstT_MenuItem)
    {
        if(model.BulkOperation != null && (model.BulkOperation == "single" || model.BulkOperation == "multiple"))
        {
            ...
            {
                FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
                _T_MenuItem = _fad.FilterDropdown<T_MenuItem>(User,  _T_MenuItem, "T_MenuItem", model.caller);
            }
            if(Convert.ToBoolean(model.BulkAssociate))
            {
                if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
                {
                    model.list = sortRecords(lstT_MenuItem.Except(_T_MenuItem),model.sortBy,model.IsAsc).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
                else
                {
                    model.list = lstT_MenuItem.Except(_T_MenuItem).OrderByDescending(c => c.Id).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
            }
            else
            {
                if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
                {
                    model.list = _T_MenuItem.ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation",model);
                }
                else
                {
                    model.list =_T_MenuItem.OrderByDescending(c => c.Id).ToCachedPagedList(model.pageNumber, model.PageSize);
                    return PartialView("BulkOperation", model);
                }
            }
        }
    ...
    public FileResult DownloadExcel(List<GeneratorBase.MVC.Models.T_MenuItem> query)
    {
        List<string> lstHiddenProp = new List<string>();
        foreach(var item in query)
        {
            item.setDateTimeToClientTime();
            //item.ApplyHiddenRule(User.businessrules, "T_MenuItem");
            lstHiddenProp = item.ApplyHiddenRule(User.businessrules, "T_MenuItem");
        }
        ...
        DataTable dt = ExcelExportHelper.ToDataTable<T_MenuItem>(User, query,"T_MenuItem");
        dt.AcceptChanges();
        string fileName = "Menu Item.xlsx";
        using(ClosedXML.Excel.XLWorkbook wb = new ClosedXML.Excel.XLWorkbook())
        {
            ...
            wb.Properties.Author = "Menu Bar Designer V1";
            wb.Properties.Title = "Menu Item";
            wb.Properties.Subject = "Menu Item data";
    ...
    private Dictionary<string,string> EntityColumns()
    {
        ...
        columns.Add("4", modelproperties.FirstOrDefault(q => q.Name == "T_ToolTip").DisplayName);
        columns.Add("5", modelproperties.FirstOrDefault(q => q.Name == "T_DisplayOrder").DisplayName);
        columns.Add("6", modelproperties.FirstOrDefault(q => q.Name == "T_ClassIcon").DisplayName);
        columns.Add("7", modelproperties.FirstOrDefault(q => q.Name == "T_MenuItemMenuItemAssociationID").DisplayName);
        columns.Add("8", modelproperties.FirstOrDefault(q => q.Name == "T_Entity").DisplayName);
        columns.Add("9", modelproperties.FirstOrDefault(q => q.Name == "T_Action").DisplayName);
        columns.Add("10", modelproperties.FirstOrDefault(q => q.Name == "T_LinkAddress").DisplayName);
        return columns;
    }
    
    /// <summary>Append search conditions in IQueryable.</summary>
    ///
    /// <param name="lstT_MenuItem">The list Menu Item.</param>
    /// <param name="searchString">The search string.</param>
    /// <param name="IsDeepSearch">Is deep search.</param>
    ///
    /// <returns>The found records.</returns>
    private IQueryable<T_MenuItem> searchRecords(IQueryable<T_MenuItem> lstT_MenuItem, string searchString, bool? IsDeepSearch)
    {
        searchString = searchString.Trim();
        if(Convert.ToBoolean(IsDeepSearch))
        {
            lstT_MenuItem = lstT_MenuItem.Where(s => (s.T_AutoNo != null && SqlFunctions.StringConvert((double)s.T_AutoNo).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_ToolTip) && s.T_ToolTip.ToUpper().Contains(searchString)) ||(s.t_menuitemmenuitemassociation!= null && (s.t_menuitemmenuitemassociation.DisplayValue.ToUpper().Contains(searchString))) ||(!String.IsNullOrEmpty(s.T_Entity) && s.T_Entity.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Action) && s.T_Action.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_LinkAddress) && s.T_LinkAddress.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_DisplayOrder) && s.T_DisplayOrder.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_ClassIcon) && s.T_ClassIcon.ToUpper().Contains(searchString)) ||s.T_MenuBarMenuItemAssociation_t_menuitem.Any(c => c.t_menubar.DisplayValue.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        }
        else
            lstT_MenuItem = lstT_MenuItem.Where(s => (s.T_AutoNo != null && SqlFunctions.StringConvert((double)s.T_AutoNo).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_ToolTip) && s.T_ToolTip.ToUpper().Contains(searchString)) ||(s.t_menuitemmenuitemassociation!= null && (s.t_menuitemmenuitemassociation.DisplayValue.ToUpper().Contains(searchString))) ||(!String.IsNullOrEmpty(s.T_Entity) && s.T_Entity.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Action) && s.T_Action.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_LinkAddress) && s.T_LinkAddress.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_DisplayOrder) && s.T_DisplayOrder.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_ClassIcon) && s.T_ClassIcon.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
        return lstT_MenuItem;
    }
    /// <summary>Order by list on column.</summary>
    ///
    /// <param name="lstT_MenuItem">The IQueryable list Menu Item.</param>
    /// <param name="sortBy">      Column used to sort list.</param>
    /// <param name="isAsc">       Is ascending.</param>
    ///
    /// <returns>The sorted records.</returns>
    private IQueryable<T_MenuItem> sortRecords(IQueryable<T_MenuItem> lstT_MenuItem, string sortBy, string isAsc)
    {
        string methodName = "";
        ...
        if(sortBy == "T_MenuItemMenuItemAssociationID")
            return isAsc.ToLower() == "asc" ? lstT_MenuItem.OrderBy(p => p.t_menuitemmenuitemassociation.DisplayValue) : lstT_MenuItem.OrderByDescending(p => p.t_menuitemmenuitemassociation.DisplayValue);
        if(sortBy == "DisplayValue")
            return isAsc.ToLower() == "asc" ? lstT_MenuItem.OrderBy(p => p.DisplayValue) : lstT_MenuItem.OrderByDescending(p => p.DisplayValue);
        if(sortBy.Contains("."))
            return isAsc.ToLower() == "asc" ? lstT_MenuItem.Sort(sortBy,true) : lstT_MenuItem.Sort(sortBy,false);
        ParameterExpression paramExpression = Expression.Parameter(typeof(T_MenuItem), "t_menuitem");
        MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
        LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
        return (IQueryable<T_MenuItem>)lstT_MenuItem.Provider.CreateQuery(
                   Expression.Call(
                       typeof(Queryable),
                       methodName,
                       new Type[] { lstT_MenuItem.ElementType, lambda.Body.Type },
                       lstT_MenuItem.Expression,
                       lambda));
    }
/// <summary>Searches for the similar records (Match Making).</summary>
    ...
    {
        if(sourceEntity == "T_MenuBarMenuItemAssociation")
        {
            var url = (Url.Action("FSearch"));
            var _Object = db.T_MenuBarMenuItemAssociations.Find(Convert.ToInt64(id));
            url += "?search=";
            var T_MenuItemMenuItemAssociation  = _Object.T_MenuItemID;
            url += "&t_menuitemmenuitemassociation=" + T_MenuItemMenuItemAssociation;
    ...
    public ActionResult SetFSearch(string searchString, string HostingEntity,string t_menuitemmenuitemassociation,string t_menubarmenuitemassociation, bool? RenderPartial,  string FsearchId = "", bool ShowDeleted = false)
    {
        int Qcount = 0;
        ...
        {
            FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            var T_MenuItemMenuItemAssociationlist = _fad.FilterDropdown<T_MenuItem>(User, db.T_MenuItems, "T_MenuItem", null);
            SetFSearchDropdownViewBag(T_MenuItemMenuItemAssociationlist, t_menuitemmenuitemassociation, "T_MenuItem", "Parent", "t_menuitemmenuitemassociation");
            var T_MenuBarMenuItemAssociationlist = db.T_MenuBars.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            SetFSearchDropdownViewBag(T_MenuBarMenuItemAssociationlist, t_menubarmenuitemassociation, "T_MenuBar", "Menu Bar", "t_menubarmenuitemassociation");
        }
        else
        {
            var objT_MenuItemMenuItemAssociation = new List<T_MenuItem>();
            ViewBag.t_menuitemmenuitemassociation = new SelectList(objT_MenuItemMenuItemAssociation, "ID", "DisplayValue");
            var objT_MenuBarMenuItemAssociation = new List<T_MenuBar>();
            ViewBag.t_menubarmenuitemassociation = new SelectList(objT_MenuBarMenuItemAssociation, "ID", "DisplayValue");
        }
        SetFSearchViewBag("T_MenuItem");
        ViewBag.HideColumns = new MultiSelectList(EntityColumns(), "Key", "Value");
        ViewBag.FsearchId = FsearchId;
        return View(new T_MenuItem());
    }
    ...
    [Audit("FacetedSearch")]
//public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_menuitemmenuitemassociation,string t_menubarmenuitemassociation ,string T_AutoNoFrom,string T_AutoNoTo,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab, bool? IsFilter=false, bool ShowDeleted = false, string ExportType = null)
    public ActionResult FSearch(T_MenuItemIndexArgsOption args)
    {
        //FSearchViewBag(currentFilter, searchString, FSFilter, sortBy, isAsc, page, itemsPerPage, search, FilterCondition, HostingEntity, AssociatedType, HostingEntityID, viewtype, SortOrder, HideColumns, GroupByColumn, IsReports, IsdrivedTab, IsFilter,"T_MenuItem");
        IndexViewBag(args);
        ...
        {
            lstT_MenuItem  = searchRecords(lstT_MenuItem, model.searchString.ToUpper(),true);
        }
        ...
        if(!string.IsNullOrEmpty(model.search))
        {
            ...
            lstT_MenuItem = searchRecords(lstT_MenuItem, model.search,true);
        }
        if(!String.IsNullOrEmpty(model.sortBy) && !String.IsNullOrEmpty(model.IsAsc))
        {
        ...
        lstT_MenuItem = CustomSorting(lstT_MenuItem,model.HostingEntity,model.AssociatedType,model.sortBy,model.IsAsc);
        //lstT_MenuItem = lstT_MenuItem.IncludeOptimized(t=>t.t_menuitemmenuitemassociation);
        if(!string.IsNullOrEmpty(model.FilterCondition))
        {
            ...
            foreach(var cond in conditions)
            {
                ...
                if(iCnt == conditions.Count())
                    LogicalConnector = "";
                model.SearchResult += " " + GetPropertyDP("T_MenuItem", PropertyName) + " " + Operator + " " + Value.Trim() + " " + LogicalConnector;
                whereCondition.Append(conditionFSearch("T_MenuItem",PropertyName, Operator, Value.Trim(),type) + LogicalConnector);
                iCnt++;
            }
            if(!string.IsNullOrEmpty(whereCondition.ToString()))
                lstT_MenuItem = lstT_MenuItem.Where(whereCondition.ToString());
        ...
        if(!string.IsNullOrEmpty(DataOrdering))
            lstT_MenuItem = Sorting.Sort<T_MenuItem>(lstT_MenuItem, DataOrdering);
        ...
        if(model.T_AutoNoFrom!=null || model.T_AutoNoTo !=null)
        {
            try
            {
                ...
                _T_MenuItem =  _T_MenuItem.Where(o => o.T_AutoNo >= from && o.T_AutoNo <= to);
                model.SearchResult += "\r\n Auto No.= "+from+"-"+to;
            }
            catch(Exception ex)
            {
        ...
        //if (lstT_MenuItem.Where(p => p.t_menuitemmenuitemassociation != null).Count() <= 50)
        //ViewBag.t_menuitemmenuitemassociation = new SelectList(lstT_MenuItem.Where(p => p.t_menuitemmenuitemassociation != null).Select(P => P.t_menuitemmenuitemassociation).Distinct(), "ID", "DisplayValue");
        if(model.t_menuitemmenuitemassociation != null)
        {
            var ids = model.t_menuitemmenuitemassociation.Split(",".ToCharArray());
            ...
            ids1 = ids1.ToList();
            _T_MenuItem = _T_MenuItem.Where(p => ids1.Contains(p.T_MenuItemMenuItemAssociationID));
        }
        if(model.t_menubarmenuitemassociation != null)
        {
            var ids = model.t_menubarmenuitemassociation.Split(",".ToCharArray());
            List<long?> ids1 = new List<long?>();
            model.SearchResult += "\r\n  Menu Bar= ";
            foreach(var str in ids)
            {
                ...
                if(!string.IsNullOrEmpty(str))
                {
                    ...
                    else
                    {
                        ...
                        model.SearchResult += db.T_MenuBars.Find(Convert.ToInt64(str)).DisplayValue + ", ";
            ...
            ids1 = ids1.ToList();
            _T_MenuItem = _T_MenuItem.Where(p => ids1.Contains(p.Id));
        }
        _T_MenuItem = FilterByHostingEntity(_T_MenuItem, model.HostingEntity, model.AssociatedType, Convert.ToInt32(model.HostingEntityID));
        if(model.viewtype == "Metric")
        {
            ...
            if(!string.IsNullOrEmpty(model.GroupByColumn))
            {
                ...
                var entityModel = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuItem");
                ...
                if(model.GroupByColumn.ToLower().Contains("displayvalue") == true)
                    if(aggregate != "Count")
                        result = _T_MenuItem.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key.DisplayValue as X, it."+aggregate+"(Value) as Y)").OrderBy("X");
                    else
                        result = _T_MenuItem.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key.DisplayValue as X, it."+aggregate+"() as Y)").OrderBy("X");
                else if(aggregate != "Count")
                    result = _T_MenuItem.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key." + model.GroupByColumn + " as X, it." + aggregate + "(Value) as Y)").OrderBy("X");
                else
                    result = _T_MenuItem.GroupBy("new(" + model.GroupByColumn + " )", "it." + aggregateproperty).Select("new (it.Key." + model.GroupByColumn + " as X, it." + aggregate + "() as Y)").OrderBy("X");
            ...
            if(aggregate!= "Count")
            {
                var result = _T_MenuItem.Aggration(aggregateproperty, aggregate);
                var entityName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuItem");
            ...
            return Json(_T_MenuItem.Count(), "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
        ...
        if(!Request.IsAjaxRequest())
        {
            ...
            if(!string.IsNullOrEmpty(model.GroupByColumn))
                foreach(var item in list)
                {
                    ...
                    item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_MenuItem", tagsSplit.ToArray());
                }
            model.list = list;
    ...
    {
        return GraphHelper.ShowGraph(db, "T_MenuItem", type, inlarge);
    }
    ...
    {
        return GraphHelper.ShowGraphEntityHome(db, "T_MenuItem", type, inlarge, XAxis);
    }
    /// <summary>Appends where clause for HostingEntity (list inside tab or accordion).</summary>
    ///
    /// <param name="_T_MenuItem">IQueryable<T_MenuItem>.</param>
    /// <param name="HostingEntity">Name of Hosting Entity.</param>
    /// <param name="AssociatedType">Association Name.</param>
    /// <param name="HostingEntityID">Id of Hosting entity.</param>
    ///
    /// <returns>Modified LINQ IQueryable<T_MenuItem>.</returns>
    private IQueryable<T_MenuItem> FilterByHostingEntity(IQueryable<T_MenuItem> _T_MenuItem, string HostingEntity, string AssociatedType, int? HostingEntityID)
    {
        if(HostingEntity == "T_MenuBar" && AssociatedType == "T_MenuBarMenuItemAssociation_T_MenuBar")
        {
            if(HostingEntityID != null)
            {
                ...
                var ids = db.T_MenuBarMenuItemAssociations.AsNoTracking().Where(p => p.T_MenuBarID == hostid).Select(p=>p.T_MenuItemID).ToList();
                _T_MenuItem = _T_MenuItem.Where(p => ids.Contains(p.Id));
        ...
        if(HostingEntity == "T_MenuItem" && AssociatedType == "T_MenuItemMenuItemAssociation")
        {
            ...
            {
                long hostid = Convert.ToInt64(HostingEntityID);
                _T_MenuItem = _T_MenuItem.Where(p => p.T_MenuItemMenuItemAssociationID == hostid);
                ViewBag.HostingEntityIDData = db.T_MenuItems.FirstOrDefault(fd => fd.Id == hostid);
            }
            else
                _T_MenuItem = _T_MenuItem.Where(p => p.T_MenuItemMenuItemAssociationID == null);
        }
        return _T_MenuItem;
    }
    ...
    public string GetDisplayValue(string id)
    {
        ...
        {
            var _Obj = appcontext.T_MenuItems.FirstOrDefault(p => p.Id == idvalue);
    ...
    public Dictionary<long, string> GetDisplayValuePivotMatrix(string EntityName, string caller)
    {
        ...
        using(var appcontext = (new ApplicationContext(new SystemUser(), true)))
        {
            IQueryable<T_MenuItem> list = appcontext.T_MenuItems;
            FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_MenuItem>(new SystemUser(), list, "T_MenuItem", caller);
    ...
    public IQueryable<T_MenuItem> GetIQueryable(ApplicationContext appdb)
    {
        return appdb.T_MenuItems.AsNoTracking();
    }
    ...
    public IQueryable<JournalEntry> GetExtraJournalEntry(int? id, Models.IUser user, JournalEntryContext jedb, string RelatedEntityRecords)
    {
        ...
        if(RelatedEntityRecords == "T_MenuItem" || string.IsNullOrEmpty(RelatedEntityRecords))
        {
            var T_MenuItemMenuItemAssociationIds = jedb.T_MenuItems.AsNoTracking().Where(p=>p.T_MenuItemMenuItemAssociationID == id).Select(p => p.Id);
            predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_MenuItem" && T_MenuItemMenuItemAssociationIds.Contains(d.RecordId) && d.Type != "View" && d.Type != "ViewList"));
        }
        if(RelatedEntityRecords == "T_MenuBarMenuItemAssociation" || string.IsNullOrEmpty(RelatedEntityRecords))
        {
            predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_MenuItem" &&  d.RecordId == id && d.PropertyName == "Menu Bar Menu Item Association" && d.Type != "View" && d.Type != "ViewList"));
        }
    ...
    public object GetRecordById(string id)
    {
        ...
        using(var appcontext = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = appcontext.T_MenuItems.Find(Convert.ToInt64(id));
            return _Obj;
        }
    }
    ...
    public IQueryable<long> SearchRecords(string searchkey, List<long> RecordIds)
    {
        ...
        {
            appcontext.Configuration.LazyLoadingEnabled = false;
            var list = appcontext.T_MenuItems.Where(p => RecordIds.Contains(p.Id));
    ...
    {
        if(string.IsNullOrEmpty(id)) return Json(new T_MenuItem(), JsonRequestBehavior.AllowGet); ;
        using(var appcontext = (new ApplicationContext(new SystemUser(), true)))
        {
            appcontext.Configuration.LazyLoadingEnabled = false;
            var _Obj = appcontext.T_MenuItems.Find(Convert.ToInt64(id));
            long? tenantId = null;
            var _updatedAssocObj = UpdateAssociationValue("T_MenuItem", _Obj, tenantId);
    ...
    public string GetRecordById_Reflection(string id)
    {
        ...
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var _Obj = context.T_MenuItems.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_MenuItem>(_Obj, "T_MenuItem", new string[] { ""  });
        }
    }
    /// <summary>Creates a JSON result with the given data as its content.</summary>
    ///
    ...
    {
        IQueryable<T_MenuItem> list = db.T_MenuItems;
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<T_MenuItem>(User, list, "T_MenuItem", caller);
        var data = from x in list.OrderBy(q => q.DisplayValue).ToList()
                   select new { Id = x.Id, Name = x.DisplayValue };
    ...
    {
        var list = db.T_MenuItems.OrderBy(p => p.DisplayValue).AsQueryable();//;
        FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        list = _fad.FilterDropdown<T_MenuItem>(User, list, "T_MenuItem", associationName+"ID",RestrictDropdownVal);
    ...
    {
        var list = CustomDropdownFilter(db.T_MenuItems.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, ExtraVal, RestrictDropdownVal, CustomParameter, "GetAllValue");
        var result = DropdownHelper.GetAllValue<T_MenuItem>(User, list, caller, key, AssoNameWithParent, AssociationID, ExtraVal, "T_MenuItem", RestrictDropdownVal);
    ...
    {
        var result = DropdownHelper.GetAllValueForRB<T_MenuItem>(User, db.T_MenuItems, caller, key, AssoNameWithParent, AssociationID, ExtraVal, "T_MenuItem");
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public JsonResult GetAllMultiSelectValue(string caller,string key, string AssoNameWithParent, string AssociationID, string bulkAdd = null, string CustomParameter = null)
    {
        ...
        var list = CustomDropdownFilter(db.T_MenuItems.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, null, null, CustomParameter, "GetAllMultiSelectValue");
        var result  = DropdownHelper.GetAllMultiSelectValue(User, list, key, AssoNameWithParent, AssociationID, "T_MenuItem", caller);
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    public JsonResult GetMultiSelectValueAllSelection(string caller,string key, string AssoNameWithParent, string AssociationID, string bulkAdd = null, string bulkSelection = null, string CustomParameter = null)
    {
        if(caller != null)
            caller = caller.Replace("?", "");
        var list = CustomDropdownFilter(db.T_MenuItems.AsNoTracking(), caller, key, AssoNameWithParent, AssociationID, null, null, CustomParameter, "GetMultiSelectValueAllSelection");
        var result  = DropdownHelper.GetMultiSelectValueAllSelection(User, list, key, AssoNameWithParent, AssociationID, "T_MenuItem", caller);
        return Json(result.Select(x => new
        {
            Id = x.Id, Name = x.DisplayValue
        }), JsonRequestBehavior.AllowGet);
    ...
    private string GetDisplayValueForImport(string AssociatedType, string HostingEntityID, string columnValue)
    {
        ...
        {
        case "T_MenuItemMenuItemAssociationID":
    ...
    public ActionResult Upload(string FileType, string AssociatedType, string HostingEntityName, string HostingEntityID, string UrlReferrer,string ImportType="")
    {
        ...
        {
            if(!((CustomPrincipal)User).CanUseVerb("ImportCSV", "T_MenuItem", User) || !User.CanAdd("T_MenuItem"))
            {
                return RedirectToAction("Index", "Error");
            }
        }
        ...
        {
            if(!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_MenuItem", User) || !User.CanAdd("T_MenuItem"))
            {
                return RedirectToAction("Index", "Error");
            }
        }
        ...
        //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_MenuItem")).Count() > 0 ? true : false;
        var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_MenuItem").ToList();
        ...
        if(FileUpload != null)
        {
            ...
            if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
            {
                ...
                if(entList != null)
                {
                    ...
                    allcolumns = new string[] { "T_AutoNo","T_Name","T_ToolTip","T_DisplayOrder","T_ClassIcon","T_MenuItemMenuItemAssociationID","T_Entity","T_Action","T_LinkAddress" };
                ...
                if(!string.IsNullOrEmpty(collection["ListOfMappings"]))
                {
                    ...
                    DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new <b>Menu Item</b>";
                    ...
                    if(entList != null)
                    {
                        ...
                        foreach(var association in entList.Associations)
                        {
                            ...
                            for(int i = 0; i < uniqueCols.Rows.Count; i++)
                            {
                                ...
                                {
                                case "T_MenuItemMenuItemAssociationID":
                                    var t_menuitemmenuitemassociationId = db.T_MenuItems.FirstOrDefault(p => p.DisplayValue == assovalue);
                                    if(t_menuitemmenuitemassociationId == null)
            ...
            if(entList != null)
            {
                ...
                foreach(var association in entList.Associations)
                {
                    ...
                    for(int i = 0; i < uniqueCols.Rows.Count; i++)
                    {
                        ...
                        {
                        case "T_MenuItemMenuItemAssociationID":
                            var strPropertyT_MenuItemMenuItemAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_MenuItemMenuItemAssociationID").Value;
                            ModelReflector.Property propT_MenuItemMenuItemAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuItem").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_MenuItemMenuItemAssociation);
                            var t_menuitemmenuitemassociationId = db.T_MenuItems.Where(propT_MenuItemMenuItemAssociation.Name + "=(@0)", assovalue).FirstOrDefault();
                            //var t_menuitemmenuitemassociationId = db.T_MenuItems.Where(propT_MenuItemMenuItemAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
                            if(t_menuitemmenuitemassociationId == null)
                                uniqueassoValues.Add(assovalue);
                            break;
                        default:
    ...
    public ActionResult ImportData(FormCollection collection)
    {
        ...
        var UrlReferrer = collection["hdnUrlReferrer"];
        var ImportType = collection["hdnImportType"];
        List<long> T_MenuItemMenuItemAssociationIds = new List<long>();
        string fileLocation = FilePath;
        //string excelConnectionString = string.Empty;
        string fileExtension = System.IO.Path.GetExtension(fileLocation).ToLower();
        ...
        if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
        {
            ...
            if(objDataSet != null && objDataSet.Tables.Count > 0 && selectedlist != null && columnlist != null)
            {
                ...
                for(int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    if(AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                        continue;
                    T_MenuItem model = new T_MenuItem();
                    ...
                    for(int j = 0; j < sheetColumns.Count; j++)
                    {
                        ...
                        case "T_ToolTip":
                            ...
                            break;
                        case "T_Entity":
                            model.T_Entity = columnValue;
                            break;
                        case "T_Action":
                            model.T_Action = columnValue;
                            break;
                        case "T_LinkAddress":
                            model.T_LinkAddress = columnValue;
                            break;
                        case "T_DisplayOrder":
                            model.T_DisplayOrder = columnValue;
                            break;
                        case "T_ClassIcon":
                            model.T_ClassIcon = columnValue;
                            break;
                        case "T_MenuItemMenuItemAssociationID":
                            dynamic t_menuitemmenuitemassociationId = null;
                            if(AssociatedType != null && AssociatedType == "T_MenuItemMenuItemAssociation" && HostingEntityID != null)
                            {
                                long id = Convert.ToInt64(HostingEntityID);
                                model.T_MenuItemMenuItemAssociationID = id;
                            }
                            else
                            {
                                if(lstEntityProp.Count > 0)
                                {
                                    var strPropertyT_MenuItemMenuItemAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_MenuItemMenuItemAssociationID").Value;
                                    ModelReflector.Property propT_MenuItemMenuItemAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuItem").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_MenuItemMenuItemAssociation);
                                    if(propT_MenuItemMenuItemAssociation.Name == "DisplayValue")
                                        t_menuitemmenuitemassociationId = db.T_MenuItems.AsNoTracking().FirstOrDefault(p=>p.DisplayValue == columnValue);
                                    else
                                        t_menuitemmenuitemassociationId = db.T_MenuItems.AsNoTracking().Where(propT_MenuItemMenuItemAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
                                }
                                else
                                    t_menuitemmenuitemassociationId = db.T_MenuItems.AsNoTracking().FirstOrDefault(p=>p.DisplayValue == columnValue);
                            }
                            if(t_menuitemmenuitemassociationId != null)
                                model.T_MenuItemMenuItemAssociationID = t_menuitemmenuitemassociationId.Id;
                            else
                            {
                                if((collection["T_MenuItemMenuItemAssociationID"] != null) && (collection["T_MenuItemMenuItemAssociationID"].ToString() == "true,false"))
                                {
                                    try
                                    {
                                        ...
                                        model.T_MenuItemMenuItemAssociationID = objT_MenuItem.Id;
                                        T_MenuItemMenuItemAssociationIds.Add(objT_MenuItem.Id);
                                    }
                                    catch
                                    {
                                        continue;
                    ...
                    var flagUpdate = MatchUpdate.Update(model, MatchColumns, db, "T_MenuItem", mappedColumns);
                    ...
                    if(AlrtMsg == "")
                    {
                        ...
                        {
                            var result = CheckMandatoryProperties(User,db,model,"T_MenuItem");
                            var validatebr_result = GetValidateBeforeSavePropertiesDictionary(User, db, model, "OnCreate", "T_MenuItem");
                ...
                foreach(var id in T_MenuItemMenuItemAssociationIds)  //code for self association import
                {
                    var obj = db.T_MenuItems.Find(id);
                    if(obj != null)
                    {
                        var displayvalue = obj.DisplayValue;
                        if(!string.IsNullOrEmpty(displayvalue))
                        {
                            var copy = db.T_MenuItems.FirstOrDefault(p => p.DisplayValue == displayvalue && p.Id != id);
                            if(copy != null)
                            {
                                foreach(var item in db.T_MenuItems.Where(p => p.T_MenuItemMenuItemAssociationID == id).ToList())
                                {
                                    item.T_MenuItemMenuItemAssociationID = copy.Id;
                                    db.Entry(item).State = EntityState.Modified;
                                    db.SaveChanges();
                                }
                                db.Entry(obj).State = EntityState.Deleted;
                                db.T_MenuItems.Remove(obj);
                                db.SaveChanges();
                            }
                        }
                    }
                }//code for self association
            }
            else
            {
    ...
    {
        string MainEntityName = "T_MenuItem";
        db.Configuration.AutoDetectChangesEnabled = false;
        db.Configuration.LazyLoadingEnabled = false;
        string FilePath = collection["hdnFilePath"];
        var columnlist = collection["hdnColumnList"];
        ...
        if(fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
        {
            ...
            if(objDataSet != null && objDataSet.Tables.Count > 0 && selectedlist != null && columnlist != null)
            {
                ...
                for(int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                {
                    ...
                    model = LoadObjectFromSheet(model, sheetColumns, tblColumns, i, objDataSet, AssociatedType, HostingEntityID, lstEntityProp, mappedColumns, MainEntityName,Db) as T_MenuItem;
                    var MatchColumns = collection["hdnListChkUpdate"];
                    var flagUpdate = MatchUpdate.Update(model, MatchColumns, db, MainEntityName, mappedColumns);
                    if(flagUpdate) continue;
                    //
                    var AlrtMsg = CheckBeforeSave(model as T_MenuItem, "ImportData");
                    if(AlrtMsg == "")
                    {
                        var customimport_hasissues = false;
                        if(ValidateModel(model))
                        {
                            ...
                            if((result == null || result.Count == 0) && (validatebr_result == null || validatebr_result.Count == 0))
                            {
                                ...
                                if(!CustomSaveOnImport(model as T_MenuItem, out customerror, i))
                                {
                                    if(!flagUpdate)
                                    {
                                        IEntity obj = null;
...
/// <summary>Creates a JSON result with the given data as its content.</summary>
    ...
    public object GetFieldValueByEntityId(long Id, string PropName)
    {
        try
        {
            using(var appcontext  = (new ApplicationContext(new SystemUser(),true)))
            {
                var obj1 = appcontext.T_MenuItems.Where(p=>p.Id == Id).FirstOrDefault();
    ...
    private JsonResult ApplyBusinessRuleOnSaving(T_MenuItem t_menuitem)
    {
        ...
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionary(User,db,t_menuitem, "OnEdit","T_MenuItem");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
            ...
            {
                var resultBR = GetMandatoryPropertiesDictionary(User,db,t_menuitem, "OnEdit","T_MenuItem");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    string BRID = "";
                    foreach(var dic in resultBR)
                    {
                        ...
                        {
                            var type = t_menuitem.GetType();
                            if(type.GetProperty(dic.Key) != null)
                            {
                                var propertyvalue = type.GetProperty(dic.Key).GetValue(t_menuitem, null);
            ...
            {
                var resultBR = GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User,db,t_menuitem, "OnEdit","T_MenuItem");
                if(resultBR.Count() > 0)
                {
                    string stringResult = "";
                    foreach(var dic in resultBR)
    ...
    private JsonResult ApplyBusinessRuleBefore(T_MenuItem t_menuitem, bool IsReadOnlyIgnore=false)
    {
        var businessrule = User.businessrules.Where(p => p.EntityName == "T_MenuItem").ToList();
        if((businessrule != null && businessrule.Count > 0))
        {
            ...
            {
                var validateLockResult = GetLockBusinessRulesDictionary(User, db,t_menuitem,"T_MenuItem").Where(p => p.Key.Contains("FailureMessage"));
            ...
            {
                var validateMandatorypropertyResult = GetReadOnlyPropertiesDictionary(User,db,t_menuitem,"T_MenuItem").Where(p => p.Key.Contains("FailureMessage"));
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetReadOnlyProperties(T_MenuItem OModel,string ruleType=null)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuItem")));
        return Json(GetReadOnlyPropertiesDictionary(User, db, OModel, "T_MenuItem",ruleType), JsonRequestBehavior.AllowGet);
    }
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetMandatoryProperties(T_MenuItem OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuItem")));
        return Json(GetMandatoryPropertiesDictionary(User,db,OModel,ruleType,"T_MenuItem"), JsonRequestBehavior.AllowGet);
    }
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetUIAlertBusinessRules(T_MenuItem OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        var conditions = "";
        if(User.businessrules != null)
        {
            var BR = User.businessrules.Where(p => p.EntityName == "T_MenuItem").ToList();
            var BRAll = BR;
            ...
            if(BR != null && BR.Count > 0)
            {
                ...
                var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_MenuItem");
    ...
    ///
    /// <returns>T_MenuItem object.</returns>
    public T_MenuItem UpdateHiddenProperties(T_MenuItem OModel, string cannotview = null)
    {
        Dictionary<string, string> hiddenProperties = new Dictionary<string, string>();
        if(OModel.Id > 0 && !string.IsNullOrEmpty(cannotview))
            using(var context = (new ApplicationContext(new SystemUser(), true)))
            {
                var obj1 = context.T_MenuItems.Find(OModel.Id);
                if(obj1 != null)
                {
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetHiddenVerb(T_MenuItem OModel, string ruleType)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        //var conditions = "";
        if(User.businessrules != null)
        {
            ...
            if(BR != null && BR.Count > 0)
            {
                OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuItem")));
                OModel.setCalculation();
                var ResultOfBusinessRules = db.GetHiddenVerb(OModel, BR, "T_MenuItem");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
    ...
    [HttpPost]
    [Audit(0)]
    public Dictionary<string, string> GetHiddenVerbDetails(T_MenuItem OModel, string ruleType, string pagetype = null)
    {
        Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
        //var conditions = "";
        if(User.businessrules != null)
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetValidateBeforeSaveProperties(T_MenuItem OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuItem")));
        return Json(GetValidateBeforeSavePropertiesDictionary(User,db,OModel,ruleType,"T_MenuItem"), JsonRequestBehavior.AllowGet);
    }
    /// <summary>(An Action that handles HTTP POST requests) creates a JSON result (Before save business rule).</summary>
    ///
    /// <param name="OModel">  The model.</param>
    ...
    [HttpPost]
    [Audit(0)]
    public JsonResult GetValidateBeforeSavePropertiesForPopupConfirm(T_MenuItem  OModel, string ruleType)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuItem")));
        return Json(GetValidateBeforeSavePropertiesDictionaryForPopupConfirm(User, db, OModel, ruleType, "T_MenuItem"), JsonRequestBehavior.AllowGet);
    }
    ...
    [Audit(0)]
    [ValidateAntiForgeryToken]
    public JsonResult GetLockBusinessRules(T_MenuItem OModel)
    {
        OModel = UpdateHiddenProperties(OModel, string.Join(",", User.CanNotView("T_MenuItem")));
        return Json(GetLockBusinessRulesDictionary(User,db,OModel,"T_MenuItem"), JsonRequestBehavior.AllowGet);
    }
    ...
    public long? GetIdFromDisplayValue(string displayvalue)
    {
        ...
        using(ApplicationContext db1 = new ApplicationContext(new SystemUser()))
        {
            ...
            var _Obj = db1.T_MenuItems.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if(_Obj != null)
    ...
    public long? GetIdFromPropertyValue(string PropName, string PropValue)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            IQueryable query = context.T_MenuItems;
            Type[] exprArgTypes = { query.ElementType };
            string propToWhere = PropName;
            ParameterExpression p = Expression.Parameter(typeof(T_MenuItem), "p");
            ...
            if(PropValue.ToLower().Trim() != "null")
            {
                ...
                {
                    PropValue = GetIdofDisplayValueforSetIfZero(PropName, "T_MenuItem", PropValue);
                    ...
                    lambda = Expression.Lambda<Func<T_MenuItem, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue1), member.Type)), p);
                }
                else
                {
                    lambda = Expression.Lambda<Func<T_MenuItem, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
                }
            }
            else
                lambda = Expression.Lambda<Func<T_MenuItem, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_MenuItem>)q);
    ...
    public JsonResult GetPropertyValueByEntityId(long Id, string PropName, string propertyofAssociation = "")
    {
        using(var context = (new ApplicationContext(new SystemUser(),true)))
        {
            ...
            var obj1 = context.T_MenuItems.Find(Id);
            if(obj1 == null)
    ...
    {
        var EntityInfo = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuItem");
        var AssociationInfo = EntityInfo.Associations.FirstOrDefault(p => p.AssociationProperty == PropName);
    ...
    public void SetPropertyValueByEntityId(long Id, string PropName, string value)
    {
        using(var context = (new ApplicationContext(new SystemUser(), true)))
        {
            T_MenuItem obj1 = context.T_MenuItems.Find(Id);
            if(obj1 != null)
            {
                ...
                if(PropName.Substring(PropName.Length - 2) == "ID")
                {
                    value = GetIdofDisplayValueforSet(PropName, "T_MenuItem", value);
    ...
    [HttpPost]
    public JsonResult Check1MThresholdValue(T_MenuItem t_menuitem)
    {
    ...
    {
        string[][] groupsarr = new string[][] { new string[] {"208384","Basic Information"},new string[] {"208385","Menu Item Display Information"},new string[] {"208571","UI Information"} };
        return groupsarr;
    }
/// <summary>(An Action that handles HTTP POST requests) gets calculation values.</summary>
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetCalculationValues(T_MenuItem t_menuitem)
    {
        t_menuitem.setCalculation();
        Dictionary<string, string> Calculations = new Dictionary<string, string>();
        System.Reflection.PropertyInfo[] properties = (t_menuitem.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        var EntityInfo = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_MenuItem");
        string DataType = string.Empty;
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetDerivedDetails(T_MenuItem t_menuitem, string IgnoreEditable=null, string source=null)
    {
    ...
    [ValidateAntiForgeryToken]
    [Audit(0)]
    public ActionResult GetDerivedDetailsInline(string host, string value, T_MenuItem t_menuitem, string IgnoreEditable)
    {
        Dictionary<string, string> derivedlist = new Dictionary<string, string>();
        return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
    }
    ...
    public async Task<ActionResult> BulkDocumentDownload(string Ids,bool Isflat = false)
    {
        ...
        var t_menuitems = await db.T_MenuItems.Where(s => eIds.Contains(s.Id)).ToListAsync();
        foreach(var t_menuitem in t_menuitems)
        {
            await AddDownloadDirectoryForEntity(dir, entityName, t_menuitem);
        }
        ...
        Response.AddHeader("Content-Disposition", string.Format("attachment; filename={0}.zip", GetProperFileDirName(dir.Name) + "_" + String.Format("{0:MM-dd-yyyy HH-mm-ss-ffff}", TimeZoneInfo.ConvertTimeFromUtc(Convert.ToDateTime(DateTime.UtcNow), t_menuitems.FirstOrDefault().m_Timezone))));
    ...
    {
        if(!User.CanView("T_MenuItem") || !CustomAuthorizationBeforeCreate(HostingEntityName, HostingEntityID, AssociatedType))
        {
            return RedirectToAction("Index", "Error");
        }
    ...
    {
        T_MenuItem t_menuitem = null;
        if(db != null)
        {
            t_menuitem = db.T_MenuItems.AsNoTracking().FirstOrDefault(r => r.Id == id);
            t_menuitem.setDateTimeToClientTime();
        }
        documentName = documentName + "_" + t_menuitem.DisplayValue + " " + String.Format("{0:MM-dd-yyyy HH-mm}", TimeZoneInfo.ConvertTimeFromUtc(Convert.ToDateTime(DateTime.UtcNow), t_menuitem.m_Timezone));
        ...
        var result =  ValidateBeforeDocGenerated(t_menuitem, documentName, outputFormat, isdownload);
        ...
        using(Stream stream = new MemoryStream(bytedata))
        {
            ...
            OnDocGenerating(document, t_menuitem, documentName, outputFormat, isdownload);
            ...
            document.MailMerge.Execute(t_menuitem);
            ...
            AfterDocGenerated(t_menuitem, documentName, outputFormat, isdownload, outstream);
    ...
    public ActionResult ExportData(string Ids, int btnid, string Notes)
    {
        ...
        if(IsBulk)
        {
            BulkfolderPath = Path.Combine(Server.MapPath("~/Files/" + entities.FirstOrDefault(fd => fd.Name == "T_MenuItem").DisplayName + "BulkExport_" + DateTime.UtcNow.Ticks));
            Bulkzipvalue = entities.FirstOrDefault(fd => fd.Name == "T_MenuItem").DisplayName + "-BulkExport-" + DateTime.UtcNow.ToString("dd-MM-yyyy hh-mm-ss") + " UTC";
        }
        foreach(var id in lstIds)
        {
            var zipdata = db.T_MenuItems.Find(id);
            zipvalue = entities.FirstOrDefault(fd => fd.Name == "T_MenuItem").DisplayName + "-" + zipdata.DisplayValue + "-" + DateTime.UtcNow.ToString("dd-MM-yyyy hh-mm-ss") + " UTC";
            ...
            {
                folderPath = Path.Combine(Server.MapPath("~/Files/" + entities.FirstOrDefault(fd => fd.Name == "T_MenuItem").DisplayName + "_" + DateTime.UtcNow.Ticks));
            ...
            foreach(var item in KeysData)
            {
                ...
                var rootlist = db.T_MenuItems.Where(wh => wh.Id == id);
                var ds = new DataSet();
            ...
            using(var dbnew = new ApplicationContext(User))
            {
                ...
                DoAuditEntry.AddJournalEntryRecordId("T_MenuItem", User.Name, objExportDataConfiguration.T_Name, zipdata.DisplayValue, Convert.ToString(id));
