...
{
/// <summary>A controller for Menu Item.</summary>
///
/// <remarks></remarks>
public partial class T_MenuItemController : BaseController
{
    ...
    public bool CheckBeforeDelete(T_MenuItem t_menuitem)
    {
        var result = true;
    ...
    public string CheckBeforeSave(T_MenuItem t_menuitem, string command ="")
    {
        var AlertMsg = "";
        // Write your logic here
    ...
    public void OnDeleting(T_MenuItem t_menuitem, ApplicationContext unitdb, GeneratorBase.MVC.Models.IUser ondeletinguser)
    {
        // Write your logic here
        if(t_menuitem.Id > 0)
        {
            using(var dbcntx = new ApplicationContext(ondeletinguser))
            {
                var menuitems = dbcntx.T_MenuBarMenuItemAssociations.Where(wh => wh.T_MenuItemID == t_menuitem.Id);
                foreach(var item in menuitems)
                {
                    dbcntx.Entry(item).State = EntityState.Deleted;
                }
    ...
    public void OnSaving(T_MenuItem t_menuitem, ApplicationContext db, GeneratorBase.MVC.Models.IUser onsavinguser)
    {
        // Write your logic here
    }
    ...
    public void AfterSave(T_MenuItem t_menuitem, GeneratorBase.MVC.Models.IUser aftersaveuser, EntityState entityState)
    {
        // Write your logic here
        CacheHelper.RemoveCache("T_MenuBar");
        CacheHelper.RemoveCache("T_MenuItem");
    ...
    ///
    /// <returns>A string.</returns>
    public string ValidateBeforeDocGenerated(T_MenuItem t_menuitem, string documentName, string outputFormat, bool? isdownload)
    {
        var AlertMsg = "";
        // Write your logic here
        //Make sure to assign AlertMsg with proper message
    ...
    /// <param name="outputFormat">The string.</param>
    /// <param name="isdownload">The bool.</param>
    public void OnDocGenerating(GemBox.Document.DocumentModel document, T_MenuItem t_menuitem, string documentName, string outputFormat, bool? isdownload)
    {
        // Modify t_menuitem before generating document
        // e.g. trim some values, add/calculate some in other fields and use in template
        // call execute merge for custom/extra fields
        //e.g.
        //Stream stream = new MemoryStream(db.Documents.FirstOrDefault(p => p.Id == t_menuitem.T_Picture).Byte);
        //document.MailMerge.Execute(new { ImageField = stream });
        ////To resize and insert use following code
        //var wi = new WebImage(db.Documents.FirstOrDefault(p => p.Id == t_menuitem.T_Picture).Byte);
    ...
    public void AfterDocGenerated(T_MenuItem t_menuitem, string documentName, string outputFormat, bool? isdownload, Stream stream)
    {
        // Write your logic here
...
/// <summary>Custom load view data list before on create.</summary>
    ...
    private void CustomLoadViewDataListBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
    {
        ...
        ViewBag.EntityList = new SelectList(EntityList, "Name", "DisplayName");
    }
/// <summary>Custom load view data list after on create.</summary>
    ///
    /// <param name="t_menuitem">The Menu Item.</param>
    
    private void CustomLoadViewDataListAfterOnCreate(T_MenuItem t_menuitem)
    {
        var EntityList = GeneratorBase.MVC.ModelReflector.Entities.Where(p => !p.IsAdminEntity && !p.IsnotificationEntity && !p.IsDefault);
        ViewBag.EntityList = new SelectList(EntityList, "Name", "DisplayName");
    }
/// <summary>Custom load view data list before edit.</summary>
    ///
    /// <param name="t_menuitem">The Menu Item.</param>
    private void CustomLoadViewDataListBeforeEdit(T_MenuItem t_menuitem)
    {
        var EntityList = GeneratorBase.MVC.ModelReflector.Entities.Where(p => !p.IsAdminEntity && !p.IsnotificationEntity && !p.IsDefault);
        ViewBag.EntityList = new SelectList(EntityList, "Name", "DisplayName");
    }
/// <summary>Custom load view data list after edit.</summary>
    ///
    /// <param name="t_menuitem">The Menu Item.</param>
    private void CustomLoadViewDataListAfterEdit(T_MenuItem t_menuitem)
    {
        var EntityList = GeneratorBase.MVC.ModelReflector.Entities.Where(p => !p.IsAdminEntity && !p.IsnotificationEntity && !p.IsDefault);
        ViewBag.EntityList = new SelectList(EntityList, "Name", "DisplayName");
    }
/// <summary>Custom load view data list on index.</summary>
    ...
    ///
    /// <returns>True if it succeeds, false if it fails.</returns>
    private bool CustomSaveOnCreate(T_MenuItem t_menuitem, out bool customcreate_hasissues, string command="")
    {
        var result = false;
...
/// <summary>Custom save on edit.</summary>
    ...
    ///
    /// <returns>True if it succeeds, false if it fails.</returns>
    private bool CustomSaveOnEdit(T_MenuItem t_menuitem, out bool customsave_hasissues, string command="")
    {
        var result = false;
...
/// <summary>Custom save on import.</summary>
    ...
    ///
    /// <returns>True if it succeeds, false if it fails.</returns>
    private bool CustomSaveOnImport(T_MenuItem t_menuitem, out string customerror, int i)
    {
        var result = false;
        customerror = "";
        return result;
...
/// <summary>Custom import data validate.</summary>
    ...
    ///
    /// <returns>True if it succeeds, false if it fails.</returns>
    private bool CustomDelete(T_MenuItem t_menuitem, out bool customdelete_hasissues, string command="")
    {
        var result = false;
    ...
    ///
    /// <param name="list">          The IQueryable<T_MenuItem> list.</param>
    /// <param name="HostingEntity"> The hosting entity.</param>
    /// <param name="AssociatedType">Type of the associated.</param>
    /// <param name="sortBy">        Describes who sort this object.</param>
    /// <param name="isAsc">         The is ascending.</param>
    ///
    /// <returns>An IQueryable<T_MenuItem></returns>
    private IQueryable<T_MenuItem> CustomSorting(IQueryable<T_MenuItem> list, string HostingEntity, string AssociatedType, string sortBy, string isAsc, string CustomParameter = null)
    {
        IQueryable<T_MenuItem> orderedList = list;
...
/// <summary>Creates a result that redirects to the given route.</summary>
    ...
    private RedirectToRouteResult CustomRedirectUrl(T_MenuItem t_menuitem,string action,string command="", string viewmode="")
    {
        // Sample Custom implemention below
        // return RedirectToAction("Index", "T_MenuItem");
        return null;
    }
    
    private IQueryable<T_MenuItem> CustomDropdownFilter(IQueryable<T_MenuItem> list, string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal, string RestrictDropdownVal, string CustomParameter, string CallFrom = null)
    {
        return list;
    }
}
